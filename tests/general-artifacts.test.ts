...
describe('Moonlight Greatsword', () => {
  ...
  it('damage scales 5d8 → 14d8 (4d8 two-handed base + 1d8 per level)', () => {
...
describe('Frostbound Returning Axe', () => {
  ...
  it('is a one-handed main-hand weapon with 3d8 → 12d8 damage (2d8 base + 1d8 per level)', () => {
    const sys = sysAt(tree, 1);
...
describe('Shadowgrave Armor', () => {
  ...
  it('uses Light Armor Artifact tables: Armor 4→8 and Evade +3→+12, both from L1', () => {
    ...
    for (let lvl = 1; lvl <= 10; lvl++) {
      ...
      expect(baseValue(tree, lvl, 'Hybrid Defense (Evade)').value).toBe(evadeTable[lvl - 1]);
    }
  });
...
describe("Lor-Keth's Staff", () => {
  ...
  it('staff damage scales 5d8 → 14d8 (4d8 two-handed base + 1d8 per level)', () => {
    for (let lvl = 1; lvl <= 10; lvl++) {
