...
export class TyhraCalendarApplication extends BaseDialog {
  ...
  private static instance: TyhraCalendarApplication | null = null;
  ...
  constructor(options: { year?: number; monthIndex?: number } = {}) {
    const localizedTitle = game.i18n?.localize('MASTERY.calendar.title') ?? 'Tyhra Calendar';
    super(
      foundry.utils.mergeObject(TyhraCalendarApplication.DEFAULT_OPTIONS, {
  ...
  static show(options: { year?: number; monthIndex?: number } = {}): void {
  ...
  protected async _prepareContext(_options: unknown): Promise<Record<string, unknown>> {
    ...
    const dayTooltip = (hasJournal: boolean) => {
      if (hasJournal) return game.i18n.localize('MASTERY.calendar.openJournal');
      if (canCreate) return game.i18n.localize('MASTERY.calendar.createJournal');
  ...
  async close(options?: Application.CloseOptions): Promise<this> {
    if (TyhraCalendarApplication.instance === this) {
      TyhraCalendarApplication.instance = null;
    }
    if (refreshCallback) refreshCallback = null;
    return super.close(options);
  }

  async #onAction(ev: Event): Promise<void> {
    ev.preventDefault();
    ...
    try {
      switch (action) {
        ...
        case 'next-hour':
          ...
          if (action === 'prev-day') await api.advanceDays(-1);
          else if (action === 'next-day') await api.advanceDays(1);
          else if (action === 'prev-hour') await api.advanceHours(-1);
          else await api.advanceHours(1);
          await this.render(false);
          break;
        case 'open-day':
        case 'open-veil': {
