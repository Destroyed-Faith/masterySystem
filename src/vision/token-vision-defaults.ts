...
export function isSightUninitialized(sight: SightLike | null | undefined): boolean {
 ...
 */
export function planPlayerSightUpdate(
  sight: SightLike | null | undefined,
  opts: { alreadyInitialized?: boolean } = {},
): Record<string, unknown> | null {
  if (opts.alreadyInitialized) return null;
  if (!isSightUninitialized(sight)) return null;
  ...
  if (!mode) update['sight.visionMode'] = PLAYER_VISION_DEFAULTS.visionMode;
  return update;
}

/** Sight block for a brand-new player character (creation data may pre-set parts). */
export function defaultPlayerSight(existing: SightLike | null | undefined): Required<SightLike> {
...
function actorTypeOf(actor: any): string {
 ...
 */
export function planPrototypeVisionUpdate(actor: any): Record<string, unknown> | null {
  if (actorTypeOf(actor) !== 'character') return null;
  ...
  const out: Record<string, unknown> = { [`flags.mastery-system.${VISION_INITIALIZED_FLAG}`]: true };
  for (const [k, v] of Object.entries(plan)) out[`prototypeToken.${k}`] = v;
  return out;
}

/** Ready-hook migration: normalize every player character once. GM only. */
export async function runPlayerVisionDefaultsMigration(actors: Iterable<any>): Promise<number> {
  const g = (globalThis as any).game;
  if (g?.user && g.user.isGM === false) return 0;
  let changed = 0;
...
export function applyPlayerVisionToCreateData(actorType: string, data: any): void {
  if (actorType !== 'character' || !data) return;
 ...
 */
export function planTokenCreateSightUpdate(actorType: string, tokenData: any): Record<string, unknown> | null {
