...
import { brighterLighting, type LightingState } from './scene-lighting.js';
...
export const LIGHT_SOURCE_PROFILES: Record<LightSourceProfileId, LightSourceProfile> = {
...
export const LIGHT_SOURCE_PROFILE_IDS: readonly LightSourceProfileId[] = ['candle', 'torch', 'lantern'];

export function isLightSourceProfileId(value: unknown): value is LightSourceProfileId {
  return typeof value === 'string' && (LIGHT_SOURCE_PROFILE_IDS as readonly string[]).includes(value);
}

/** Token / AmbientLight document `light` data for a profile (Foundry native rendering). */
export function foundryLightDataForProfile(profile: LightSourceProfile): Record<string, unknown> {
  return {
...
export function foundryLightDataOff(): Record<string, unknown> {
 ...
 */
export function lightingStateAtPoint(
  sceneState: LightingState,
  sources: readonly PointLightSource[],
  point: { x: number; y: number },
  distanceM: (a: { x: number; y: number }, b: { x: number; y: number }) => number,
): LightingState {
  ...
  for (const src of sources) {
    ...
    if (src.brightM > 0 && d <= src.brightM) {
      ...
      if (state === 'daylight') return state;
...
export function collectPointLightSources(scene: any): PointLightSource[] {
  const out: PointLightSource[] = [];
  const push = (doc: any): void => {
  ...
  for (const light of iter(scene?.lights)) push(light);
  for (const token of iter(scene?.tokens)) push(token);
  return out;
}
