...
import {
  ...
  LIGHTING_MODE_LABEL,
  LIGHTING_STATE_ICON,
  LIGHTING_STATE_LABEL,
  lightingStateForManualMode,
  type LightingMode,
} from './scene-lighting.js';
import {
  getSceneLightingMode,
  getSceneLightingStatus,
  LIGHTING_MODE_FLAG,
  setSceneLightingMode,
} from './scene-lighting-foundry.js';
import {
  foundryLightDataForProfile,
  foundryLightDataOff,
  isLightSourceProfileId,
  LIGHT_SOURCE_PROFILE_IDS,
  LIGHT_SOURCE_PROFILES,
  type LightSourceProfileId,
...
function viewedScene(): any {
  return g().canvas?.scene ?? g().game?.scenes?.viewed ?? null;
}

function modeIcon(mode: LightingMode): string {
  if (mode === 'auto') return 'fas fa-clock';
  const state = lightingStateForManualMode(mode);
  return state ? LIGHTING_STATE_ICON[state] : 'fas fa-adjust';
}

function modeTitle(mode: LightingMode, current: LightingMode): string {
  const state = lightingStateForManualMode(mode);
  const detail =
    mode === 'auto'
...
async function handleSetMode(mode: LightingMode): Promise<void> {
  if (!g().game?.user?.isGM) {
  ...
  if (status) g().ui?.notifications?.info?.(status.label);
  try {
...
function selectedTokenDocs(): any[] {
  const controlled: any[] = g().canvas?.tokens?.controlled ?? [];
  return controlled.map((t) => t?.document).filter(Boolean);
}

async function handleLightSource(profileId: LightSourceProfileId | 'off'): Promise<void> {
  ...
  if (n) g().ui?.notifications?.info?.(`${n} token(s): ${label}.`);
}

export function buildLightingSceneControl(isGM: boolean): any {
  const scene = viewedScene();
  const current = getSceneLightingMode(scene);
  const status = getSceneLightingStatus(scene);
  const tools: any[] = LIGHTING_MODES.map((mode) => ({
    name: `lighting-${mode}`,
...
export function handleLightingToolClick(toolName: string): boolean {
  if (toolName.startsWith('lighting-')) {
    const mode = toolName.slice('lighting-'.length) as LightingMode;
    if ((LIGHTING_MODES as readonly string[]).includes(mode)) {
...
export function injectSceneConfigLightingField(app: any, html: any): void {
  try {
    const root: HTMLElement | null =
      html instanceof HTMLElement ? html : (html?.[0] as HTMLElement | undefined) ?? app?.element ?? null;
    if (!root) return;
    if (root.querySelector('[name="flags.mastery-system.lightingMode"]')) return;
    const scene = app?.document ?? app?.object ?? null;
    const current = getSceneLightingMode(scene);
    const options = LIGHTING_MODES.map(
      (m) => `<option value="${m}" ${m === current ? 'selected' : ''}>${LIGHTING_MODE_LABEL[m]}${m === 'auto' ? ' (from Watch)' : ''}</option>`,
    ).join('');
    ...
    const anchor =
      form.querySelector('[name="environment.darknessLevel"], [name="darkness"]')?.closest('.form-group') ?? null;
...
export function initializeLightingSceneControls(): void {
  ...
  Hooks.on('getSceneControlButtons', (controls: any) => {
    ...
    else controls.masteryLighting = control;
  });
  Hooks.on('renderSceneConfig', (app: any, html: any) => injectSceneConfigLightingField(app, html));
}
