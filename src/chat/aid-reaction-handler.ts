...
export function listEligibleAidCharacters(skillKey: string, rollerActorId: string): any[] {
  const g = globalThis as any;
  const actors: any[] = g.game?.actors?.contents ?? [];
  return actors.filter((a: any) => {
    if (a?.type !== 'character') return false;
    if (String(a.id) === String(rollerActorId)) return false;
...
async function applyAidToMessage(message: any, assistant: any): Promise<void> {
  const g = globalThis as any;
  ...
  if (combat?.started) {
    const inCombat = (combat.combatants ?? []).some?.(
      (c: any) => String(c?.actor?.id ?? '') === String(assistant.id),
    );
    if (inCombat) {
  ...
  const aidLine =
    `<div class="breakdown-line"><span>Aid (${escape(String(assistant.name ?? 'Ally'))}):</span>` +
    `<span class="value">+${AID_BONUS}</span></div>`;
  content = content.replace(
    /<div class="breakdown-line total">/,
    `${aidLine}<div class="breakdown-line total">`,
  );
  content = content.replace(
    /(<div class="breakdown-line total">\s*<span><strong>Final Total:<\/strong><\/span>\s*<span class="value"><strong>)\d+(<\/strong><\/span>)/,
    `$1${newTotal}$2`,
  );
  if (rollResult.tn > 0) {
    content = content.replace(
      /(<span><strong>Result:<\/strong><\/span>\s*<span class="value"><strong>)[^<]+(<\/strong><\/span>)/,
      `$1${rollResult.success ? 'SUCCESS' : 'FAILURE'}$2`,
    );
  }
  ...
  g.ui?.notifications?.info?.(
    `${assistant.name} aids the check (+${AID_BONUS}). New total: ${newTotal}.`,
  );
}
...
export function registerAidReactionClickHandler(): void {
  Hooks.on('renderChatMessageHTML', (message: any, htmlRaw: HTMLElement | JQuery) => {
    try {
      const htmlEl = htmlRaw instanceof HTMLElement ? $(htmlRaw) : htmlRaw;
      const aidButton = htmlEl.find('[data-action="aid-skill-check"]');
      if (aidButton.length === 0) return;
      if (aidButton.data('handler-attached')) return;
      ...
      aidButton.off('click.aid-skill').on('click.aid-skill', async (event: JQuery.ClickEvent) => {
        ...
        const candidates = listEligibleAidCharacters(skillKey, rollerActorId).filter(
          (a: any) => a.isOwner || (game as any).user?.isGM,
        );
        ...
        const options = candidates
          .map((a: any) => `<option value="${a.id}">${escape(String(a.name))}</option>`)
          .join('');
        const picked = await new Promise<string | null>((resolve) => {
          new Dialog({
            title: `Aid — ${skillName} Check`,
            content:
              `<p>Choose the assisting character (requires ${skillName} ≥ 2 × their Mastery Rank; ` +
            ...
            buttons: {
              ok: {
                ...
                callback: (dlgHtml: JQuery) =>
                  resolve(String(dlgHtml.find('[name="aid-assistant"]').val() || '') || null),
              },
        ...
        if (!picked) return;
        const assistant = (game as any).actors?.get(picked);
        if (!assistant) return;
