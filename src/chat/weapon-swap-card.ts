...
function parseTarget(raw: string): WeaponSwapTarget | null {
  if (raw === 'unarmed') return 'unarmed';
  if (raw === '1') return 1;
  if (raw === '2') return 2;
  return null;
}
...
export function buildWeaponSwapCardHtml(actor: any): string {
...
export function buildWeaponSwapDoneHtml(actor: any, spentMovement: boolean): string {
...
export async function postWeaponSwapCard(actor: any, token?: any): Promise<void> {
...
async function resolveSwapActor(uuid: string, actorId: string): Promise<any | null> {
  ...
  for (const token of tokens) {
    if (String(token?.actor?.id || '') === actorId) return token.actor;
  }
  return null;
}
...
export function registerWeaponSwapChatHandler(): void {
  ...
  $(document)
    ...
    .on('click.msWeaponSwap', '[data-action="weapon-swap-pick"]', async (ev: any) => {
      ev.preventDefault();
      ev.stopPropagation();
      ...
      if (swapsInFlight.has(flightKey)) return;
      swapsInFlight.add(flightKey);
      card.find('[data-action="weapon-swap-pick"]').prop('disabled', true);
      try {
        const message = messageId ? (globalThis as any).game?.messages?.get?.(messageId) : null;
        const flags = message?.flags?.['mastery-system'] || {};
        if (flags.weaponSwapDone) return;
        const actor = await resolveSwapActor(
          String(btn.attr('data-actor-uuid') || flags.actorUuid || ''),
          String(btn.attr('data-actor-id') || flags.actorId || ''),
        );
        if (!actor || !canCurrentUserUpdateDocument(actor)) {
          (globalThis as any).ui?.notifications?.warn?.(loc('pickDenied', 'Only the owner can switch here.'));
