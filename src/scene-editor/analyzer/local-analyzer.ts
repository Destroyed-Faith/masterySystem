...
import type { Analyzer, AnalyzerInput, AnalyzerOutput, Hint, Point, Segment, Suggestion } from '../types.js';

export const LOCAL_ANALYZER_ID = 'local-edges';
export const LOCAL_ANALYZER_VERSION = '1.0.0';

function drawSource(image: AnalyzerInput['image'], w: number, h: number): HTMLCanvasElement | OffscreenCanvas {
  const canvas =
  ...
  if (!ctx) throw new Error('Could not open an analysis canvas.');
  ctx.drawImage(image as CanvasImageSource, 0, 0, w, h);
  return canvas;
}

function sobelMask(data: Uint8ClampedArray, w: number, h: number): { mag: Float32Array; mask: Uint8Array } {
...
function extractSegments(mask: Uint8Array, w: number, h: number, minLen: number): Segment[] {
...
function mergeCollinear(segs: Segment[], angleEps: number, distEps: number): Segment[] {
  ...
  for (let i = 0; i < segs.length; i += 1) {
    ...
    while (changed) {
      ...
      for (let j = 0; j < segs.length; j += 1) {
        ...
        const near =
          distance(cur.a, other.a) <= distEps ||
        ...
        if (dot < Math.cos(angleEps)) continue;
        let a = ends[0]!;
        let b = ends[0]!;
        let best = 0;
        for (const p of ends) {
...
function toScene(p: Point, input: AnalyzerInput, aw: number, ah: number): Point {
  return {
...
function ignoreRects(hints: Hint[]) {
  return hints.filter((h) => h.kind === 'ignore').map((h) => rectFromPoints(h.a, h.b));
}

function classifyKind(seg: Segment, hints: Hint[]): { kind: Suggestion['kind']; hintIds: string[]; assisted: boolean } {
  let best: Hint | null = null;
  let bestD = 36;
  for (const hint of hints) {
    if (hint.kind === 'ignore') continue;
    const d = Math.min(
      projectPointOnSegment(hint.a, seg).distance,
      projectPointOnSegment(hint.b, seg).distance,
      projectPointOnSegment(seg.a, { a: hint.a, b: hint.b }).distance,
    );
    if (d <= bestD) {
      bestD = d;
      best = hint;
...
export const localAnalyzer: Analyzer = {
  ...
  async analyze(input: AnalyzerInput, signal?: AbortSignal): Promise<AnalyzerOutput> {
    ...
    const ctx = (canvas as HTMLCanvasElement).getContext('2d', { willReadFrequently: true });
    if (!ctx) throw new Error('Could not read the analysis canvas.');
    ...
    for (const line of raw) {
      if (signal?.aborted) throw new DOMException('Aborted', 'AbortError');
      const sceneSeg: Segment = { a: toScene(line.a, input, aw, ah), b: toScene(line.b, input, aw, ah) };
      debugLines.push(sceneSeg);
      if (ignores.some((rect) => segmentHitsRect(sceneSeg, rect))) continue;
      if (input.region) {
        const r = input.region;
        if (!segmentHitsRect(sceneSeg, r) && !pointInRect(sceneSeg.a, r) && !pointInRect(sceneSeg.b, r)) continue;
      }
      ...
      if (tooCloseToConfirmed) continue;
...
export async function loadSceneImage(src: string): Promise<HTMLImageElement> {
