...
function buildSummaryHtml(session: EpicMasteryRollSession): string {
...
export async function postEpicMasteryRollSummary(session: EpicMasteryRollSession): Promise<void> {
  ...
  const whisper =
    visibility === 'gm'
      ? ChatMessage.getWhisperRecipients('GM').map((u: any) => u.id)
      : [];
...
export function buildEpicMasteryRollSummaryHtml(session: EpicMasteryRollSession): string {
