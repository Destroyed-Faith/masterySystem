...
export function explainXpAccount(actor: any): XpConsistencyReport {
  ...
  const historySpend: Record<string, number> = {
    attribute: 0,
    skill: 0,
    power: 0,
    artifact: 0,
  ...
  const legacy = new Set<string>(LEGACY_MARTIAL_SKILL_KEYS);
  for (const row of expandHistoryRows(xp.history)) {
    const bucket = historySpend[row.category] != null ? row.category : 'other';
    if (row.signedAmount < 0) historySpend[bucket] += -row.signedAmount;
