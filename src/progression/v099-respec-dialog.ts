...
import {
  ...
  stoneSlotProgress,
  type StoneSlotChoice,
} from './v099-respec-flow.js';

function readField(html: any, name: string): string {
  const jq = html?.find?.(`[name="${name}"]`);
  if (jq && typeof jq.val === 'function') return String(jq.val() ?? '');
  const root = html?.[0] ?? html;
  const el = root?.querySelector?.(`[name="${name}"]`);
  return String(el?.value ?? '');
}

function suggestedStarting(actor: any): Record<AttributeKeyName, number> {
...
function dialogRoot(dialog: any, html: any): HTMLElement | null {
...
interface AttributeDraft {
  ...
  finals: Record<AttributeKeyName, number>;
  lifetimeXp: number | null;
}

function copyAttrs(source: Record<string, number>): Record<AttributeKeyName, number> {
  const out = {} as Record<AttributeKeyName, number>;
  for (const key of ATTRIBUTE_KEYS) out[key] = Math.floor(Number(source[key]) || 0);
  return out;
}

function readLifetime(root: HTMLElement | null, fallback: number | null): number | null {
  const input = root?.querySelector?.('[name="lifetime"]') as HTMLInputElement | null;
 ...
 */
function openAttributeStep(
  actor: any,
  draft: AttributeDraft,
): Promise<'postpone' | 'stones'> {
  const DialogCtor = (globalThis as any).Dialog;
  if (!DialogCtor) return Promise.resolve('postpone');
  ...
  return new Promise((resolve) => {
    let settled = false;
    const finish = (value: 'postpone' | 'stones') => {
      if (settled) return;
      settled = true;
      resolve(value);
    };
    ...
    const paint = () => {
      ...
      if (!root) return;
      const selects: Record<string, number> = {};
      for (const key of ATTRIBUTE_KEYS) {
        const select = root.querySelector(`[name="start-${key}"]`) as HTMLSelectElement | null;
      ...
      const applyBtn = root.querySelector('.v099-apply-start') as HTMLButtonElement | null;
      if (applyBtn) {
        applyBtn.textContent = draft.applied ? 'Starting ändern' : 'Starting übernehmen';
        applyBtn.disabled = !draft.applied && !status.valid;
      }
      const adjust = planFinalAttributes(
        draft.applied ? draft.starting : selects,
        draft.applied ? draft.finals : selects,
        preserved,
      );
      ...
      const continueBtn = root.querySelector('.v099-continue') as HTMLButtonElement | null;
      if (continueBtn) continueBtn.disabled = !(draft.applied && !adjust.overBudget);
    };
    ...
    const continueToStones = () => {
      ...
      if (needsLife) {
        ...
        if (life == null) {
          (globalThis as any).ui?.notifications?.warn?.('Enter a Lifetime XP of 0 or more.');
          return;
        }
      ...
      if (adjust.overBudget) {
        (globalThis as any).ui?.notifications?.warn?.(
          `Those Attributes cost ${adjust.spent} XP, but only ${preserved} preserved Attribute XP is available.`,
        );
        return;
      }
      finish('stones');
    ...
    dialog.activateListeners = function activate(html: any) {
      ...
      if (!root || (root as any)._v099Bound) return;
      (root as any)._v099Bound = true;
      root.addEventListener('change', (ev) => {
        const target = ev.target as HTMLElement | null;
        if (!target?.matches?.('select[name^="start-"]') || draft.applied) return;
        paint();
      });
      root.addEventListener('click', (ev) => {
        const target = ev.target as HTMLElement | null;
        const apply = target?.closest?.('.v099-apply-start') as HTMLButtonElement | null;
        if (apply) {
          ev.preventDefault();
          if (draft.applied) {
            draft.applied = false;
          ...
          const next = {} as Record<AttributeKeyName, number>;
          for (const key of ATTRIBUTE_KEYS) {
            const select = root.querySelector(`[name="start-${key}"]`) as HTMLSelectElement | null;
            next[key] = Math.floor(Number(select?.value) || 0);
          }
          if (!describeStartingPackage(next).valid) return;
          draft.starting = next;
        ...
        const plus = target?.closest?.('.v099-plus') as HTMLButtonElement | null;
        const minus = target?.closest?.('.v099-minus') as HTMLButtonElement | null;
        const step = plus || minus;
        if (!step || !draft.applied) return;
        ev.preventDefault();
        const key = step.dataset.key as AttributeKeyName;
        if (!key) return;
        const row = planFinalAttributes(draft.starting, draft.finals, preserved).rows.find((r) => r.key === key);
        if (!row) return;
        if (plus && row.canIncrease) draft.finals[key] = row.value + 1;
        if (minus && row.canDecrease) draft.finals[key] = row.value - 1;
        paint();
      });
      paint();
    };
...
export function openStoneSlotChoice(args: {
  ...
  allowClear: boolean;
  partners: Array<{ index: number; label: string }>;
}): Promise<StoneChoice> {
  const DialogCtor = (globalThis as any).Dialog;
  if (!DialogCtor) return Promise.resolve({ kind: 'cancel' });
  ...
  const clearBtn = args.allowClear
    ...
    : '';
  return new Promise<StoneChoice>((resolve) => {
    let settled = false;
    const finish = (value: StoneChoice) => {
      if (settled) return;
      settled = true;
      resolve(value);
    };
    ...
    dialog.activateListeners = function activate(html: any) {
      ...
      root?.querySelectorAll('.v099-pick').forEach((btn) => {
        btn.addEventListener('click', (ev) => {
          ...
          if (choice === 'attribute') {
            finish({ kind: 'attribute', key: el.dataset.key as AttributeKeyName });
    ...
    if (choice.kind !== 'colorless') return choice;
    const partner = await openPartnerChoice(args.label, args.partners);
    if (partner == null) return { kind: 'cancel' };
    return { kind: 'colorless', otherIndex: partner };
  });
}

function openPartnerChoice(label: string, partners: Array<{ index: number; label: string }>): Promise<number | null> {
  const DialogCtor = (globalThis as any).Dialog;
  if (!DialogCtor || !partners.length) return Promise.resolve(null);
  let startNth = 0;
  ...
  return new Promise((resolve) => {
    ...
    dialog.activateListeners = function activate(html: any) {
      ...
      root?.querySelectorAll('.v099-pick').forEach((btn) => {
        btn.addEventListener('click', (ev) => {
          ev.preventDefault();
          finish(Math.floor(Number((btn as HTMLElement).dataset.index)));
          dialog.close();
        });
      });
    };
...
function openStoneStep(
  actor: any,
  draft: AttributeDraft,
  order: StoneSlotChoice[],
): Promise<'postpone' | 'back' | 'saved'> {
  const DialogCtor = (globalThis as any).Dialog;
  if (!DialogCtor) return Promise.resolve('postpone');
  const storedRank = Math.max(1, Math.floor(Number(actor?.system?.mastery?.rank) || 1));
  return new Promise((resolve) => {
    let settled = false;
    const finish = (value: 'postpone' | 'back' | 'saved') => {
      if (settled) return;
      settled = true;
      resolve(value);
    };
    ...
    const paint = () => {
      ...
      const confirmBtn = root.querySelector('.v099-confirm') as HTMLButtonElement | null;
      if (confirmBtn) confirmBtn.disabled = progress.openIndexes.length !== 0;
    };

    const confirmMigration = async () => {
      const progress = stoneSlotProgress(order);
      if (progress.openIndexes.length) return;
      ...
      if (!plan.ok) {
        (globalThis as any).ui?.notifications?.warn?.(plan.reason || 'Migration is not valid.');
        return;
      }
      await actor.update(v099RespecUpdate(actor, plan));
      (globalThis as any).ui?.notifications?.info?.('v0.9.9 migration saved.');
      finish('saved');
      dialog.close();
    };
    ...
    dialog.activateListeners = function activate(html: any) {
      ...
      (root as any)._v099Bound = true;
      root.addEventListener('click', (ev) => {
        const target = (ev.target as HTMLElement | null)?.closest?.('.v099-stone-slot') as HTMLElement | null;
        if (target?.closest?.('.v099-confirm') || (ev.target as HTMLElement | null)?.closest?.('.v099-confirm')) {
          ev.preventDefault();
        ...
        if (!Number.isFinite(index) || index < 0 || index >= order.length) return;
        void onSlot(index);
      });
      paint();
    };
      ...
      if (choice.kind === 'clear') {
        ...
        if (isColorlessSlot(current)) {
          order.forEach((entry, i) => {
            if (entry === current) order[i] = null;
          });
        } else order[index] = null;
...
export async function openV099RespecDialog(actor: any): Promise<void> {
  const DialogCtor = (globalThis as any).Dialog;
  ...
  let order: StoneSlotChoice[] | null = null;
  for (;;) {
    const step = await openAttributeStep(actor, draft);
    if (step !== 'stones' || draft.lifetimeXp == null) return;
        ...
        buttons: {
          ...
          confirm: {
            ...
            callback: async (html: any) => {
              ...
              (globalThis as any).ui?.notifications?.info?.('Lifetime XP saved. Assign the unlocked Stones.');
              resolve();
            },
          },
        },
