...
import { statusEntryId, type RawStatusEntry } from '../system/active-specials.js';
...
export function hudStackTotals(list: RawStatusEntry[] | null | undefined): Record<string, number> {
  const out: Record<string, number> = {};
  for (const entry of list || []) {
    const id = statusEntryId(entry);
    if (!id || !isHudTokenSpecialId(id, entry?.name)) continue;
...
export function hudStackGains(
  before: RawStatusEntry[] | null | undefined,
  after: RawStatusEntry[] | null | undefined,
): Array<{ id: string; added: number }> {
  const prev = hudStackTotals(before);
  const next = hudStackTotals(after);
  const gains: Array<{ id: string; added: number }> = [];
  for (const id of Object.keys(next)) {
    const added = next[id] - (prev[id] || 0);
    if (added > 0) gains.push({ id, added });
  }
  return gains;
}

function canvasToken(actor: any): any {
  const placeables = (globalThis as any).canvas?.tokens?.placeables || [];
  const id = actor?.id;
  return placeables.find((token: any) => token?.actor?.id === id || token?.document?.actorId === id) || null;
}

function tokenScreenCenter(actor: any): { x: number; y: number } | null {
  const token = canvasToken(actor);
  const canvas = (globalThis as any).canvas;
  if (!token || !canvas?.stage?.toGlobal) return null;
...
function shakeCanvasToken(actor: any): void {
...
function landShake(specialId: string): void {
  const landed = document.querySelector<HTMLElement>(
    `#mastery-special-token-area .sta-token[data-special-id="${specialId}"]`,
  );
  if (!landed) return;
...
function flyCoin(from: { x: number; y: number }, specialId: string, delay: number): void {
  window.setTimeout(() => {
    const board = document.querySelector<HTMLElement>('#mastery-special-token-area .sta-board');
    const dest = board?.getBoundingClientRect();
    if (!dest || dest.width < 2) return;
    const size = 46;
...
export async function announceStatusGains(
  actor: any,
  before: RawStatusEntry[] | null | undefined,
  after: RawStatusEntry[] | null | undefined,
): Promise<void> {
  const gains = hudStackGains(before, after);
  if (!gains.length || typeof document === 'undefined') return;
