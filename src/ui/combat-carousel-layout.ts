...
export type CarouselUserSize = { width: number | null; height: number | null };

export function carouselDensityForHeight(height: number | null | undefined): CarouselDensity {
  if (height == null || !Number.isFinite(height) || height <= 0) return 'full';
  if (height <= CAROUSEL_RAIL_HEIGHT) return 'rail';
  if (height <= CAROUSEL_MEDIUM_HEIGHT) return 'medium';
  return 'full';
}

export function clampCarouselHeight(height: number, maxHeight?: number): number {
...
export function clampCarouselWidth(width: number, maxWidth?: number): number {
...
export function readCarouselUserSize(store: Storage | null = storage()): CarouselUserSize {
  const empty: CarouselUserSize = { width: null, height: null };
  if (!store) return empty;
  try {
    const raw = store.getItem(CAROUSEL_SIZE_STORAGE_KEY);
    if (!raw) return empty;
    const parsed = JSON.parse(raw) as Partial<CarouselUserSize>;
...
export function writeCarouselUserSize(
  size: CarouselUserSize,
...
export function applyCarouselUserSize(
