...
const LINEAGE_TO_STRIDE_KEY: Record<string, string> = {
...
function resolveStrideKey(actor: any, item: any): string {
  const fromItem = String((item.system as any)?.elementalLineage || '').toLowerCase();
  const fromActor = String((actor.system as any)?.echo?.subChoiceKey || '').toLowerCase();
  return LINEAGE_TO_STRIDE_KEY[fromItem] || LINEAGE_TO_STRIDE_KEY[fromActor] || 'elvenStrideFire';
}

function isLegacyElvenStrideItem(item: any): boolean {
  return (
    item?.type === 'artifact' &&
...
export async function runElvenStrideLineageMigration(actors: any[]): Promise<void> {
  if (!game.user?.isGM) return;

  let migrated = 0;

  for (const actor of actors) {
    const legacyItems = Array.from(actor.items || []).filter(isLegacyElvenStrideItem) as any[];
