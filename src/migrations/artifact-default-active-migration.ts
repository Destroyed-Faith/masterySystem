...
export function registerArtifactDefaultActiveMigrationSetting(): void {
  try {
...
export async function runArtifactDefaultActiveMigration(): Promise<void> {
  ...
  for (const actor of actors) {
    ...
    for (const emb of artifacts) {
      ...
      if (!root || !rootNodeId) continue;

      const levels = {
        ...((root.getFlag('mastery-system', 'actorLevels') || {}) as Record<string, unknown>),
      };
      const nodeId =
        (emb.getFlag?.('mastery-system', 'evolutionNodeId') as string | undefined) || rootNodeId;
  ...
  if (total > 0) {
    const msg =
      `Mastery System | Artifacts default-active: woke ${activated} item(s), ` +
