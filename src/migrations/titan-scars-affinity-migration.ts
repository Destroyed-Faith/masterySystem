...
function resolveTitanKey(actor: any): { attr: string; key: string } {
  const sub = String((actor.system as any)?.echo?.subChoiceKey || '').toLowerCase();
  const attr = ATTR_TO_TITAN_KEY[sub] ? sub : 'might';
  return { attr, key: ATTR_TO_TITAN_KEY[attr] };
}

function isLegacyTitanScarsItem(item: any): boolean {
  return (
    item?.type === 'artifact' &&
...
export async function runTitanScarsAffinityMigration(actors: any[]): Promise<void> {
  if (!game.user?.isGM) return;

  let migrated = 0;

  for (const actor of actors) {
    const legacyItems = Array.from(actor.items || []).filter(isLegacyTitanScarsItem) as any[];
