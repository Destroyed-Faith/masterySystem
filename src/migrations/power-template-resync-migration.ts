...
function isResyncablePower(item: any): boolean {
    if (item?.type !== 'power') return false;
    const sys = item.system ?? {};
    const category = String(sys.category ?? '');
    if (category !== 'active' && category !== 'activeBuff') return false;
...
export async function runPowerTemplateResyncMigration(): Promise<void> {
    ...
    for (const actor of actors) {
        const items = Array.from(actor.items ?? []).filter(isResyncablePower) as any[];
        if (items.length === 0) continue;
        let touchedThisActor = false;

        for (const item of items) {
            const sys = item.system ?? {};
            const template = findTemplateById(String(sys.templateId)) as EmbeddedPowerData | null;
