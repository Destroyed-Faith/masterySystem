...
const DELETE_IDS: ReadonlySet<string> = new Set(['dread', 'frightened']);
...
function remapId(value: unknown): unknown {
 ...
 */
function remapSpecialString(value: unknown): unknown {
...
function remapConditionExpr(value: unknown): unknown {
  if (typeof value !== 'string') return value;
  let out = value;
  ...
  if (out !== value) changedFlag = true;
  return out;
}

/** Recursively rewrite a plain `system` object in place. */
function remapNode(node: any): void {
  if (!node || typeof node !== 'object') return;

  if (Array.isArray(node)) {
    for (const el of node) remapNode(el);
    return;
  }
  ...
  for (const [key, value] of Object.entries(node)) {
    if (key === 'statusEffects' && Array.isArray(value)) {
      ...
      for (const eff of value) {
        ...
        if (typeof (eff as any).name === 'string') {
          const nameKey = String((eff as any).name).trim().toLowerCase();
          ...
          if (mapped) {
            (eff as any).name = mapped;
            changedFlag = true;
          }
        }
        if (typeof (eff as any).id === 'string') {
          const idKey = String((eff as any).id).toLowerCase();
          if (DELETE_IDS.has(idKey)) {
            changedFlag = true;
            continue;
          }
          (eff as any).id = remapId((eff as any).id);
        }
        kept.push(eff);
      }
    ...
    if (key === 'specials' && Array.isArray(value)) {
      ...
      for (const el of value) {
        if (typeof el === 'string') {
          ...
          if (typeof (el as any).key === 'string') {
            const mapped = remapId((el as any).key);
            if (mapped === '') continue;
            (el as any).key = mapped;
          }
          remapNode(el);
          kept.push(el);
        } else {
    ...
    if (typeof value === 'string') {
      if (key === 'special') node[key] = remapSpecialString(value);
      else if (key === 'key' || key === 'vsCondition') {
        const mapped = remapId(value);
        node[key] = mapped === '' ? value : mapped;
        if (value === 'disrupt') node[key] = 'challenge';
      } else if (key === 'condition') node[key] = CONDITION_TOKEN_MAP[value] ?? value;
      else if (key === 'conditionExpr') node[key] = remapConditionExpr(value);
 ...
 * and all world items.
 */
export async function runSpecialEffectRenameMigration(actors: any[]): Promise<void> {
  ...
  for (const actor of actors || []) {
    try {
      if (await migrateDocument(actor)) migrated += 1;
    } catch (err) {
    ...
    for (const item of Array.from(actor?.items || []) as any[]) {
      try {
        if (await migrateDocument(item)) migrated += 1;
      } catch (err) {
