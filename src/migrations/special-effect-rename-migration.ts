...
function remapNode(node: any): void {
  ...
  for (const [key, value] of Object.entries(node)) {
      ...
      for (const eff of value) {
        if (eff && typeof eff === 'object') {
          if (typeof (eff as any).name === 'string') {
            const mapped = DISPLAY_MAP[String((eff as any).name).trim().toLowerCase()];
            if (mapped) { (eff as any).name = mapped; changedFlag = true; }
          }
          if (typeof (eff as any).id === 'string') (eff as any).id = remapId((eff as any).id);
        }
      }
      continue;
    }
    ...
    if (key === 'specials' && Array.isArray(value)) {
      for (let i = 0; i < value.length; i++) {
        ...
        if (typeof el === 'string') {
          ...
          if (typeof (el as any).key === 'string') (el as any).key = remapId((el as any).key);
          remapNode(el);
        }
      }
      continue;
    ...
    if (typeof value === 'string') {
      ...
      else if (key === 'key' || key === 'vsCondition') node[key] = remapId(value);
