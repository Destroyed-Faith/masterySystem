...
type PrintSkillRow = {
  ...
  pool: number | string;
  poolLabel: string;
  poolChips: { attributeKey: string; attributeLabel: string; rollLabel: string; halfPool: boolean }[];
  halfPool: boolean;
  keep: string;
  rating: number;
...
function buildPrintSkillPoolFields(
  ...
  pool: number | string;
  keep: string;
  poolChips: PrintSkillRow['poolChips'];
} {
...
function powerSortRank(label: string): number {
  const c = String(label || '').toLowerCase();
  if (c.includes('buff')) return 1;
  if (c.includes('reaction')) return 2;
 ...
 */
function prettyPowerName(item: any, rank: number): string {
  ...
  const levels = (sys?.levels ?? {}) as Record<string, any>;
  const findRank = (rw: any): number | undefined => {
    const sp = Array.isArray(rw?.specials)
      ? rw.specials.find((s: any) => String(s?.key) === key)
      : null;
    return sp && typeof sp.rank === 'number' ? sp.rank : undefined;
  };
  let value = findRank(levels[String(rank)]);
  for (let i = rank; value === undefined && i >= 1; i--) value = findRank(levels[String(i)]);
  if (value === undefined) {
...
function powerEffectForRank(sys: any, level: number): string {
  ...
  if (tid) {
    ...
    if (catalogLevels && typeof catalogLevels === 'object') {
      ...
      const catalogText = (catalogLevels as Record<string, any>)[ckey]?.effect?.text;
      if (typeof catalogText === 'string' && catalogText.trim()) return catalogText;
    }
  }
 ...
 */
function absImg(img: unknown): string {
...
function equipmentFlag(item: any): any {
  return (
 ...
 */
function buildPrintEquipment(allItems: any[]): {
  cols: number;
  rows: number;
  cells: any[];
  slots: any[];
  ...
  const equipmentItems = allItems.filter(
    (i: any) =>
      ['weapon', 'armor', 'shield', 'gear', 'artifact'].includes(String(i?.type)) &&
  ...
  for (const item of equipmentItems) {
    ...
    if (!slot && (item?.system as any)?.equipped === true) {
      if (item.type === 'weapon') { if (!slotMap['mainhand']) slotMap['mainhand'] = item; continue; }
      if (item.type === 'shield') { if (!slotMap['offhand']) slotMap['offhand'] = item; continue; }
      if (item.type === 'armor') { if (!slotMap['body']) slotMap['body'] = item; continue; }
    }
    if (slot) {
  ...
  const cellIndex = (col: number, row: number) => (row - 1) * cols + (col - 1);
  const rects: { x: number; y: number; w: number; h: number }[] = [];

  const place = (item: any, x: number, y: number, w: number, h: number) => {
    rects.push({ x, y, w, h });
...
export function buildCharacterPrintContext(
  ...
  const battleMovement: any[] = [
    ...
    {
      name: (globalThis as any).game?.i18n?.localize?.('MASTERY.weaponSets.actionName') || 'Weapon Swap',
      effect:
        (globalThis as any).game?.i18n?.localize?.('MASTERY.weaponSets.actionDescription') ||
  ...
  const perceptionSkill = buildPrintSkillRow(
    actor as Actor,
    'perception',
    num(skillMap.perception),
    num(skillsSpent.perception),
    masteryRank || 1,
  );
  ...
  const echoCards = getLicensedEchoCardIds(selectedEchoCardIds, masteryRank)
    .map((rawId: any) => {
      ...
      return {
        ...
        options: card.options.map((o: any) => ({
  ...
  const stonePowerGroups = allStoneGroups.filter((g) =>
  ...
  const attackPool = Math.max(
    num(system?.attributes?.might?.value),
    num(system?.attributes?.agility?.value),
  );
...
function buildPrintLifetimeProgression(system: any) {
...
function compactPlayText(value: unknown): string {
...
export function buildCharacterCompactPrintContext(actor: any): Record<string, unknown> {
  ...
  const healthBars = compactTrackBars(
    Array.isArray(system?.health?.bars) ? system.health.bars : [],
    ['Healthy', 'Bruised', 'Injured', 'Wounded', 'Broken'],
    ['Incapacitated'],
  );
  const stressFallback = ['Healthy', 'Stressed', 'Not Well', 'Breaking'];
  const stressBars = compactTrackBars(
    Array.isArray(system?.stress?.bars) ? system.stress.bars : [],
    stressFallback,
  );
  ...
  for (const p of powerItems) {
    ...
    powers[phase]!.push({
      phase,
      phaseClass: phaseCssClass(phase),
      name: prettyPowerName(p, rank).replace(
        /^(Passive|Active Buff|Reaction|Active|Movement):\s*/i,
        '',
      ),
      rank,
