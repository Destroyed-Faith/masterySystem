...
export function buildCharacterStatusRows(actor: Actor | null | undefined): CharacterStatusRow[] {
  ...
  const tempHP = Math.max(0, Math.floor(Number((actor as any)?.system?.health?.tempHP ?? 0) || 0));
  if (tempHP > 0) {
    rows.push({
      kind: 'tempHP',
      index: -1,
...
export async function removeCharacterStatusRow(actor: Actor, row: CharacterStatusRow): Promise<void> {
  if (row.kind === 'tempHP') {
