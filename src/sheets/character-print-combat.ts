...
function levelRow(sys: any, rank: number): any | null {
...
function powerDamageForRank(sys: any, rank: number): string {
  ...
  if (rider != null && String(rider).trim()) return cleanPowerDamage(rider);
  return cleanPowerDamage(sys?.roll?.damage ?? '0');
}

function healDiceForRank(sys: any, rank: number): string {
  const row = levelRow(sys, rank);
  const flat = row?.mechanics?.healing?.flat;
  if (flat != null && String(flat).trim()) return cleanPowerDamage(flat);
  return powerDamageForRank(sys, rank);
}

function healFootnoteForRank(rank: number, aoe: boolean): string {
  ...
  if (aoe) text += ' Only one ally per use.';
  return text;
}

function resolvePowerSpecialsLabel(sys: any, rank: number): string {
  const catalog = catalogLevelRow(sys, rank);
  const baked = levelRow(sys, rank);
  ...
  for (const entry of raw) {
    if (typeof entry === 'string') {
      const s = entry.trim();
      if (s && !/^special\b/i.test(s)) labels.push(s);
      continue;
    }
...
function isAoePower(sys: any, rank: number): boolean {
  const row = levelRow(sys, rank);
  const typeStr = String(row?.type ?? '');
  if (/aoe|zone/i.test(typeStr)) return true;
  const aoe = row?.aoe;
  if (!aoe || typeof aoe !== 'object') return false;
  const shape = String(aoe.shape ?? '').toLowerCase();
  if (!shape || shape === 'none') return false;
  return Number(aoe.radiusM) > 0;
}

function powerAttackTypeForRank(sys: any, rank: number): 'melee' | 'ranged' {
  const row = levelRow(sys, rank);
  const typeStr = String(row?.type ?? sys?.type ?? '');
  if (/ranged|zone/i.test(typeStr)) return 'ranged';
  return 'melee';
}

function attackKindLabel(sys: any, rank: number): string {
  const row = levelRow(sys, rank);
  const typeStr = String(row?.type ?? '');
  const aoe = isAoePower(sys, rank);
  const ranged = /ranged/i.test(typeStr);
  if (aoe) return ranged ? 'Ranged AoE Attack' : 'Melee AoE Attack';
  return ranged ? 'Ranged Attack' : 'Melee Attack';
}

function isHealPower(sys: any, rank: number): boolean {
  const row = levelRow(sys, rank);
  if (row?.mechanics?.healing) return true;
  const sub = `${sys?.subfamily ?? ''} ${sys?.templateId ?? ''}`;
  return /heal/i.test(sub);
}

function isAttackPower(sys: any): boolean {
  const action = sys?.cost?.action;
  if (action === 'attack' || action === true) return true;
  const slot = String(sys?.slot ?? '').toLowerCase();
  return slot === 'attack';
}

function powerTemplateKey(sys: any): string {
...
function weaponKind(weapon: any): 'melee' | 'ranged' {
...
function findItemById(items: any[], id: string | null | undefined): any | null {
  if (!id) return null;
  const key = String(id);
  return items.find((i: any) => String(i?.id) === key) ?? null;
}

/** True when an item contributes weapon damage of the requested attack type. */
function itemMatchesAttackType(item: any, attackType: 'melee' | 'ranged'): boolean {
  if (!item) return false;
  if (item.type === 'weapon') {
 ...
 */
function resolveWeaponForPrint(
  actor: any,
  items: any[],
  ...
  try {
    ...
    const order: WeaponSetIndex[] = [state.active, state.active === 1 ? 2 : 1];
    for (const idx of order) {
      ...
      for (const rawId of [hands.mainhand, hands.offhand]) {
        ...
        if (itemMatchesAttackType(item, attackType)) return item;
      }
    }
  } catch {
  ...
  if (resolved.system?.fromArtifact && resolved.id) {
    const artifact = items.find((i: any) => i.id === resolved.id && i.type === 'artifact');
    if (artifact) return artifact;
  }
  return resolved;
}

function resolveWeaponBaseDamageString(weapon: any | null): string {
  if (!weapon?.system) return '1d8';
  const sys = weapon.system;
  if (weapon.type === 'artifact' && artifactSystemHasSpellFocus(sys)) return '0';
 ...
 */
export function listPreparedWeaponsByAttackType(
  actor: any,
  items: any[],
): { melee: any | null; ranged: any | null } {
  return {
...
export function buildBasicAttackCompactDamageLines(
  actor: any,
  items: any[],
  mrBonusFormula: string,
): string[] {
  const { melee, ranged } = listPreparedWeaponsByAttackType(actor, items);
  const lines: string[] = [];
  if (melee) {
    const wd = resolveWeaponBaseDamageString(melee);
    if (parseD8Count(wd) > 0) lines.push(`Melee: WD ${wd} + ${mrBonusFormula}`);
  }
  if (ranged) {
    const wd = resolveWeaponBaseDamageString(ranged);
    if (parseD8Count(wd) > 0) lines.push(`Ranged: WD ${wd} + ${mrBonusFormula}`);
  }
  if (lines.length === 0) {
...
function actorHasDualAttributeWeapon(items: any[]): boolean {
  for (const item of items) {
    if (item?.type !== 'artifact' || !isArtifactEquipped(item)) continue;
    const text = `${item.system?.description ?? ''} ${item.system?.lore ?? ''}`;
    if (/may use might or agility/i.test(text)) return true;
  }
  return false;
}
...
function resolveAttackAttribute(
  ...
  if (weapon) {
    ...
    if (innate.some((a: unknown) => String(a).toLowerCase().includes('finesse'))) {
      return 'Agility';
    }
  }
  const fromTree = getAttackAttributeForPowerTreeOrSchool(sys.tree);
  if (fromTree) return capAttr(fromTree);
  const attr = sys.roll?.attribute || sys.attribute;
  if (attr) return capAttr(String(attr).toLowerCase());
...
function formatBattleRollFormula(
  weaponDmg: string,
  powerDmg: string,
  weaponSpecials: string[],
  powerSpecials: string,
  spellFocusDice: number,
): { formula: string; show: boolean } {
  const parts: string[] = [];
  const wCount = parseD8Count(weaponDmg);
  const pCount = parseD8Count(powerDmg);

  if (wCount > 0) parts.push(`WD ${weaponDmg}`);
  if (pCount > 0) parts.push(powerDmg);
  for (const ws of weaponSpecials) {
    if (ws && !parts.includes(ws)) parts.push(ws);
  }
  if (powerSpecials) parts.push(powerSpecials);
...
function shouldIncludeWeapon(
  ...
  if (!weapon || !usesWeaponDamage(sys, rank)) return false;
  if (spell && !isWeaponAttackPower(sys)) return false;
  if (artifactSystemHasSpellFocus(weapon.system)) return false;
  if (weaponKind(weapon) !== attackType) return false;
...
export function buildPrintCombatPreview(
  ...
  if (heal) {
    ...
    if (parseD8Count(healDice) <= 0 && !footnote) return null;
    return {
      attackLabel: '',
      attackValue: 0,
      rollKind: 'heal',
  ...
  const wsForRoll =
    shouldIncludeWeapon(slot, spell, weapon, attackType, sys, rank) ? weaponSpecials : [];
  const { formula, show } = formatBattleRollFormula(
    weaponDmg,
    powerDmg,
    wsForRoll,
    powerSpecials,
