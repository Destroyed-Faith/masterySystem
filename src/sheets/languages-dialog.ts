...
function actorEchoKey(actor: any): string {
    return String(actor?.system?.echo?.key || '');
}

function isCreationIncomplete(actor: any): boolean {
    return actor?.system?.creation?.complete === false;
}

function buildLanguageDialogContent(
    known: string[],
                ...
                save: {
                    ...
                    callback: async (html: any) => {
                        ...
                        if (lockOthers && lockedKey) {
                            ...
                            if (lockedKey) picked.push(lockedKey);
                        }
