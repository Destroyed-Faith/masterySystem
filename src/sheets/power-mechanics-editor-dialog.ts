...
function readFormValues(root: HTMLElement): PowerMechanics | null {
  ...
  const persisted = persistPowerMechanics(m as PowerMechanics);
  const keys = Object.keys(persisted).filter(
    (k) =>
      k !== 'applyWhen' &&
      k !== 'duration' &&
      k !== 'usageLimit' &&
        ...
        callback: async (html: any) => {
          ...
          if (jsonRaw) {
            try {
              ...
              nextMech = persistPowerMechanics(parsed as PowerMechanics);
            } catch (err: any) {
