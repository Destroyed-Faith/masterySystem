...
export async function showPowerCreationDialog(actor: any) {
  ...
  const dialog = new Dialog({
    ...
    buttons: {
      create: {
        ...
        callback: async (html) => {
          ...
          if (powerData) {
            levelData = powerData.levels.find((l: any) => l.level === level);
          }

          const mappedTypeMap: Record<string, string> = {
            Melee: 'active',
            Ranged: 'active',
            Buff: 'buff',
            Utility: 'utility',
    ...
    render: async (html: JQuery) => {
      ...
      const loadPowersForTree = async (treeName: string) => {
      ...
      levelSelect?.addEventListener('change', function () {
        ...
        const power = powersData[powerName];
        const lvlData = power.levels.find((l: any) => l.level === level);
        if (!lvlData) return;
