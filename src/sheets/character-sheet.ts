  ...
  async #mountBattleSensesArea(html: JQuery): Promise<void> {
    ...
    try {
      ...
      const markup = await foundry.applications.handlebars.renderTemplate(
        'systems/mastery-system/templates/actor/partials/battle-senses-area.hbs',
        { editable: this.isEditable, combatSensesBattle },
      );
  ...
  activateListeners(html: JQuery) {
    ...
    html.off('drop.ms-equipment-drop').on('drop.ms-equipment-drop', '[data-df-drop]', async (ev: JQuery.DropEvent) => {
      ...
      console.log('Mastery System | [Equipment Drop] Delegated drop handler', {
        ...
        cellCol: (dragEvent as any).__msDropCell?.dataset?.col,
        cellRow: (dragEvent as any).__msDropCell?.dataset?.row
      });
    ...
    if (ContextMenuCls) {
      const rootEl = (html as any)?.[0] ?? (this.element as any)?.[0] ?? document.body;
