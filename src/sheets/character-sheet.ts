...
import { getPassiveSlots, getAvailablePassives, slotPassive, unslotPassive, activatePassive } from '../powers/passives';
import { getHealthLevelsData } from '../combat/health';
import { getCharges, burnStoneForCharges } from '../powers/charges';
import { getActiveBuffs } from '../powers/buffs';
import { getAvailableManeuvers, performManeuver } from '../combat/maneuvers';
...
export class MasteryCharacterSheet extends ActorSheet {
   ...
   */
  async #onPassiveSlotClick(event: JQuery.ClickEvent) {
    event.preventDefault();
    const element = event.currentTarget;
   ...
   */
  async #showPassiveSelectDialog(slotIndex: number): Promise<void> {
    ...
    const selectablePassives = availablePassives.filter(
   ...
   */
  async #onPassiveActivate(event: JQuery.ClickEvent) {
    event.preventDefault();
   ...
   */
  async #onPassiveRemove(event: JQuery.ClickEvent) {
    event.preventDefault();
    event.stopPropagation();
    const element = event.currentTarget;
    const slotIndex = parseInt(element.dataset.slotIndex || '0');
   ...
   */
  async #onHealthBoxClick(event: JQuery.ClickEvent) {
    event.preventDefault();
    const element = event.currentTarget;
   ...
   */
  async #onBurnStone(event: JQuery.ClickEvent) {
    event.preventDefault();
   ...
   */
  async #showPowerSelectionDialog(treeName: string) {
    const { getPowersForTree } = await import('../utils/mastery-powers.js');
   ...
   * Handle maneuver use
   */
  async #onManeuverUse(event: JQuery.ClickEvent) {
    event.preventDefault();
    const element = event.currentTarget;
    ...
    await performManeuver(this.actor, maneuverType as any, target);
    this.render(false);
  }
   ...
   * Handle buff removal
   */
  async #onBuffRemove(event: JQuery.ClickEvent) {
    event.preventDefault();
    const element = event.currentTarget;
