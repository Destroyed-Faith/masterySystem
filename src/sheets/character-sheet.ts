...
export class MasteryCharacterSheet extends BaseActorSheet {
  ...
  getData(options?: any) {
    ...
    // Find equipped armor
    const equippedArmor = preparedItems.armor?.find((armor: any) => armor.system?.equipped) || null;
    const armorValue = equippedArmor?.system?.armorValue || 0;
    
    // Find equipped shield
    const equippedShield = preparedItems.shields?.find((shield: any) => shield.system?.equipped) || null;
  ...
  async #onFinalizeCreation(event: JQuery.ClickEvent) {
    ...
    const invalidPowers = selectedPowers.filter((p: any) => (p.system?.level || 1) > masteryRank);
