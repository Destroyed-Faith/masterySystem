...
const DEFAULT_STAGE_POWER_LEVELS: PowerLevelKey[] = ['4', '10', '16'];

function stagePowerLevelsForPick(pick: ArtifactProgressionPick): PowerLevelKey[] {
  const custom = pick.stagePowerLevels;
  if (Array.isArray(custom) && custom.length === STAGE_NUMERALS.length) {
    return custom as PowerLevelKey[];
  }
  return DEFAULT_STAGE_POWER_LEVELS;
}

function stageNumeralsForPick(pick: ArtifactProgressionPick): readonly string[] {
...
const WEAPON_AOE_PICK_SPECIAL_RANKS: Record<string, readonly [number, number, number]> = {
  lacerate: [3, 5, 7],
  push: [2, 6, 8],
};

function optionalWeaponAoeSpecialText(chosenKey: string, stageIndex: number): string {
  const ef = getEffect(chosenKey);
  const label = ef ? getEffectBaseName(ef.name) : chosenKey;
...
function stoneFunctionEffect(sf: ArtifactStoneFunction, stageIndex: 0 | 1 | 2): string {
  ...
  if (sf.kind === 'stonePowerSupport') {
    const sp = sf.stonePowerId
      ? (STONE_POWERS as Record<string, { name?: string }>)[sf.stonePowerId]
      : undefined;
...
export function deriveLevelProgressionFromPicks(
  ...
  for (const baseLevel of [1, 2, 3]) {
    ...
    if (pick.kind === 'power') {
      ...
      const perStageTemplates =
        Array.isArray(pick.stageTemplateIds) &&
        pick.stageTemplateIds.length === STAGE_NUMERALS.length
          ? pick.stageTemplateIds
          : null;
      const perStageNames =
        Array.isArray(pick.stageNames) && pick.stageNames.length === STAGE_NUMERALS.length
          ? pick.stageNames
          : null;
      ...
      for (let s = 0; s < STAGE_NUMERALS.length; s++) {
        ...
        rows.push({
          ...
          aoe: '',
          duration:
            sf.kind === 'stonePowerSupport' || sf.kind === 'stoneRefresh' ? 'Instant' : 'Passive',
          effect: stoneFunctionEffect(sf, s as 0 | 1 | 2),
