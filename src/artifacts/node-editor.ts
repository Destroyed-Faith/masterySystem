...
function lettersForSpecSlot(slot: string, limit: number): Array<'a' | 'b' | 'c'> {
  if (slot === 'body') return ['a', 'a'];
...
function readArmorWeightForEditor(
  system: any,
  armorProfile: { type?: string },
): ArtifactArmorWeightClass {
  return (
    normalizeArtifactArmorWeight(armorProfile?.type) ||
...
export class NodeEditor extends BaseDialog {
  ...
  getData(options?: any): any {
    ...
    const innatesForRows = (weapon.innateAbilities || []).filter(
      (a: string) => String(a).trim() !== freeTrait || lineage.lockedInnateSet.has(String(a).trim()),
    );
    ...
    const typeDerivedLabelMap: Record<string, string> = {};
    for (const type of Object.keys(BASE_VALUE_TYPE_LABELS)) {
      const profileForType =
        type === 'bodyArmor' || type === 'evade'
          ? specSlot === 'body'
            ? bodyDeriveProfile
            : specBaseProfile || undefined
          : specBaseProfile || undefined;
      const derived = deriveBaseValueDisplay(
    ...
    (this as any)._typeDerivedLabelMap = typeDerivedLabelMap;
    (this as any)._specialValueMap = specialValueMap;
    (this as any)._nodeLevel = nodeLevel;
    (this as any)._armorWeight = armorWeight;
    ...
    const deriveRowLabel = (slot: 'a' | 'b' | 'c', type: string, specialId: string): string => {
      if (!isBaseValueSlotUnlocked(slot, nodeLevel)) return '';
      if (type === 'weaponSpecial') return specialValueMap[specialId] || '';
    ...
    const takeBvForLetter = (letter: 'a' | 'b' | 'c', preferredType?: string): ArtifactBaseValue | undefined => {
      let idx = -1;
      if (preferredType) {
        idx = remainingBvs.findIndex(
          (bv) =>
            (bv.slot === 'b' || bv.slot === 'c' ? bv.slot : 'a') === letter && bv.type === preferredType,
        );
      }
      if (idx < 0) {
        idx = remainingBvs.findIndex(
          (bv) => (bv.slot === 'b' || bv.slot === 'c' ? bv.slot : 'a') === letter,
        );
      }
    ...
    data.specBaseValueRows = bvLetters.map((letter, index) => {
      const preferredType =
        specSlot === 'body' ? (index === 0 ? 'bodyArmor' : 'evade') : undefined;
      const slotLabel =
        specSlot === 'body'
          ? index === 0
            ? 'A · Armor'
      ...
      const overrideStr =
        storedValue && storedValue !== derivedValue && storedValue !== derivedDisplay
          ? storedValue
          : '';
      return {
        slot: letter,
  ...
  activateListeners(html: JQuery): void {
    ...
    const typeDerivedLabelMap = ((this as any)._typeDerivedLabelMap || {}) as Record<string, string>;
    ...
    const rebuildBaseValueRows = (slot: ArtifactSlotKey | '') => {
      ...
      $specBvContainer.find('.node-spec-bv-row').each((i, el) => {
        ...
        const bodyLabel =
          slot === 'body' ? (i === 0 ? 'A · Armor' : 'A · Evade') : BV_LETTER_LABELS[letter];
    ...
    const applyBaseTypePrefill = (id: string) => {
      ...
      if (res.kind === 'weapon') {
        ...
        const lvl = Math.max(1, Math.min(10, Number((this as any)._nodeLevel) || 1));
        ...
        html.find('#node-armor-skill-penalty').val(
          def?.skillPenalty === '—' ? '' : def?.skillPenalty || '',
        );
        // Refresh Armor/Evade derived displays for the new weight class.
        (this as any)._armorWeight = weight;
        for (const type of ['bodyArmor', 'evade']) {
          const derived = deriveBaseValueDisplay(type as ArtifactBaseValueType, lvl, weight);
          typeDerivedMap[type] = derived.display;
          typeDerivedLabelMap[type] = derived.label || derived.display;
        }
  ...
  async saveNode(html: JQuery): Promise<void> {
    ...
    const prevFreeTrait = String((this.item.system as any).freeTrait || '').trim();
    let freeTrait = lineage.isLineageRoot
      ? String(html.find('#node-spec-free-trait').val() || '').trim()
      : prevFreeTrait;
    if (!ARTIFACT_FREE_TRAIT_OPTIONS.includes(freeTrait)) freeTrait = '';
    if (freeTrait && !innateAbilities.includes(freeTrait)) innateAbilities.push(freeTrait);

    const prevAttackAttribute = String((this.item.system as any).attackAttribute || '').trim();
    ...
    const nodeLevelForArmor = Math.max(1, Math.min(10, Number((this.item.system as any).level) || 1));
    ...
    for (const row of baseValueRows) {
      ...
      if (usedLetters.has(slotLetter) && specSlot !== 'body') continue;
      ...
      if (typeRaw === 'bodyArmor' && specSlot === 'body') {
        (entry as any).armorWeightClass = armorWeight;
      }
