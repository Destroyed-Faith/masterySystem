...
export interface ConditionData {
  name: string;                  // Condition name (e.g., "Poisoned", "Bleeding")
  value: number;                 // Intensity (e.g., Poisoned(3), Bleeding(2))
  diminishing: boolean;          // Does value decrease each round?
  duration: {
    type: 'rounds' | 'scene' | 'permanent';
    remaining?: number;          // Rounds remaining (if type = 'rounds')
  };
  save: {
    type: 'body' | 'mind' | 'spirit';
    tn: number;                  // TN to save against
...
export async function applyCondition(
  ...
  const existingCondition = target.items.find((i: any) => 
    i.type === 'condition' && i.name === conditionData.name
  );
...
export async function removeCondition(
  ...
  const condition = target.items.find((i: any) => 
 ...
 */
export async function updateConditionsForRound(actor: any): Promise<void> {
 ...
 */
export function getActiveConditions(actor: any): any[] {
  return actor.items.filter((i: any) => i.type === 'condition');
}
 ...
 */
export function hasCondition(actor: any, conditionName: string): boolean {
  return actor.items.some((i: any) => 
    i.type === 'condition' && i.name === conditionName
  );
}
 ...
 * @returns Save TN
 */
export function calculateSaveTN(masteryRank: number): number {
