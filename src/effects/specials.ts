...
import { applyCondition, type ConditionData } from '../effects/conditions';
...
export interface SpecialEffectData {
  type: SpecialEffect;
  intensity: number;         // Intensity/value (e.g., Bleed(3), Mark(2))
  duration?: number;         // Rounds (if applicable)
  source?: any;              // Actor that applied it
}
 ...
 */
export async function applyProne(target: any, source?: any): Promise<void> {
 ...
 */
export async function applyBleed(
  target: any,
  intensity: number,
  source?: any
): Promise<void> {
 ...
 */
export async function applyDisarm(target: any, source?: any): Promise<void> {
  const conditionData: ConditionData = {
    name: 'Disarmed',
    value: 1,
    diminishing: false,
 ...
 * Lose next Action
 */
export async function applyStun(
  target: any,
  duration: number,
  source?: any
): Promise<void> {
 ...
 */
export async function applyFrightened(
  target: any,
  intensity: number,
  duration: number,
  source?: any
 ...
 * -1 Action next round
 */
export async function applyDazed(target: any, source?: any): Promise<void> {
  const conditionData: ConditionData = {
    name: 'Dazed',
    value: 1,
    diminishing: false,
 ...
 */
export async function applySlowed(
  target: any,
  duration: number,
  source?: any
): Promise<void> {
 ...
 */
export async function applyGrappled(target: any, source?: any): Promise<void> {
  const conditionData: ConditionData = {
    name: 'Grappled',
    value: 1,
    diminishing: false,
 ...
 */
export async function applySpecialEffect(
  target: any,
 ...
 */
export function parseSpecialEffects(item: any): Array<{ effect: SpecialEffect; intensity: number }> {
  const specials = item.system.specials || [];
  const parsed: Array<{ effect: SpecialEffect; intensity: number }> = [];
