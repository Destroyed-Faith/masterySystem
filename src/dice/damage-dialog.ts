...
export async function showDamageDialog(
  ...
  if (isNpcAttackFlow) {
    ...
    if ((!Array.isArray(atk?.specials) || !atk!.specials!.length) && atk?.autoApplySpecial && atk.special) {
      ...
      if (eff) {
        ...
        if (idx >= 0) inlineSpecials.splice(idx, 1);
      }
    }
    selectedPowerData = {
  ...
  const availableSpecials = await collectAvailableSpecials(actorToUse as Actor, weaponForDamage, selectedPowerData);
...
export async function applyDamageToTargetFromAoe(
  ...
  attacker: Actor,
  count8s: number = 0,
  attackContext?: ApplyDamageOptions,
...
export async function applyDamageToTarget(
  ...
  try {
    ...
    const barsBefore = (system.health.bars as Array<{ current?: number }>).map((b) => ({
    ...
    if (barDamage > 0 && (globalThis as any).canvas?.ready) {
      try {
        ...
        const tokens = (target as any).getActiveTokens?.() ?? [];
        let targetToken = tokens[0] ?? null;
        if (!targetToken) {
          const sceneTokens = (globalThis as any).game?.scenes?.active?.tokens;
          const doc = sceneTokens?.find?.((t: any) => t.actor?.id === (target as any).id);
...
const pendingDamageFaithPrompts = new Map<string, PendingDamageFaithPrompt>();
let damageFaithChatHooksRegistered = false;

function registerDamageFaithRerollChatHooks(): void {
  if (damageFaithChatHooksRegistered) return;
  damageFaithChatHooksRegistered = true;
  Hooks.on('renderChatMessageHTML', (message: ChatMessage, htmlRaw: HTMLElement | JQuery) => {
    try {
      const ms = (message.flags as any)?.['mastery-system'];
      if (ms?.type !== 'damageFaithRerollPrompt' || ms?.resolved) return;
...
async function settleDamageFaithPrompt(
  ...
  if (!pending) {
    (ui as any).notifications?.warn?.(
      'Only the player who rolled this damage can choose Keep / Reroll here.',
    );
    return;
  }
...
function attachDamageFaithPromptHandlers(
  message: ChatMessage,
  ...
  rerollBtn.off('click.msDmgFaith').on('click.msDmgFaith', (ev: JQuery.ClickEvent) => {
    ev.preventDefault();
    ev.stopPropagation();
    void lock(true);
  });
