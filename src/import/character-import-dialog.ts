...
export class CharacterImportDialog extends BaseDialog {
  private jsonText = '';
  private validationMessage = 'Paste or upload a JSON file to validate.';
  ...
  protected _onRender(context: unknown, options: unknown): void {
    ...
    const textarea = root.querySelector<HTMLTextAreaElement>('.js-ci-json');
    const fileInput = root.querySelector<HTMLInputElement>('.js-ci-file');
    const validateBtn = root.querySelector<HTMLButtonElement>('.js-ci-validate');
    const importBtn = root.querySelector<HTMLButtonElement>('.js-ci-import');
    ...
    importBtn?.addEventListener('click', async (ev) => {
      ...
      try {
        ...
        if (result.actor) {
          (result.actor as any).sheet?.render(true);
        }
        this.close();
      } finally {
...
export function showCharacterImportDialog(): void {
  if (!game.user?.isGM) {
