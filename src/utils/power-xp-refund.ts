...
type PowerItemLike = {
    system?: { level?: number; minLevel?: number; rank?: number; category?: string; powerType?: string };
    type?: string;
};
 ...
 */
export function creationBaselineRank(item: PowerItemLike): number {
    const cat = item.system?.category
        ?? (item.system?.powerType === 'buff' ? 'activeBuff' : item.system?.powerType);
    if (cat === 'active') return CREATION_OFFENSIVE_RANK;
    if (cat === 'passive' || cat === 'activeBuff' || cat === 'reaction') return CREATION_DEFENSIVE_RANK;
    return 1;
}
 ...
 */
export function getPowerMinLevel(item: PowerItemLike): number {
    ...
    const candidate = hasValid ? Math.min(Math.floor(raw as number), lvl) : floor;
    return Math.max(candidate, floor);
}

/** Refund XP spent raising this power above its creation baseline. */
export function calculatePowerUpgradeRefund(item: PowerItemLike): number {
