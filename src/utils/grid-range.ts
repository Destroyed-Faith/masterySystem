...
export function measureSceneDistanceBetweenPoints(a: { x: number; y: number }, b: { x: number; y: number }): number {
  ...
  if (grid?.measurePath) {
    try {
      ...
      if (path != null) {
        if (typeof path.distance === "number") return path.distance;
        if (typeof path.total === "number") return path.total;
        if (Array.isArray(path) && path.length > 0 && typeof path[0] === "number") return path[0];
      }
    } catch {
      // fall through
    }
