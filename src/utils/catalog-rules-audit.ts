...
import {
    ...
    REACTION_TEMPLATES,
    MOVEMENT_TEMPLATES,
} from './powers/templates/index.js';
...
export type CatalogAuditStatus =
    | 'correct'
    | 'corrected'
    | 'missing'
    | 'obsolete'
    | 'requires-rule-decision';
 ...
 */
export const AUDIT_CORRECTED_TEMPLATE_IDS = new Set<string>([
    'ab-evade',
    'ab-evade-temp-hp',
    'ab-armor-evade',
]);
...
export const RULES_EXPECTED_ACTIVE_BUFFS: ReadonlyArray<{ rulesName: string; id: string }> = [
...
export const RULES_EXPECTED_REACTIONS: ReadonlyArray<{ rulesName: string; id: string }> = [
...
export const RULES_EXPECTED_MOVEMENT: ReadonlyArray<{ rulesName: string; id: string }> = [
 ...
 */
export const RULES_EXPECTED_ACTIVES: ReadonlyArray<{ rulesName: string; id: string }> = [
    ...
    { rulesName: 'Melee Targeted Special Attack', id: 'active-melee-targeted-special' },
    { rulesName: 'Ranged Targeted Special Attack', id: 'active-ranged-targeted-special' },
    { rulesName: 'Melee AoE Targeted Special Attack', id: 'active-melee-aoe-targeted-special' },
    { rulesName: 'Ranged AoE Targeted Special Attack', id: 'active-ranged-aoe-targeted-special' },
 ...
 */
export const SYSTEM_SUPPORT_PASSIVE_IDS: ReadonlyArray<{ rulesName: string; id: string }> = [
...
export const RULES_EXPECTED_PASSIVES: ReadonlyArray<{ rulesName: string; id: string }> = [
    { rulesName: 'Armor', id: 'passive-fortified-frame' },
    ...
    { rulesName: 'Temporary HP', id: 'passive-temp-hp' },
    { rulesName: 'Healing', id: 'passive-regeneration' },
    { rulesName: 'Phasing', id: 'passive-ghostform' },
    { rulesName: 'Damage', id: 'passive-killing-intent' },
    { rulesName: 'Health', id: 'passive-deep-vitality' },
    { rulesName: 'Spell Resistance', id: 'passive-spell-resistance' },
    { rulesName: 'Ward', id: 'passive-ward' },
    { rulesName: 'Armor (Moved 0 m Last Turn) (Conditional)', id: 'passive-stone-stance' },
    { rulesName: 'Armor (Adjacent to 2+ Enemies) (Conditional)', id: 'passive-surrounded-bulwark' },
    { rulesName: 'Evade (Moved 8+ m This Turn) (Conditional)', id: 'passive-flowing-step' },
    { rulesName: 'Evade (Exactly 1 Adjacent Enemy) (Conditional)', id: 'passive-duelist-footwork' },
    { rulesName: 'Damage (Moved 8+ m This Turn) (Conditional)', id: 'passive-momentum' },
    { rulesName: 'Damage (Target Cannot Perceive You) (Conditional)', id: 'passive-ambusher' },
    { rulesName: 'Damage (Affected by Lacerate) (Conditional)', id: 'passive-bloodlust' },
    { rulesName: 'Damage (Target Injured or Worse) (Conditional)', id: 'passive-executioner' },
    { rulesName: 'Healing (Wounded or Worse) (Conditional)', id: 'passive-blood-feast' },
    { rulesName: 'Healing (Adjacent to Enemy) (Conditional)', id: 'passive-battle-trance' },
    { rulesName: 'Healing (Moved 0 m Last Turn) (Conditional)', id: 'passive-stillness-recovery' },
    ...
    { rulesName: 'Armor + Temporary HP (Adjacent to Ally) (Conditional)', id: 'conditional-passive-armor-temp-hp' },
    { rulesName: 'Armor + Healing (Moved 0 m Last Turn) (Conditional)', id: 'conditional-passive-armor-healing' },
    { rulesName: 'Armor + Health (Adjacent to Ally) (Conditional)', id: 'conditional-passive-armor-health' },
    { rulesName: 'Evade + Temporary HP (Moved 8+ m) (Conditional)', id: 'conditional-passive-evade-temp-hp' },
    { rulesName: 'Evade + Healing (Moved 8+ m Last Turn) (Conditional)', id: 'conditional-passive-evade-healing' },
    { rulesName: 'Evade + Damage (Moved 8+ m This Turn) (Conditional)', id: 'conditional-passive-evade-damage' },
    { rulesName: 'Damage + Healing (Affected by Lacerate) (Conditional)', id: 'conditional-passive-damage-healing' },
    { rulesName: 'Damage + Temporary HP (Moved 8+ m) (Conditional)', id: 'conditional-passive-damage-temp-hp' },
    { rulesName: 'Health + Healing (Wounded or Worse) (Conditional)', id: 'conditional-passive-health-healing' },
    { rulesName: 'Health + Temporary HP (Adjacent to Ally at Combat Start) (Conditional)', id: 'conditional-passive-health-temp-hp' },
    { rulesName: 'Special Aura', id: 'passive-special-aura' },
    { rulesName: 'Telepathy', id: 'passive-telepathy-mind-link' },
    { rulesName: 'Summon Tokens', id: 'passive-bound-host' },
    { rulesName: 'Thorns', id: 'passive-thornhide' },
...
export const RULES_EXPECTED_ARTIFACTS: ReadonlyArray<{
    rulesName: string;
    id: string;
    source: 'echo' | 'general';
}> = [
    ...
    { rulesName: 'Root Staff', id: 'witchStaffRoot', source: 'echo' },
    { rulesName: 'Ruin Staff', id: 'witchStaffRuin', source: 'echo' },
    { rulesName: 'Blight Staff', id: 'witchStaffBlight', source: 'echo' },
    { rulesName: 'Alchemist Coat', id: 'alchemistCoat', source: 'echo' },
    { rulesName: 'Green Warden Mantle', id: 'greenWardenMantle', source: 'echo' },
    { rulesName: "Hunter's Scourge", id: 'huntersScourge', source: 'echo' },
...
export function parsePrefixedPowerNames(markdown: string, prefix: string): string[] {
    const out: string[] = [];
    const re = new RegExp(`^\\s*${escapeRegExp(prefix)}:\\s*(.+?)\\s*$`, 'gim');
    let m: RegExpExecArray | null;
    while ((m = re.exec(markdown)) !== null) {
        const name = cleanRulesName(m[1] ?? '');
        if (!name || HTML_FRAGMENT_RE.test(name) || name.length > 80) continue;
...
export function parsePassiveNamesFromRules(markdown: string): string[] {
    const out: string[] = [];
    const h3Re = /<h3[^>]*>\s*([^<]+)/gi;
    let m: RegExpExecArray | null;
    while ((m = h3Re.exec(markdown)) !== null) {
        let raw = cleanRulesName(m[1] ?? '');
        if (!raw || HTML_FRAGMENT_RE.test(raw)) continue;
        // "Passive: Armor" → Armor; "Conditional Passive: X" → X (Conditional)
        const paren = raw.match(/^(.+?)\s*\(Passive:/i);
        if (paren) raw = cleanRulesName(paren[1] ?? '');
        else if (/^Passive:/i.test(raw)) raw = cleanRulesName(raw.replace(/^Passive:\s*/i, ''));
...
export function parseMovementNamesFromRules(markdown: string): string[] {
    ...
    for (const exp of RULES_EXPECTED_MOVEMENT) {
        ...
        if (re.test(markdown) && !out.includes(exp.rulesName)) out.push(exp.rulesName);
    }
    return out;
}

/** Detect Artifact Summon Token Generator conversion table (4 tokens / stone). */
export function parseArtifactSummonTokenRatio(markdown: string): number | null {
    const m = markdown.match(/1\s+Summon\s+Stone\s*\|\s*(\d+)\s+Tokens/i);
    if (!m) return null;
    return Number(m[1]);
}

function cleanRulesName(s: string): string {
    return s
        .replace(/\{[^}]*\}/g, '')
        .replace(/\s+/g, ' ')
        .replace(/^[\s*#]+|[\s*#]+$/g, '')
...
export function checkTemplateStructure(template: PowerTemplate): StructuralIssue[] {
    ...
    const levels = template.levels ?? ({} as PowerTemplate['levels']);
    ...
    for (const key of POWER_LEVEL_KEYS) {
        ...
        if (!row) continue;
        if (row.mechanics && Object.keys(row.mechanics as object).length > 0) anyMechanics = true;
...
export function readMechanicsCurve(
    template: PowerTemplate,
    field: 'armor' | 'evade' | 'critical',
): Array<number | undefined> {
    return POWER_LEVEL_KEYS.map((k) => {
        const m = template.levels?.[k]?.mechanics as Record<string, unknown> | undefined;
        const v = m?.[field];
...
function emptySummary(): CatalogAuditSummary {
    return {
        correct: 0,
        corrected: 0,
        missing: 0,
...
function tally(summary: CatalogAuditSummary, status: CatalogAuditStatus): void {
    summary[status] += 1;
}
...
function auditExpectedAgainstCatalog(args: {
    category: CatalogAuditCategory;
    expected: ReadonlyArray<{ rulesName: string; id: string }>;
    catalogById: Map<string, { name: string }>;
    parsedRulesNames: Set<string>;
    correctedIds: ReadonlySet<string>;
    entries: CatalogAuditEntry[];
    summary: CatalogAuditSummary;
    /** Optional per-id notes override when matched. */
    notesForId?: (id: string, rulesName: string) => string | undefined;
    /** Ids that should be requires-rule-decision even when present. */
    decisionIds?: ReadonlySet<string>;
    /** Structural issue strings keyed by templateId. */
    structuralById?: Map<string, string[]>;
}): void {
    const withStruct = (id: string, notes: string): string => {
        const struct = args.structuralById?.get(id);
        if (!struct?.length) return notes;
    ...
    for (const exp of args.expected) {
        ...
        const inRules =
            args.parsedRulesNames.size === 0 ||
            args.parsedRulesNames.has(normalizeName(exp.rulesName)) ||
            [...args.parsedRulesNames].some(
                (n) =>
                    n.includes(normalizeName(exp.rulesName)) ||
        ...
        if (args.structuralById?.has(exp.id)) {
            pushEntry(args.entries, args.summary, {
                ...
                notes: withStruct(
                    exp.id,
                    note ?? (inRules ? 'Matches Rules entry' : 'Matches curated RULES_EXPECTED manifest'),
                ),
            });
            continue;
        }
    ...
    for (const [id, row] of args.catalogById) {
        if (matchedCatalogIds.has(id)) continue;
        ...
        pushEntry(args.entries, args.summary, {
            ...
            notes: withStruct(
                id,
                'Present in Foundry catalog but not in curated RULES_EXPECTED manifest',
            ),
        });
    }
}

function templateCatalogMap(
    templates: readonly PowerTemplate[],
): Map<string, { name: string; template: PowerTemplate }> {
    const map = new Map<string, { name: string; template: PowerTemplate }>();
 ...
 */
export function runCatalogRulesAudit(options: CatalogAuditOptions = {}): CatalogAuditReport {
    ...
    const parsedActiveBuffs = new Set(
        parsePrefixedPowerNames(mdActiveBuffs, 'Active Buff').map(normalizeName),
    );
    ...
    for (const bare of ['Summon Damage Aura', 'Summon Armor Aura', 'Intensified Absorption']) {
        if (mdActiveBuffs.includes(bare)) parsedActiveBuffs.add(normalizeName(bare));
    }
    ...
    const parsedReactions = new Set(
        parsePrefixedPowerNames(mdReactions, 'Reaction').map(normalizeName),
    );
    ...
    const entries: CatalogAuditEntry[] = [];
    const summary = emptySummary();

    // ── Structural checks (merged into per-template notes below) ───────────
    const structuralById = new Map<string, string[]>();
    const allTemplates: PowerTemplate[] = [
        ...ACTIVE_TEMPLATES,
        ...ACTIVE_BUFF_TEMPLATES,
        ...PASSIVE_TEMPLATES,
        ...REACTION_TEMPLATES,
    ...
    const parsedActives = new Set<string>();
    ...
    auditExpectedAgainstCatalog({
        ...
        structuralById,
        notesForId: (id) =>
            id.startsWith('active-melee-damage-t') || id.startsWith('active-ranged-damage-t')
    ...
    const echoById = new Map(
        Object.entries(ECHO_ARTIFACTS).map(([id, def]) => [id, { name: def.name }]),
    );
...
export function writeCatalogAuditReport(
...
export function runAndWriteCatalogAudit(
    options: CatalogAuditOptions & { outPath?: string } = {},
): CatalogAuditReport {
    const rootDir = options.rootDir ?? defaultRootDir();
...
export function getBlockingAuditEntries(
    report: CatalogAuditReport,
    opts: { allowMissingIds?: ReadonlySet<string>; allowObsoleteIds?: ReadonlySet<string> } = {},
): CatalogAuditEntry[] {
    const allowMissing = opts.allowMissingIds ?? new Set<string>();
    const allowObsolete = opts.allowObsoleteIds ?? new Set<string>();
    return report.entries.filter((e) => {
        if (e.status === 'missing' && !allowMissing.has(e.id)) return true;
        if (e.status === 'obsolete' && !allowObsolete.has(e.id)) return true;
        return false;
    });
}
