...
export function findLegacyDialogRoot(html: JQuery): JQuery {
...
export function centerLegacyDialogOnViewport(
    root: JQuery,
    app?: { setPosition?: (pos: { left?: number; top?: number }) => unknown },
): void {
    if (!root?.length) return;
    const el = root[0] as HTMLElement;
...
export function scheduleCenterLegacyDialog(
    html: JQuery,
    app?: { setPosition?: (pos: { left?: number; top?: number }) => unknown },
): void {
...
export function placeLegacyDialogOverlay(
    root: JQuery,
    width: number,
    height: number,
): void {
