...
export function isSmiteValidTarget(actor: { system?: any } | null | undefined): boolean {
 ...
 */
export function extractSmiteDice(specialStrings: readonly string[]): number {
  let total = 0;
...
export function stripSmiteSpecials(specialStrings: readonly string[]): string[] {
  return specialStrings.filter((raw) => {
    const s = String(raw ?? '').trim();
    if (/^smite$/i.test(s)) return false;
    if (/^smite\s*\(/i.test(s)) return false;
    return true;
  });
}
