...
export function mergeNpcAttackValueLists(existing: unknown, submitted: unknown): any[] {
  ...
  return ex.map((row, i) => {
    ...
    const { specials: overlaySpecials, ...overlayRest } = overlay as Record<string, unknown>;
    const merged: Record<string, any> = { ...row, ...overlayRest };
...
export function clampNpcAttackSlots(raw: unknown): number {
 ...
 */
export function resolveNpcAttackSlots(system: any): number {
  ...
  if (phases.length > 0) {
    ...
    if (Number.isFinite(explicit) && explicit >= 1) return clampNpcAttackSlots(explicit);
 ...
 */
function numericListKeys(raw: unknown): number[] {
  if (Array.isArray(raw)) return raw.map((_, i) => i);
  if (raw && typeof raw === 'object') {
    return Object.keys(raw as object)
      .filter((k) => /^\d+$/.test(k))
      .map((k) => Number(k));
 ...
 */
export function buildNpcPhasesReplacePatch(
  previousRaw: unknown,
  nextPhases: any[],
 ...
 */
export function buildNpcAttackValuesReplacePatch(
  pathPrefix: string,
  previousRaw: unknown,
  nextRows: any[],
): Record<string, unknown> {
...
export function npcAttackDiceCount(attack: AttackValue | null | undefined): number {
  if (!attack) return 0;
  const raw = attack.attackDiceCount as unknown;
  if (raw === 0 || raw === '0') return 0;
  const n = Math.floor(Number(raw));
  if (Number.isFinite(n) && n > 0) return Math.min(MAX_D, n);
...
export function npcAttackDiceCountIsBlank(attack: AttackValue | null | undefined): boolean {
...
export function resolveNpcSheetToHit(args: {
  ...
  if (type !== 'npc' && type !== 'summon') return null;

  const idx =
    args.attackIndex == null ? 0 : Math.max(0, Math.floor(Number(args.attackIndex) || 0));
  ...
  if (npcAttackDiceCountIsBlank(row) && args.prototypeSystem) {
    ...
    if (proto && !npcAttackDiceCountIsBlank(proto)) row = proto;
  }
