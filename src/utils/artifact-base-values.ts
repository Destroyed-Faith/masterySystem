...
function actorHasEquippedBodyArmor(actor: any): boolean {
  const items: any[] = Array.from(actor?.items ?? []);
  for (const it of items) {
    if (it.type === 'armor' && (it.system as any)?.equipped === true) return true;
  }
  return false;
}
...
function rowLabel(bv: ArtifactBaseValue): string {
 ...
 */
export function buildArtifactBaseValueBreakdown(actor: any): ArtifactBaseValueBreakdown {
  ...
  for (const item of items) {
    ...
    for (const bv of baseValues) {
      ...
      const type = (bv.type || 'minorFeature') as ArtifactBaseValueType;
