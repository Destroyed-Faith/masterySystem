...
function isNegativeSpecialEntry(entry: any): boolean {
  const id = statusEntryId(entry);
  if (!id) return true; // unknown stacks are treated as combat residue
...
export function hasReceivedFirstAid(target: any): boolean {
  return target?.getFlag?.('mastery-system', FIRST_AID_FLAG) === true;
}

/** New combat: the once-per-combat First Aid limit resets. */
export async function clearFirstAidFlags(actors: Iterable<any>): Promise<void> {
 ...
 */
export async function promptFirstAidForSelectedToken(): Promise<void> {
  ...
  const ok = await (Dialog as any).confirm({
    ...
    content:
      `<p>Apply <strong>First Aid</strong> to <strong>${target.name}</strong>?</p>` +
