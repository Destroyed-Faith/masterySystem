...
import type { EchoArtifactDefinition, EchoArtifactProgressionPickSpec, EchoArtifactStoneFunctionHint } from './echo-artifacts.js';
...
function tenLevels(
  a: [StageRow, StageRow, StageRow],
  b: [StageRow, StageRow, StageRow],
  c: [StageRow, StageRow, StageRow],
  ultimate: StageRow,
): ArtifactLevelProgressionRow[] {
  const placed: Array<[number, StageRow]> = [
    [1, a[0]],
    [2, b[0]],
    [3, c[0]],
    [4, a[1]],
...
const RETALIATORY_BITE: [StageRow, StageRow, StageRow] = [
  {
...
const RED_HUNT: [StageRow, StageRow, StageRow] = [
  {
    name: 'Red Hunt I',
    type: 'Active Buff',
    range: 'Self',
...
function predatorStoneTrack(
  label: string,
  technical: string,
): [StageRow, StageRow, StageRow] {
  return [
    {
...
function makePredatorCrown(opts: {
  ...
  return {
    ...
    description:
      'Beast Unbound Echo Artifact: the awakened hunting mind written into the head. Choose a Predator Shape; all Predator Crowns use the same rules.',
    restriction:
    ...
    levelProgression: tenLevels(
      predatorStoneTrack(opts.stoneLabel, opts.stoneTechnical),
      RETALIATORY_BITE,
      RED_HUNT,
      {
...
function witchOverdrive(special: 'Slow' | 'Hex' | 'Corrode'): [StageRow, StageRow, StageRow] {
  return [
    {
...
function witchMainSpell(name: string, special: 'Slow' | 'Hex' | 'Corrode'): [StageRow, StageRow, StageRow] {
  return [
    {
      name: 'Main Spell I',
      type: 'Active, Spell',
...
const WITCH_STONE: [StageRow, StageRow, StageRow] = [
  {
    name: 'Witch Stone I',
    type: 'Stone Power Support',
    range: 'Self',
...
function makeWitchStaff(opts: {
  ...
  subChoice: string;
  tradition: string;
  special: 'Slow' | 'Hex' | 'Corrode';
  specialKey: 'slow' | 'hex' | 'corrode';
  mainSpell: string;
  description: string;
}): EchoArtifactDefinition {
  const overdriveSpecs: EchoArtifactProgressionPickSpec = {
  ...
  return {
    ...
    restriction:
      'Witch Staff occupies both hand Slots. A Witch cannot use another weapon, shield, hand focus, claw Artifact, or hand-based magical item at the same time. Counts as a Spell Focus from Level 1. May be used with Intellect.',
    stoneFunction: {
      level: 1,
      kind: 'stonePowerSupport',
      attribute: 'intellect',
    ...
    levelProgression: tenLevels(
      WITCH_STONE,
      witchOverdrive(opts.special),
      witchMainSpell(opts.mainSpell, opts.special),
      {
...
const WITCH_STAFF_ROOT = makeWitchStaff({
  ...
  description:
    'Witch Unbound Echo Artifact grown from buried wood, grave roots, thorn knots, and old circles. Tradition Special: Slow. Main Spell: Rootbinding (Ranged Attack + Slow).',
});
...
const WITCH_STAFF_RUIN = makeWitchStaff({
  ...
  description:
    'Witch Unbound Echo Artifact of blackened wood, cracked bone, ash, and erased names. Tradition Special: Hex. Main Spell: Ruinous Word (Ranged Attack + Hex).',
});
...
const WITCH_STAFF_BLIGHT = makeWitchStaff({
  ...
  description:
    'Witch Unbound Echo Artifact swollen with fungus, rust, sour water, and living decay. Tradition Special: Corrode. Main Spell: Blightcall (Ranged Attack + Corrode).',
});

const ALCHEMIST_COAT: EchoArtifactDefinition = {
  key: 'alchemistCoat',
  name: 'Alchemist Coat',
  echoKey: 'unbound',
  slot: 'body',
  ...
  description:
    'Bane Echo Artifact: hardened leather, sealed vials, and a potion harness. Real Medium Armor. Red Potion is Active Buff: Damage. Black Potion is Active Buff: Critical.',
  restriction:
...
const GREEN_WARDEN_MANTLE: EchoArtifactDefinition = {
  ...
  description:
    'Bane Echo Artifact grown from bark, thorn, moss, and the last living strength of the wild places. Thorns is Active Buff: Thorns. Warden\'s Guard is Reaction: Ally Armor.',
  restriction:
...
const HUNTERS_SCOURGE: EchoArtifactDefinition = {
  key: 'huntersScourge',
  ...
  description:
    "Demon Hunter / Relic Hunter Echo Artifact: a two-handed chain-whip. Keeps Finesse and Reach from the Whip base. Exorcism Lash is Melee Attack + Exorcism. Snap Chain is Artifact Reaction: Damage + Pull. Whirling Scourge is Melee AoE Attack + Exorcism.",
  restriction:
    "Hunter's Scourge occupies both hand Slots. A Relic Hunter cannot use another weapon, shield, hand focus, claw Artifact, or hand-based magical item at the same time.",
  progressionPickSpecs: {
...
export const UNBOUND_ECHO_ARTIFACTS: Record<string, EchoArtifactDefinition> = {
