...
export function getPowerDefinitionRank(level: number | undefined, levels: unknown): number {
  ...
  if (Array.isArray(levels)) {
    const rankNums = levels
      .map((row: { level?: number }) => Number(row?.level))
  ...
  if (typeof levels === 'object' && levels !== null) {
    const keys = Object.keys(levels as object)
      .filter(k => /^\d+$/.test(k))
      .map(Number);
