...
export function getCreationSkillBudget(): { total: number; maxPerSkill: number; step: number } {
  const cfg = (CONFIG as any)?.MASTERY?.creation;
...
export function sumActorSkillPoints(system: any): number {
  let total = 0;
...
export function actorHasNoProgressionXp(actor: any): boolean {
...
export function isSkillsRedistributing(actor: any): boolean {
  return actor?.system?.creation?.skillsRedistributing === true;
}

export function canStartSkillsRedistribute(actor: any): { ok: boolean; reason?: string } {
...
export function nextCreationSkillValue(
  current: number,
  remaining: number,
  maxPerSkill = getCreationSkillBudget().maxPerSkill,
...
export function prevCreationSkillValue(
  current: number,
  maxPerSkill = getCreationSkillBudget().maxPerSkill,
): { ok: boolean; value?: number; reason?: string } {
  const cur = Math.max(0, Math.floor(Number(current) || 0));
...
function cloneSkillsRecord(skills: any): Record<string, number> {
...
export function buildStartSkillsRedistributeUpdates(actor: any): Record<string, unknown> {
...
export function buildCancelSkillsRedistributeUpdates(actor: any): Record<string, unknown> {
 ...
 */
export function buildFinishSkillsRedistributeUpdates(actor: any): {
  ok: boolean;
  reason?: string;
