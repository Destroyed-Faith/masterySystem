...
export const WEAPON_PROPERTIES: Record<string, string> = {
  Finesse:
    'Attack Roll uses Agility (for To-Hit only, not for damage).',
  Light:
    'Usable in off-hand (required for Full Dual-Wield).',
  Versatile:
    'When wielded two-handed, gain +2d8 weapon damage (you lose your shield / off-hand).',
  'Reach (+1 m)':
    'Your melee reach is 2 m by default; this increases it. Measured from the attacker’s token center.',
  'Reach (+2 m)':
    'Your melee reach is 2 m by default; this increases it. Measured from the attacker’s token center.',
  ...
  'Spell Focus (+2d8)':
    'When you deal damage with a Power that has the Spell tag, add +2d8 bonus damage dice.',
  'Spell Focus (+4d8)':
    'When you deal damage with a Power that has the Spell tag, add +4d8 bonus damage dice.'
};
 ...
 */
export function describeInnateAbility(ability: string): string {
  const trimmed = (ability || '').trim();
  if (!trimmed) return '';
  if (WEAPON_PROPERTIES[trimmed]) return WEAPON_PROPERTIES[trimmed];
...
export function getWeaponsByHands(hands: 1 | 2): WeaponDefinition[] {
...
export function getWeaponsByType(type: 'melee' | 'ranged'): WeaponDefinition[] {
  if (type === 'ranged') {
    return WEAPONS.filter((w) =>
      w.innateAbilities.some((a) => /^ranged/i.test(a.trim()))
    );
  }
  return WEAPONS.filter(
    (w) => !w.innateAbilities.some((a) => /^ranged/i.test(a.trim()))
  );
}
...
export function getWeaponsWithProperty(property: string): WeaponDefinition[] {
  const p = property.toLowerCase();
  return WEAPONS.filter((w) =>
    w.innateAbilities.some((ability) => ability.toLowerCase().includes(p))
  );
}
