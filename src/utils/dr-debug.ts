...
export function isDrDebugEnabled(): boolean {
  try {
    if ((globalThis as any).MSY_DEBUG_DR === true) return true;
    const g = (globalThis as any).game;
    if (!g?.settings?.get) return false;
    if (g.settings.get('mastery-system', 'debugDamageReduction') === true) return true;
...
export function logDrDebug(phase: string, payload: Record<string, unknown>): void {
