...
function asPoint(raw: any): WorldPoint | null {
  if (!raw) return null;
...
export function snapWorldCenter(x: number, y: number): WorldPoint {
  ...
  if (typeof grid.getSnappedPoint === 'function') {
    try {
      const modes = (globalThis as any).CONST?.GRID_SNAPPING_MODES;
...
export function snapWorldTopLeft(x: number, y: number): WorldPoint {
  const grid: any = (globalThis as any).canvas?.grid;
  if (!grid) return { x, y };
...
export function eventWorldPoint(ev: any): WorldPoint {
  const stage = (globalThis as any).canvas?.stage ?? (globalThis as any).canvas?.app?.stage;
...
export function resolveOverlayContainer(): any | null {
