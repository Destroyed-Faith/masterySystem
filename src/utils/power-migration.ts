 ...
 */
export function isOldPowerStructure(power: any): power is ArtifactPowerData {
...
function parseAoe(aoeStr: string): AoeSpec | null {
  const finish = (raw: AoeSpec): AoeSpec => normalizeAoeSpec(raw) ?? raw;
 ...
 */
function parseDuration(durationStr: string, powerType: string): DurationSpec {
 ...
 */
function parseEffect(effectStr: string, damageStr: string, healingStr: string): EffectSpec {
 ...
 */
function determineType(powerType: string, tags: string[], rangeStr: string): string {
  if (tags.includes('melee')) return 'melee';
  if (tags.includes('ranged')) return 'ranged';
  
  if (powerType === 'utility') return 'utility';
  if (powerType === 'buff' || powerType === 'activeBuff') return 'buff';
 ...
 */
export function migratePowerData(oldPower: PowerData): PowerData {
