...
declare const TileDocument: any;
 ...
 */
export async function createBloodPool(token: any, damage: number, persistent: boolean = false, bloodColor?: string): Promise<void> {
  ...
  if (!finalBloodColor && token.actor) {
    const actorSystem = (token.actor as any).system;
 ...
 */
function createTemporaryBloodPool(token: any, damage: number, radius: number, pixiColor: number, darkerColor: number): void {
  const tokenCenter = token.center;
  ...
  let backgroundContainer: PIXI.Container | null = null;
  
  if (canvas.background) {
    if ((canvas.background as any).container && typeof (canvas.background as any).container.addChild === 'function') {
      backgroundContainer = (canvas.background as any).container;
    } else if (typeof (canvas.background as any).addChild === 'function') {
      backgroundContainer = canvas.background as any;
    }
  }
  
  // Fallback to tiles layer
  if (!backgroundContainer && canvas.tiles) {
    if ((canvas.tiles as any).container && typeof (canvas.tiles as any).container.addChild === 'function') {
      backgroundContainer = (canvas.tiles as any).container;
    } else if (typeof (canvas.tiles as any).addChild === 'function') {
      backgroundContainer = canvas.tiles as any;
    }
  }
  ...
  if (backgroundContainer) {
    ...
    (bloodPool as any).msTokenId = token.id;
