...
export const MASTERY_TREE_POWER_MAP: Record<string, EmbeddedPowerData[]> = {};

/** @deprecated — trees removed. Returns empty array. */
export const ALL_MASTERY_POWERS: EmbeddedPowerData[] = [];

/** @deprecated — trees removed. Always returns `undefined`. */
export function getPowersForTree(_treeName: string): EmbeddedPowerData[] {
    return [];
}

/** @deprecated — trees removed. Always returns `undefined`. */
export function getPower(_treeName: string, _powerName: string): EmbeddedPowerData | undefined {
    return undefined;
}
