...
type HealthBarCount = { wounded: number; injured: number; bruised: number; healthy: number };
function deepVitalityBars(lvl: number): HealthBarCount {
    if (lvl <= 3)  return { wounded: 1, injured: 0, bruised: 0, healthy: 0 };
    if (lvl <= 7)  return { wounded: 1, injured: 1, bruised: 0, healthy: 0 };
    if (lvl <= 11) return { wounded: 1, injured: 1, bruised: 1, healthy: 0 };
    return              { wounded: 1, injured: 1, bruised: 1, healthy: 1 };
}
function combinedHealthBars(lvl: number): HealthBarCount {
    if (lvl <= 3)  return { wounded: 0, injured: 0, bruised: 0, healthy: 0 };
    if (lvl <= 7)  return { wounded: 1, injured: 0, bruised: 0, healthy: 0 };
    if (lvl <= 11) return { wounded: 1, injured: 1, bruised: 0, healthy: 0 };
    return              { wounded: 1, injured: 1, bruised: 1, healthy: 0 };
}
function healthBarText(h: HealthBarCount): string {
    const parts: string[] = [];
    if (h.wounded) parts.push(`**+${h.wounded} Wounded Health Bar${h.wounded > 1 ? 's' : ''}**`);
    if (h.injured) parts.push(`**+${h.injured} Injured Health Bar${h.injured > 1 ? 's' : ''}**`);
    if (h.bruised) parts.push(`**+${h.bruised} Bruised Health Bar${h.bruised > 1 ? 's' : ''}**`);
    if (h.healthy) parts.push(`**+${h.healthy} Healthy Health Bar${h.healthy > 1 ? 's' : ''}**`);
...
function combatSenseCount(lvl: number): { count: number; presence: boolean } {
    if (lvl <= 3)  return { count: 0, presence: false };
    if (lvl <= 7)  return { count: 1, presence: false };
    if (lvl <= 11) return { count: 2, presence: false };
    if (lvl <= 14) return { count: 3, presence: true };
    return              { count: 4, presence: true };
}
// Combined Awareness progression (fewer milestones vs the pure Heightened Senses).
function combinedSenseCount(lvl: number): { count: number; presence: boolean } {
    if (lvl <= 3)  return { count: 0, presence: false };
...
function damageReductionPct(lvl: number): number {
    if (lvl <= 2)  return 0;
    if (lvl <= 7)  return 10;
    if (lvl <= 14) return 20;
    return               30;
...
function phasingCharges(lvl: number): number {
    if (lvl <= 3)  return 0;
    if (lvl <= 7)  return 1;
    if (lvl <= 14) return 2;
    return               3;
...
function specialAuraRadius(lvl: number): number {
...
function basePassive(def: {
    ...
    tags?: string[];
    passiveType?: string;
    perLevel: (lvl: number) => { text: string; mechanics?: Partial<PowerMechanics> };
}): PowerTemplate {
    return {
        templateId: def.id,
...
function empowermentPassive(def: { id: string; name: string; axisLabel: string }): PowerTemplate {
    return {
        templateId: def.id,
        templateName: def.name,
        name: `Passive: ${def.name}`,
        ...
        levels: buildLevels((lvl) => {
            ...
            return passiveRow({
                type: 'Support',
                effectText:
                    `When you activate an Active Buff that grants ${def.axisLabel} as one of its effects, ` +
...
const EMPOWER_BUFF_TEMPLATES: PowerTemplate[] = [
...
const RAW_PASSIVE_TEMPLATES: PowerTemplate[] = [
    ...
    basePassive({
        ...
        perLevel: (lvl) => {
            ...
            if (radius === 0) return { text: '—', mechanics: {} };
            return {
...
export const PASSIVE_TEMPLATES: PowerTemplate[] = RAW_PASSIVE_TEMPLATES.filter(
    (t) => !RETIRED_AWARENESS_PASSIVE_IDS.has(t.templateId),
);
