...
export function resolveFullLevelProgression(
  authored: ArtifactLevelProgressionRow[] | null | undefined,
  ...
  if (fromPicks.length > 0) {
    ...
    return [...fromPicks, ...ultimates].sort(
...
function clampArtifactLevel(level: number): number {
...
export function isUltimateProgressionRow(row: ArtifactLevelProgressionRow): boolean {
  if (Number(row.level) !== 10) return false;
  const type = String(row.type || '').toLowerCase();
  if (type === 'ultimate') return true;
  return /^true\b/i.test(String(row.name || '').trim());
}

/** Which of the three standard pick slots a staged row belongs to (0..2). */
export function progressionSlotIndex(row: ArtifactLevelProgressionRow): number {
 ...
 */
export function visibleAbilityRows(
  allRows: ArtifactLevelProgressionRow[] | null | undefined,
  currentLevel: number,
): ArtifactLevelProgressionRow[] {
  ...
  const out: ArtifactLevelProgressionRow[] = [];

  for (let slot = 0; slot < 3; slot++) {
    const unlockAt = slot + 1;
    if (level < unlockAt) continue;
  ...
  if (level >= 10) {
    for (const u of ultimates) out.push(u);
  }
