...
export function inferArtifactEquipSlots(system: {
  ...
  if (newSlot) {
    if (baseProfile === 'twoHandedWeapon' || baseProfile === 'twoHandedWeaponRanged' || newSlot === 'bothHands') return ['mainhand', 'offhand'];
...
export function listCarriedItemsForPaperdollSlot(
  items: Iterable<any> | null | undefined,
  slotKey: string,
  opts?: { allowOffhandWeapon?: (item: any) => boolean },
): any[] {
