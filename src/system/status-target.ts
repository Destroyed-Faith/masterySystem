...
export function tokenDocOfActor(actor: any): any {
  if (!actor) return null;
  if (isSceneTokenDoc(actor)) return actor;
...
function isSceneTokenDoc(doc: any): boolean {
  if (!doc || doc.documentName !== 'Token') return false;
  const parentName = doc.parent?.documentName;
  return parentName === 'Scene' || parentName == null;
}

export function tokenIdOfActor(actor: any, fallback?: unknown): string {
  const fromFlag = String(fallback ?? '').trim();
  if (fromFlag) return fromFlag;
  return String(tokenDocOfActor(actor)?.id || '').trim();
}

function tokenActorFromScene(scene: any, tokenId: string): any {
...
export function resolveLiveActor(actorId?: unknown, tokenId?: unknown): any {
  ...
  if (tid) {
    ...
    const combatant = combat?.combatants?.find?.(
      (c: any) => String(c?.tokenId || c?.token?.id || '') === tid,
    );
  ...
  const combat = g.game?.combat;
  if (combat) {
    const matches = [...(combat.combatants ?? [])].filter(
      (c: any) => String(c?.actorId || c?.actor?.id || '') === aid,
    );
    if (matches.length === 1) {
      const only = matches[0];
      if (only?.token?.actor) return only.token.actor;
      if (only?.actor) return only.actor;
    }
  }
