...
function effectIconSrc(effect: any): string {
...
async function refreshStatusIconArt(actor: any, choice: StatusAddChoice): Promise<void> {
  const stale = actorEffects(actor).filter(
    (effect) => statusIdFromEffect(effect) === choice.id && effectIconSrc(effect) !== choice.img,
  );
  ...
  if (typeof actor.updateEmbeddedDocuments !== 'function') return;
