...
export const WEAKEN_ATTRIBUTES: ReadonlySet<string> = new Set(['might', 'agility', 'intellect']);

/** Attributes whose rolled pools are reduced by Soulburn(X). */
export const SOULBURN_ATTRIBUTES: ReadonlySet<string> = new Set(['wits', 'influence', 'resolve']);
 ...
 */
export function attributePoolReduction(actor: any, poolAttribute: string | undefined): PoolReductionResult {
  const out: PoolReductionResult = { reduction: 0, notes: [] };
  const attr = String(poolAttribute ?? '').trim().toLowerCase();
  if (!actor || !attr) return out;
...
export function readChallengeState(actor: any): ChallengeState {
  const state: ChallengeState = { value: 0, challengerUuid: null, challengerName: null };
  ...
  for (const entry of list) {
    ...
    if (id === 'challenge' || name === 'challenge') {
      ...
      if (!state.challengerUuid && entry?.sourceUuid) state.challengerUuid = String(entry.sourceUuid);
      if (!state.challengerName && entry?.source) state.challengerName = String(entry.source);
    }
  }
  // Fallback via canonical reader in case entries only carry legacy names.
  if (state.value === 0) state.value = getActiveSpecialValue(actor, 'challenge');
  return state;
}
 ...
 */
function refMatchesChallenger(challengerUuid: string, ref: string): boolean {
  if (!ref) return false;
  if (ref === challengerUuid) return true;
  // Compare trailing id segments (Actor.<id> vs plain <id>).
  const tail = (s: string) => s.split('.').pop() ?? s;
  if (tail(ref) === tail(challengerUuid)) return true;
  return false;
}
 ...
 */
export function normalizeTargetRefs(refs: Array<string | null | undefined>): string[] {
  const out: string[] = [];
  for (const raw of refs) {
    ...
    out.push(r);
    try {
      const tokenDoc = (globalThis as any).canvas?.tokens?.get?.(r)?.document;
      const tokenActor = tokenDoc?.actor;
      if (tokenActor?.uuid) out.push(String(tokenActor.uuid));
      if (tokenActor?.id) out.push(String(tokenActor.id));
    } catch {
 ...
 */
export function challengePoolReduction(actor: any, targetRefs: string[]): PoolReductionResult {
  const out: PoolReductionResult = { reduction: 0, notes: [] };
  if (!actor) return out;
  const state = readChallengeState(actor);
  if (state.value <= 0) return out;
 ...
 */
export function mergeChallengeEntry(
  list: any[],
  newValue: number,
  sourceName: string,
  sourceUuid: string | null,
): any[] {
  const isChallenge = (e: any) =>
    String(e?.id ?? '').toLowerCase() === 'challenge' ||
  ...
  const sameSource =
    (sourceUuid && existing.sourceUuid && String(existing.sourceUuid) === sourceUuid) ||
    (!existing.sourceUuid && !sourceUuid);
  if (sameSource) {
    existing.value = Math.max(0, Math.floor(Number(existing.value ?? 0))) + newValue;
    if (sourceUuid && !existing.sourceUuid) existing.sourceUuid = sourceUuid;
    return list;
  }
...
export function cleansableSpecials(actor: any): Array<{ id: string; value: number }> {
  return readActiveSpecials(actor).filter((s) => s.value > 0 && isCleansable(s.id));
}

function isCleansable(id: string): boolean {
...
export function applyCleanseToList(
  ...
  const empty: CleanseApplyResult = {
    applied: false,
    specialId: null,
    reducedBy: 0,
    remaining: 0,
  ...
  const candidates = list.filter((e) => {
    ...
    const canon = SPECIAL_EFFECTS_BY_ID.get(id) ? id : [...SPECIAL_EFFECTS_BY_ID.values()].find(
      (eff) => eff.name.toLowerCase() === name,
    )?.id;
  ...
  let target = chosenId
    ? candidates.find(
        (e) =>
          String(e?.id ?? '').toLowerCase() === chosenId.toLowerCase() ||
