...
export interface AttackData {
  // Source
  item: any;                     // The weapon/special/spell item
  
  // Target
  target: any;                   // Target actor
  targetEvade: number;           // Target's Evade value
  
  // Attack configuration
  attribute: string;             // Attribute for attack roll
  attributeValue: number;        // Value of that attribute
  skill?: string;                // Optional skill (e.g., "meleeWeapons")
  skillValue?: number;           // Skill bonus
  ...
  // Damage
  baseDamage: string;            // Base damage formula (e.g., "2d8")
...
export interface AttackResult {
  roll: RollKeepResult;
  hit: boolean;
  totalRaises: number;           // Declared raises (can be spent on damage/specials)
}
 ...
 */
export async function performAttackWithDialog(
  attacker: any,
  target: any,
  item: any
): Promise<void> {
 ...
 */
async function promptAndApplyDamage(
  attacker: any,
  attackData: AttackData,
    ...
    const dialog = new Dialog({
      ...
      buttons: {
        roll: {
          ...
          label: 'Roll Damage',
          callback: async (html: JQuery) => {
            const damageRaises = parseInt((html.find('[name="damageRaises"]').val() as string) || '0');
      ...
      render: (html: JQuery) => {
        html.find('[name="damageRaises"], [name="specialRaises"]').on('input', () => {
          const damage = parseInt((html.find('[name="damageRaises"]').val() as string) || '0');
          const special = parseInt((html.find('[name="specialRaises"]').val() as string) || '0');
