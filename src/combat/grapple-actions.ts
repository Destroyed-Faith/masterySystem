...
import {
  detectTargetUnaware,
  promptContestAttribute,
  startAttributeContest,
} from '../contests/contest-card.js';
import { endGrapple, readGrappleState, resolveGrapplePartner } from './grapple-state.js';

function notify(kind: 'warn' | 'info', text: string): void {
  (globalThis as any).ui?.notifications?.[kind]?.(text);
}

async function postNote(actor: any, token: any, html: string): Promise<void> {
  const CM = (globalThis as any).ChatMessage;
  if (typeof CM?.create !== 'function') return;
  try {
    await CM.create({
...
export async function beginGrappleContest(attackerToken: any, targetToken: any): Promise<void> {
  ...
  await startAttributeContest({
    ...
    combat: (globalThis as any).game?.combat ?? null,
  });
}

/** Radial "Escape Grapple" for the held creature: contest vs the grappler. */
export async function beginGrappleEscape(token: any): Promise<void> {
...
export async function releaseGrappleAction(token: any, opts: { reason?: string } = {}): Promise<boolean> {
  ...
  await postNote(
    ...
    `<h3><i class="fas fa-hand-paper"></i> Release Grapple</h3><p><strong>${String(actor.name)}</strong> lets go of <strong>${partnerName}</strong>.${why}</p>`,
  );
  return true;
}
 ...
 */
export async function confirmReleaseGrappleForWeaponAttack(token: any, attackName: string): Promise<boolean> {
  const actor = token?.actor ?? token;
  const state = readGrappleState(actor);
  if (!state || state.role !== 'grappler') return true;
  const DialogCtor = (globalThis as any).Dialog;
  let release = true;
  if (typeof DialogCtor === 'function') {
    release = await new Promise<boolean>((resolve) => {
      let settled = false;
