...
async function executeReactionSpend(params: {
  ...
  if (allowOffensiveVsShooter && attacker && isCounterDamageReaction(power)) {
    ...
    try {
      const { readPushPullMetersFromPower, offerForcedMovementFromActors } = await import(
        './forced-movement.js'
      );
      const { push: pushM, pull: pullM } = readPushPullMetersFromPower(power);
      if (pushM > 0 || pullM > 0) {
        const bits: string[] = [];
        if (pushM > 0) bits.push(`Push ${pushM} m (away)`);
        if (pullM > 0) bits.push(`Pull ${pullM} m (toward)`);
