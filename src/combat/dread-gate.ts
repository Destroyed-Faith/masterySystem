 ...
 */
export async function resolveDreadPreAttack(attacker: any): Promise<{ blocked: boolean; note: string }> {
  const dread = getActiveSpecialValue(attacker, 'dread');
  if (dread <= 0) return { blocked: false, note: '' };
  ...
  try {
    ...
    const result: any = await masteryRoll({
      numDice,
      keepDice,
      skill: 0,
      tn: dc,
 ...
 */
export async function consumePowerDisrupt(attacker: any): Promise<boolean> {
  const disrupt = getActiveSpecialValue(attacker, 'disrupt');
  if (disrupt <= 0) return true;
  try {
    const system = (attacker as any).system;
    const list: any[] = Array.isArray(system?.statusEffects) ? system.statusEffects : [];
    const next = list.filter(
      (e) => !(e?.id === 'disrupt' || String(e?.name ?? '').toLowerCase() === 'disrupt'),
    );
    await (attacker as any).update({ 'system.statusEffects': next });
    try {
