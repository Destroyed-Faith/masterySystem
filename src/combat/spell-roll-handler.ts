 ...
 */
export function castingBaseTnForMasteryRank(
  masteryRank: number,
  opts?: { mental?: boolean },
): number {
...
export async function rollSpell(params: SpellRollParams): Promise<SpellRollResult> {
  ...
  const spellBase =
    castingBaseTnForMasteryRank(masteryRank, { mental: mentalPower }) +
    (Number(gmModifier) || 0);
  const effectiveSr =
    target && !supportMode ? spellResistanceAfterPenetration(target, actor) : 0;
