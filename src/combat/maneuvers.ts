 ...
 */
export function getAvailableManeuvers(actor: any): ManeuverData[] {
  const maneuvers: ManeuverData[] = [
    {
 ...
 */
export function hasMovementAvailable(actor: any): boolean {
 ...
 */
async function executeDash(actor: any): Promise<void> {
 ...
 */
async function executeDisengage(actor: any): Promise<void> {
  // Set flag for this turn
 ...
 */
async function executeCharge(actor: any, target?: any): Promise<void> {
  if (!target) {
 ...
 */
async function executeLeap(actor: any): Promise<void> {
 ...
 */
async function executeTeleport(actor: any): Promise<void> {
  await ChatMessage.create({
    user: (game as any).user?.id,
    speaker: ChatMessage.getSpeaker({ actor }),
    content: `
 ...
 */
async function executeGenericManeuver(actor: any, maneuver: ManeuverData): Promise<void> {
  await ChatMessage.create({
    user: (game as any).user?.id,
    speaker: ChatMessage.getSpeaker({ actor }),
    content: `
 ...
 */
export function hasDashActive(actor: any): boolean {
 ...
 */
export function getCurrentSpeed(actor: any): number {
  const baseSpeed = actor.system.combat?.speed || 6;
 ...
 */
export function hasChargeBonusVs(actor: any, targetId: string): boolean {
 ...
 */
export async function clearManeuverFlags(actor: any): Promise<void> {
