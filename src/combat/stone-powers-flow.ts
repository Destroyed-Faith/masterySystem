...
interface StonePowersState {
  roundStonesPrompted: Record<number, boolean>; // round -> has been prompted
  stonesDone: Record<string, number>; // combatantId -> round number
  initiativeShopDone: boolean; // Whether initiative shop has been done for round 1
}
...
function areAllCombatantsDone(combat: Combat, round: number): boolean {
  ...
  return allCombatants.every((combatant: Combatant) => {
 ...
 */
async function openInitiativeShopForRound1(combat: Combat): Promise<void> {
  const state = getStonePowersState(combat);
  ...
  // Get all PCs
  const pcs = Array.from(combat.combatants).filter((c: any) => 
    (c as Combatant).actor?.type === 'character'
  ) as Combatant[];

  // Open initiative shop for each PC
  for (const pc of pcs as Combatant[]) {
