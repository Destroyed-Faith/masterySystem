...
function formatPowerPreviewLine(
  item: any,
  baseEvade: number,
  attackTotal: number | null,
): string {
  ...
  if (ev > 0) {
    ...
    if (evEval.unknown) {
      ...
      bits.push(
        `+${ev} Evade: ${evEval.baseEvade}→${evEval.effectiveEvade} vs Attack ${evEval.attackTotal} → <strong style="color:#2ecc71">NEGATES hit</strong>`,
      );
    } else {
      bits.push(
        `+${ev} Evade: ${evEval.baseEvade}→${evEval.effectiveEvade} vs Attack ${evEval.attackTotal} → <strong style="color:#e74c3c">still hits</strong>`,
      );
    }
  }
  if (armor > 0) bits.push(`+${armor} Armor (this hit)`);
  if (dr > 0) bits.push(`+${dr}% DR (this hit)`);
  if (ini > 0) bits.push(`+${ini} Initiative after resolve`);
  if (mech?.tempHP) bits.push(`Temp HP ${mech.tempHP} if damage remains`);
  if (!bits.length) {
...
export async function promptDefenderReactionsBeforeMitigation(params: {
  ...
  const attackTotal =
    Number.isFinite(attackTotalRaw) && params.attackTotal != null ? attackTotalRaw : null;
