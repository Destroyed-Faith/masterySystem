...
export interface DeathSaveData {
  ...
  deathMarks: number;       // 0-3 death marks
  stabilized: boolean;      // Has stabilized (3 successes)
  dead: boolean;            // Has died (3 death marks)
}

/**
 * Get current death save data for actor
 */
export function getDeathSaveData(actor: any): DeathSaveData {
...
export function isIncapacitated(actor: any): boolean {
  ...
  if (healthLevels.length === 0) return false;
 ...
 */
export async function initializeDeathSaves(actor: any): Promise<void> {
  if (!isIncapacitated(actor)) return;
  
  const currentData = getDeathSaveData(actor);
  
  // Don't reset if already stabilized or dead
  if (currentData.stabilized || currentData.dead) return;
 ...
 */
export async function performDeathSave(actor: any): Promise<{ success: boolean; total: number }> {
 ...
 */
export async function addDeathMark(actor: any, isCritical: boolean = false): Promise<void> {
 ...
 */
export async function removeDeathMarksFromHealing(actor: any, healingAmount: number): Promise<void> {
  
  const deathSaveData = getDeathSaveData(actor);
  
  if (deathSaveData.deathMarks === 0) return;
 ...
 */
export async function resetDeathSaves(actor: any): Promise<void> {
