...
function readSkillNumber(value: unknown): number {
  return Math.max(0, Math.floor(Number(value) || 0));
}

function masteryRankPoints(masteryRank: number): number {
  return Math.max(1, Math.floor(Number(masteryRank) || 1));
}

/** Amounts taken this round, oldest first. */
export function combatReflexesRoundSpends(combatant: any): number[] {
...
export function combatReflexesInitiativeState(
  ...
  const rating = Math.min(
    readSkillNumber(actor?.system?.skills?.[CR_SKILL_KEY]),
  ...
  const boxes: CombatReflexesUseBox[] = raw.map((box, index) => ({
    index,
    size: box.size,
    used: box.used,
 ...
 */
export async function spendCombatReflexesUse(
  actor: any,
  combatant: any,
  masteryRank: number
): Promise<number | null> {
  ...
  if (!state.canSpend || state.nextUse <= 0) return null;
 ...
 */
export async function undoCombatReflexesUse(
  actor: any,
  combatant: any,
  masteryRank: number
): Promise<number | null> {
  ...
  const initiative = Math.max(0, Math.floor(Number(combatant.initiative) || 0));
  if (amount <= 0 || initiative < amount) return null;
  const nextInitiative = initiative - amount;
...
export async function resetCombatReflexesRoundUsage(combatant: any): Promise<void> {
  if (!combatReflexesRoundSpends(combatant).length) return;
  await combatant?.setFlag?.('mastery-system', CR_ROUND_FLAG, []);
}
