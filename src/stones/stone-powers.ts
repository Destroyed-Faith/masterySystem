...
type StonePowerDraft = Omit<StonePower, 'effect' | 'premium'>;
...
const PREMIUM_SET = new Set<string>(PREMIUM_STONE_POWER_IDS);

export function isPremiumStonePower(powerId: string): boolean {
...
export function stonePowerSegmentSizes(powerId: string): readonly number[] {
...
export function stonePowerRankCost(powerId: string, rank: number): number {
  const r = Math.floor(Number(rank) || 0);
  if (r < 1 || r > STONE_TIER_HARD_MAX) return 0;
  return stonePowerSegmentSizes(powerId)[r - 1] ?? 0;
}

/** Cumulative Stones to reach `rank` (Normal 1/3/7/15, Premium 2/6/12/20). */
export function cumulativeStoneCostForRank(powerId: string, rank: number): number {
  const r = Math.max(0, Math.min(STONE_TIER_HARD_MAX, Math.floor(Number(rank) || 0)));
  let total = 0;
  for (let t = 1; t <= r; t += 1) total += stonePowerRankCost(powerId, t);
  return total;
}
...
export function highestCompleteStoneTierFromPlaced(
  ...
  for (let r = 1; r <= cap; r += 1) {
    if (r !== prefill) cost += stonePowerRankCost(powerId, r);
    if (n < cost) break;
    best = r;
  }
  return best;
...
function compileEffectText(name: string, tiers: readonly StoneTier[], powerId: string): string {
...
const AGILITY_POWERS_RAW: StonePowerDraft[] = [
  ...
  },
  {
    ...
    category: 'action',
    description:
      'Replace your normal Movement with a move of 4/8/12/16 m. This does not provoke movement-triggered Reactions and does not cost an Attack Action or a Reaction.',
    tiers: [
    ...
    apply: async ({ actor, tier }) => {
      ...
      if (meters <= 0) return;
      await (actor as any).setFlag?.('mastery-system', PENDING_SAFE_MOVEMENT_FLAG, { meters });
    },
  },
  {
    id: 'agility.slip',
    ...
    category: 'reaction',
    description:
      'Once before the start of your next turn, when an enemy misses you with an Attack, you may move 4/8/12/16 m. This does not use your normal Movement and does not provoke movement-triggered Reactions.',
    tiers: [
    ...
    apply: async ({ actor, tier }) => {
      ...
      if (meters <= 0) return;
      const combat = (game as any).combat;
      await (actor as any).setFlag?.('mastery-system', PENDING_SLIP_FLAG, {
        meters,
        used: false,
      ...
      try {
        ...
        if (typeof ChatMessage?.create === 'function') {
          const source = String((actor as any).name || 'Someone')
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;');
...
const INFLUENCE_POWERS_RAW: StonePowerDraft[] = [
  ...
  },
  {
    ...
    category: 'action',
    description:
      'One ally within 8 / 16 / 24 / 32 m gains Regeneration(2 / 4 / 6 / 8).',
    tiers: [
    ...
    apply: async ({ actor, tier }) => {
      ...
      ui.notifications?.info(
        `${(actor as any).name}: Regeneration — choose one ally within ${range} m (${hint})`,
      );
    },
  },
  {
  ...
  },
  {
    ...
    apply: async ({ actor, tier }) => {
      ...
      const meters = scaleStoneTier([8, 16, 24, 32], tier);
      if (enemies <= 0) return;
      await (actor as any).setFlag?.('mastery-system', 'pendingNotATarget', {
        enemies,
        range: meters,
      });
      ui.notifications?.info(
...
const WITS_POWERS_RAW: StonePowerDraft[] = [
  ...
  },
  {
    ...
    oncePerCombat: true,
    description:
      'Premium. Once per combat, gain Phasing Charges that persist until spent or combat ends (R1–R4: 1/2/3/4).',
    tiers: [
    ...
    apply: async ({ actor, combatant, tier }) => {
      if (combatant && isPhasingStoneUsedThisCombat(combatant)) {
        ui.notifications?.warn(`${(actor as any).name}: Phasing already used this combat.`);
        return;
      }
      const charges = scaleStoneTier([1, 2, 3, 4], tier);
      if (charges <= 0) return;
      ...
      if (combatant) await markPhasingStoneUsedThisCombat(combatant);
      ui.notifications?.info(
        `${(actor as any).name}: Phasing — ${charges} charge(s) (once per combat).`,
      );
    },
  },
  {
...
function leadPremium(list: StonePower[]): StonePower[] {
...
export function stonePaymentLanesForTier(powerId: string, rank: number): number[] {
  const r = Math.floor(Number(rank) || 0);
  if (r < 1 || r > STONE_TIER_HARD_MAX) return [];
  const sizes = stonePowerSegmentSizes(powerId);
  let start = 0;
  for (let i = 0; i < r - 1; i += 1) start += sizes[i] ?? 0;
  const size = sizes[r - 1] ?? 0;
  return Array.from({ length: size }, (_, i) => start + i);
}

/** Total payment lanes for this Ability (Normal 15, Premium 20). */
export function stonePaymentLaneCount(powerId: string): number {
