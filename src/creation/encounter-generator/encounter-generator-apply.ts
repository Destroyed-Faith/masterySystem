...
function cycleEntryToAttackRow(entry: CyclePowerEntry): Record<string, unknown> {
...
function tacticsHtml(plan: EncounterProjectPlan): string {
  const lines: string[] = [];
  lines.push('<h3>Taktik</h3><ul>');
  for (const t of plan.tactics) lines.push(`<li>${t}</li>`);
  lines.push('</ul>');
...
export function buildProjectAddSystem(plan: EncounterProjectPlan): Record<string, unknown> | null {
  ...
  return {
    ...
    bio: {
      description:
        `<p>Add-Prototyp — bei Bedarf duplizieren. Spawn: ${adds.spawnPerRound}/Runde, max. ${adds.maxActive} aktiv.` +
...
export function buildProjectEnvironmentSystem(plan: EncounterProjectPlan): Record<string, unknown> | null {
...
function fmtSpecial(special: string | null, value: number): string {
  if (!special) return '—';
  return `${specialLabel(special)}(${value})`;
}

function cycleTableHtml(cycle: CyclePowerEntry[], cycleStyle: string): string {
  ...
  return (
    '<table><thead><tr><th>#</th><th>Power</th><th>Pool</th><th>Schaden</th><th>Special</th><th>Reichweite</th><th>Hinweis</th></tr></thead><tbody>' +
    rows.join('') +
    '</tbody></table>'
  );
}
...
export function buildNpcSheetHtml(name: string, plan: EncounterProjectPlan): string {
  const boss = plan.boss;
  const parts: string[] = [];
  ...
  plan.phasePlans.forEach((p) => {
    if (plan.phasePlans.length > 1) parts.push(`<h4>${p.name}</h4>`);
    if (p.changes.length) parts.push(`<p><em>${p.changes.join(' · ')}</em></p>`);
    parts.push(cycleTableHtml(p.cycle, plan.concept.cycleStyle));
  });

  if (plan.tactics.length) {
    parts.push('<h3>Tactics</h3><ul>');
    for (const t of plan.tactics) parts.push(`<li>${t}</li>`);
    parts.push('</ul>');
  }
  return parts.join('');
}
...
export function buildSummaryHtml(
  ...
  if (plan.notes.length) {
    ...
    for (const n of plan.notes) parts.push(`<li>${n}</li>`);
    parts.push('</ul>');
  }

  if (report.warnings.length) {
    parts.push('<h3>Balancing-Warnungen</h3><ul>');
    for (const w of report.warnings) parts.push(`<li>⚠ ${w}</li>`);
    parts.push('</ul>');
  }
  return parts.join('');
}
 ...
 */
export async function applyEncounterProject(
  name: string,
  party: PartyMetrics,
  plan: EncounterProjectPlan,
  report: ThreatReport,
