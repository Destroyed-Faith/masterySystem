...
const PASSIVE_TEMPLATE_CATEGORIES: Readonly<Record<string, readonly string[]>> = {
...
const SUBFAMILY_DEFAULT_CATEGORY: Readonly<Record<string, string>> = {
    armor: 'armor',
    evade: 'evade',
    'damage-reduction': 'damage-reduction',
    'damage-negation': 'damage-negation',
...
export type SecondPassiveBucket =
    | 'armor'
    | 'evade'
    | 'parry'
    | 'damage-reduction'
...
export function normalizePassiveCategory(category: string): string {
...
export function inferPassiveCategoriesFromTemplateId(templateId: string): string[] {
    const id = templateId.toLowerCase();
    const cats = new Set<string>();

    if (id.includes('damage-reduction')) cats.add('damage-reduction');
    if (id.includes('damage-negation') || id.includes('negation')) cats.add('damage-negation');
    if (id.includes('ghostform') || id.includes('phasing')) cats.add('phasing');
    if (id.includes('passive-parry') || id.endsWith('-parry')) cats.add('parry');
    if (id.includes('invisibility')) cats.add('invisibility');
    ...
    if (id.includes('passive-initiative')) cats.add('initiative');
    if (id.includes('temp-hp')) cats.add('temporary-hp');
    if (id.includes('deep-vitality') || id.includes('-health-') || id.includes('passive-health') || id.includes('conditional-passive-health')) {
        cats.add('health');
    }
    if (id.includes('healing')) cats.add('healing');
    if (id.includes('regeneration') || id === 'passive-regeneration') cats.add('regeneration');
    if (id.includes('blood-feast') || id.includes('battle-trance') || id.includes('stillness-recovery') || id.includes('recovery')) {
        cats.add('recovery');
    }
    if (id.includes('heightened-senses') || id.includes('awareness')) cats.add('awareness');
    if (id.includes('spell-resistance') || id.includes('ward')) cats.add('ward');
    if (id.includes('special-aura')) cats.add('special-aura');
    if (
        id.includes('killing-intent')
...
export function getPassiveMechanicalCategories(templateId: string): string[] {
    const explicit = PASSIVE_TEMPLATE_CATEGORIES[templateId];
    if (explicit?.length) return [...explicit].map(normalizePassiveCategory);
    ...
    if (entry?.subfamily) return [normalizePassiveCategory(entry.subfamily)];
    return [];
}

export function getNormalizedPassiveCategories(templateId: string): string[] {
    return getPassiveMechanicalCategories(templateId);
}

export function formatPassiveCategoryLabel(category: string): string {
...
export function formatPassiveCategoryList(templateId: string): string {
...
export function passivesAreCategoryCompatible(passive1TemplateId: string, passive2TemplateId: string): boolean {
    ...
    for (const category of b) {
        if (a.has(category)) return false;
    }
    return true;
}

function isHiddenFromGuidedSecondPassive(categories: string[]): boolean {
    return categories.some((c) => GUIDED_SECOND_PASSIVE_HIDDEN_CATEGORIES.has(normalizePassiveCategory(c)));
}

function isLegalForEchoAndPrerequisites(templateId: string, actorEchoKey?: string | null): boolean {
    const entry = findCatalogEntry(templateId);
...
export function isAllowedSecondPassive(
    passive2TemplateId: string,
    passive1TemplateId: string,
    actorEchoKey?: string | null,
): boolean {
    if (!passive1TemplateId || !passive2TemplateId) return false;
    if (passive2TemplateId === passive1TemplateId) return false;

    const categories2 = getNormalizedPassiveCategories(passive2TemplateId);
    if (!categories2.length) return false;
    if (isHiddenFromGuidedSecondPassive(categories2)) return false;
    if (!passivesAreCategoryCompatible(passive1TemplateId, passive2TemplateId)) return false;
    if (!isLegalForEchoAndPrerequisites(passive2TemplateId, actorEchoKey)) return false;

    return true;
}

export function getPassiveCategoryConflictMessage(
    passive1TemplateId: string,
    passive2TemplateId: string,
): string | null {
...
export function secondPassiveBucketFor(templateId: string): SecondPassiveBucket {
    const c = new Set(getNormalizedPassiveCategories(templateId));

    if (c.has('damage-reduction')) return 'damage-reduction';
    if (c.has('phasing')) return 'phasing';
    if (c.has('damage-negation')) return 'damage-negation';
    if (c.has('parry')) return 'parry';
    if (c.has('invisibility')) return 'invisibility';

    if (c.has('special-aura')) return 'offense';

    if (c.has('damage')) {
        if (c.has('healing') && !c.has('evade') && !c.has('armor')) return 'sustain';
        return 'offense';
    }

    if (c.has('evade')) return 'evade';
    if (c.has('armor')) return 'armor';
    if (c.has('health')) return 'health';
    if (c.has('temporary-hp')) return 'temporary-hp';

    if (c.has('regeneration') || c.has('recovery') || c.has('healing')) return 'sustain';

    return 'advanced';
}

export function secondPassiveCardWarning(_templateId: string): string | undefined {
    return undefined;
}
