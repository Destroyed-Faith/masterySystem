...
const OFFENSE_SYNTHETIC_GROUP_LABELS: Record<string, string> = {
...
function isNormalWeaponAttack(entry: CatalogEntry): boolean {
    return /active-(?:melee|ranged)-weapon-(single|aoe|split)$/.test(entry.templateId)
        || entry.templateId === 'active-ranged-weapon-autofire';
}

function isControlActive(entry: CatalogEntry): boolean {
    return entry.subfamily === 'control' || entry.subfamily === 'hard-control';
}

function isHealSupportActive(entry: CatalogEntry): boolean {
    if (entry.subfamily === 'support-heal' || entry.subfamily === 'support-cleanse') return true;
    return entry.templateId.includes('heal-cleanse-mixed');
}

function offenseCatalogGroupKey(entry: CatalogEntry): string {
    if (isNormalWeaponAttack(entry)) return OFFENSE_GROUP_NORMAL;
    if (isControlActive(entry)) return OFFENSE_GROUP_CONTROL;
    if (isHealSupportActive(entry)) return OFFENSE_GROUP_HEAL;
    if (entry.chosenSpecial?.key) return entry.chosenSpecial.key;
    return OFFENSE_UTILITY_GROUP_KEY;
}

function offenseGroupSortIndex(groupKey: string): number {
    const fixed = (OFFENSE_GROUP_ORDER as readonly string[]).indexOf(groupKey);
    if (fixed >= 0) return fixed;
    if (groupKey === OFFENSE_UTILITY_GROUP_KEY) return 1000;
    return 100;
}

function offensePatternSortIndex(groupKey: string, patternId: string): number {
...
function offenseSpecialGroupTooltip(groupKey: string, specialKey: string | null): string {
    if (groupKey === OFFENSE_GROUP_NORMAL) {
