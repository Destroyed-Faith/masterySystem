...
let singleDieCache: Pmf | null = null;

/** PMF of one exploding d8, truncated at EXPLOSION_DEPTH explosions. */
export function singleDiePmf(): Pmf {
  if (singleDieCache) return singleDieCache;
...
const sumCache = new Map<number, Pmf>();
 ...
 */
export function explodingSumPmf(count: number): Pmf {
  const n = Math.max(0, Math.floor(count));
  if (n === 0) return Float64Array.of(1);
...
const plainCache = new Map<number, Pmf>();

/** PMF of the sum of `count` plain d8 (damage dice never explode by default). */
export function plainDamagePmf(count: number): Pmf {
  const n = Math.max(0, Math.floor(count));
  if (n === 0) return Float64Array.of(1);
  ...
  for (let s = 0; s < prev.length; s += 1) {
    const p = prev[s];
    if (p === 0) continue;
    for (let v = 1; v <= 8; v += 1) out[s + v] += p / 8;
  }
  plainCache.set(n, out);
  return out;
}

const jointCache = new Map<number, Pmf[]>();
 ...
 */
export function damageJointPmf(count: number): Pmf[] {
  const n = Math.max(0, Math.floor(count));
  const key = n;
  ...
  const out: Pmf[] = [];
  for (let e = 0; e <= n; e += 1) out.push(new Float64Array(8 * n + 1));
  for (let e = 0; e < prev.length; e += 1) {
    const pmf = prev[e];
    for (let s = 0; s < pmf.length; s += 1) {
      const p = pmf[s];
      if (p === 0) continue;
      for (let v = 1; v <= 7; v += 1) out[e][s + v] += p / 8;
      out[e + 1][s + 8] += p / 8;
    }
  }
  jointCache.set(key, out);
...
const poolKeepCache = new Map<string, Pmf>();
...
function binomialPmf(n: number, p: number): number[] {
  ...
  for (let t = 0; t <= n; t += 1) {
    out[t] = prob;
    if (t < n) prob = prob * ((n - t) / (t + 1)) * ratio;
  }
  return out;
}
 ...
 * sampling.
 */
export function poolKeepPmf(pool: number, keep: number): Pmf {
  ...
  const values: number[] = [];
  for (let v = MAX_DIE_VALUE; v >= 1; v -= 1) if (die[v] > 0) values.push(v);
  ...
  // states[r][j] = PMF over partial sums (weighted by state probability).
  type StateGrid = (Float64Array | null)[][];
  let states: StateGrid = Array.from({ length: n + 1 }, () =>
    new Array<Float64Array | null>(k + 1).fill(null),
  );
  states[n][k] = (() => {
  ...
  const absorb = (pmf: Float64Array) => {
    for (let s = 0; s <= maxSum; s += 1) if (pmf[s] > 0) result[s] += pmf[s];
  };

  for (const v of values) {
    const next: StateGrid = Array.from({ length: n + 1 }, () =>
      new Array<Float64Array | null>(k + 1).fill(null),
    );
        ...
        for (let t = 0; t <= r; t += 1) {
          ...
          if (shift === 0) {
            for (let s = 0; s <= maxSum; s += 1) {
              if (pmf[s] > 0) target[s] += pmf[s] * b;
            }
          } else {
            for (let s = 0; s + shift <= maxSum; s += 1) {
              if (pmf[s] > 0) target[s + shift] += pmf[s] * b;
            }
          }
        }
      }
...
export function pmfMean(pmf: Pmf): number {
  let m = 0;
  for (let s = 0; s < pmf.length; s += 1) m += s * pmf[s];
  return m;
}

/** P(X >= tn). */
export function pmfAtLeast(pmf: Pmf, tn: number): number {
  const t = Math.max(0, Math.ceil(tn));
  let p = 0;
  for (let s = t; s < pmf.length; s += 1) p += pmf[s];
  return p;
}
...
export function pmfQuantile(pmf: Pmf, q: number): number {
  ...
  for (let s = 0; s < pmf.length; s += 1) {
    acc += pmf[s];
    if (acc >= target - 1e-12) return s;
  }
  return pmf.length - 1;
}

/** Hit chance of a pool/keep roll against a target number. */
export function hitChance(pool: number, keep: number, tn: number): number {
 ...
 */
export function expectedRaises(pool: number, keep: number, tn: number, raiseStep = 4): number {
...
export function expectedOverThreshold(pmf: Pmf, reduction: number): number {
  const r = Math.max(0, reduction);
  let e = 0;
  for (let s = 0; s < pmf.length; s += 1) {
    if (pmf[s] > 0 && s > r) e += pmf[s] * (s - r);
  }
  return e;
}

/** Clear all caches (only used by tests measuring determinism/perf). */
export function clearProbabilityCaches(): void {
  singleDieCache = null;
  sumCache.clear();
  plainCache.clear();
  jointCache.clear();
