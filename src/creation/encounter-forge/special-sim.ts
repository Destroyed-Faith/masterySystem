 ...
 * Decay 1 per stack.
 */
export function processTurnStart(stacks: SpecialStacks, victimMr: number): TurnStartResult {
  let tickDamage = 0;
  let tickStress = 0;
  for (const [id, value] of stacks) {
    if (value <= 0) continue;
    if (TICK_DAMAGE.has(id)) tickDamage += value;
    if (id === 'blight') tickStress += value;
  }
  ...
  if (root > 0) {
    ...
    if (next > 0) stacks.set('root', next);
    else stacks.delete('root');
  }
  ...
  while (budget > 0) {
    ...
    let bestValue = 0;
    for (const [id, value] of stacks) {
      if (id === 'root' || id === 'regeneration') continue;
      if (value > bestValue) {
        bestValue = value;
        bestId = id;
      }
    }
    if (!bestId || bestValue <= 0) break;
    const take = Math.min(budget, bestValue);
    budget -= take;
    const next = bestValue - take;
    if (next > 0) stacks.set(bestId, next);
    else stacks.delete(bestId);
  }
  // Decay 1 on everything remaining (diminishing rule).
  for (const [id, value] of [...stacks]) {
    if (id === 'root') continue;
    const next = value - SPECIAL_DECAY_PER_TURN;
    if (next > 0) stacks.set(id, next);
    else stacks.delete(id);
  }
...
export function armorDelta(stacks: SpecialStacks): number {
  let d = 0;
  for (const id of ARMOR_REDUCERS) d += stacks.get(id) ?? 0;
  return -d;
}

export function evadeDelta(stacks: SpecialStacks): number {
  let d = 0;
  for (const id of EVADE_REDUCERS) d += stacks.get(id) ?? 0;
  return -d;
}

export function attackDiceDelta(stacks: SpecialStacks): number {
  let d = 0;
  for (const id of ATTACK_DICE_REDUCERS) d += stacks.get(id) ?? 0;
  return -d;
}

/** Pool reduction for physical (might/agility) attackers. */
export function physicalPoolDelta(stacks: SpecialStacks): number {
  let d = 0;
  for (const id of PHYSICAL_POOL_REDUCERS) d += stacks.get(id) ?? 0;
  return -d;
}

/** Pool reduction for casting attackers. */
export function castingPoolDelta(stacks: SpecialStacks): number {
  let d = 0;
  for (const id of CASTING_POOL_REDUCERS) d += stacks.get(id) ?? 0;
  return -d;
}

/** Bonus damage dice against this victim: Sundered (non-spell) / Hex (spell). */
export function bonusDiceVsVictim(stacks: SpecialStacks, isSpellHit: boolean): number {
 ...
 */
export function expectedMovementDamage(stacks: SpecialStacks, expectedMovesPerRound: number): number {
...
export function totalNegativeStacks(stacks: SpecialStacks): number {
  let total = 0;
  for (const [id, value] of stacks) {
    if (id === 'regeneration') continue;
