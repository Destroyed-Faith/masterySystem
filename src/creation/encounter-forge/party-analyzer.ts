...
export interface PcCombatProfile {
  ...
  canCleanse: boolean;
  defense: { baseline: DefenseBand; sustained: DefenseBand; burst: DefenseBand };
  offense: { baseline: OffenseBand; sustained: OffenseBand; burst: OffenseBand };
...
export function parseSpecialStrings(list: unknown): PcSpecialOnHit[] {
  ...
  for (const entry of list) {
    if (entry && typeof entry === 'object') {
      const key = String((entry as any).key ?? (entry as any).id ?? '').trim();
      const value = num((entry as any).rank ?? (entry as any).value ?? (entry as any).tier, 0);
      const id = canonicalSpecialId(key.toLowerCase());
      if (id && value > 0) out.push({ id, value });
      continue;
    }
...
function isMentalPower(sys: any): boolean {
  const tags: string[] = Array.isArray(sys?.tags) ? sys.tags.map((t: unknown) => String(t).toLowerCase()) : [];
  const tid = String(sys?.templateId ?? '').toLowerCase();
  return (
    tags.includes('mental') ||
    /mental/i.test(tid) ||
...
function isLimitedPower(sys: any): boolean {
  if (sys?.oncePerCombat === true || sys?.oncePerEncounter === true) return true;
  const freq = String(sys?.frequency ?? sys?.useLimit ?? '').toLowerCase();
  if (freq.includes('encounter') || freq.includes('combat') || freq.includes('once')) return true;
  const uses = num(sys?.usesPerCombat ?? sys?.uses?.max, 0);
  return uses > 0 && uses <= 2;
}

function isActiveBuffItem(it: any): boolean {
  const sys = it?.system ?? {};
  const cat = String(sys.category ?? sys.powerType ?? sys.subfamily ?? '').toLowerCase();
  return cat.includes('buff') || sys.isActiveBuff === true;
}

function powerLevelRow(sys: any): any | null {
  ...
  if (Array.isArray(levels)) {
    return levels.find((row: any) => Number(row?.level) === rank) ?? levels[rank - 1] ?? null;
  }
  return levels[String(rank)] ?? levels[String(level)] ?? null;
}

function rawAttackScore(a: PcAttackProfile): number {
...
export function analyzePc(actor: any): PcCombatProfile {
  ...
  if (!system.mastery?.rank && !combat.evadeTotal && !system.health?.bars) {
    warnings.push(
      `${actor?.name ?? '?'}: sieht nicht nach einem Mastery-Charakter aus — Fallbacks (MR ${mr}, Evade ${baseEvadeForMr(mr)}) werden verwendet.`,
    );
  }
  ...
  try {
    ...
    for (const c of collectMechanicsContributions(actor)) {
      const dn = num((c.mechanics as any)?.damageNegationDice ?? (c.mechanics as any)?.damageNegation, 0);
      if (dn > 0) mechDamageNegation = Math.max(mechDamageNegation, dn);
    }
  } catch {
  ...
  for (const it of items) {
    ...
    if (tid.includes('parry')) {
      const level = Math.max(1, num(sys.level ?? sys.rank, 1));
      parryPoolMax = Math.max(
        parryPoolMax,
        Math.min(Math.max(might, agility), passiveParryPoolForLevel(level)),
      );
    }
    ...
    if (dn > 0) damageNegationDice = Math.max(damageNegationDice, dn);
  }
  ...
  const defenseBaseline: DefenseBand = {
  ...
  const defenseSustained: DefenseBand = {
  ...
  const defenseBurst: DefenseBand = {
    ...defenseSustained,
  ...
  if (weaponDamage.dice === 0 && weaponDamage.flat === 0) weaponDamage.dice = 1;
  ...
  const basicAttack: PcAttackProfile = {
  ...
  const sustainedAttacks: PcAttackProfile[] = [basicAttack];
  const burstOnlyAttacks: PcAttackProfile[] = [];
  ...
  for (const it of items) {
    ...
    if (sys.chosenSpecial?.key) {
      ...
      if (id && value > 0 && !specials.some((s) => s.id === id)) specials.push({ id, value });
    }
    ...
    if (castKey && num((attributes as any)[castKey]?.value, 0) > 0) {
      castAttr = num((attributes as any)[castKey].value, 2);
    }

    let profile: PcAttackProfile;
    if (spell) {
      profile = {
    ...
    if (limited) burstOnlyAttacks.push(profile);
    else sustainedAttacks.push(profile);
  }

  const pickBest = (list: PcAttackProfile[]) =>
  ...
  const sustainedBand: OffenseBand = {
  ...
  const knownBuffs: KnownBuffPotential[] = [];
  for (const it of items) {
    if (it?.type !== 'power' || !isActiveBuffItem(it)) continue;
    const row = powerLevelRow(it.system);
    const mech = row?.mechanics ?? it.system?.mechanics ?? {};
    const potential: KnownBuffPotential = { name: String(it.name ?? 'Buff') };
    if (num(mech.evade, 0) > 0) potential.evade = num(mech.evade, 0);
    if (num(mech.armor, 0) > 0) potential.armor = num(mech.armor, 0);
    if (num(mech.damageReductionPct, 0) > 0) potential.drPct = num(mech.damageReductionPct, 0);
    if (num(mech.spellResistance, 0) > 0) potential.spellResistance = num(mech.spellResistance, 0);
    if (num(mech.wardIncoming, 0) > 0) potential.ward = num(mech.wardIncoming, 0);
    if (
      potential.evade ||
      potential.armor ||
      potential.drPct ||
