...
import {
  ENCOUNTER_FORGE_LIMITS,
  PRIMARY_DEFENSE_PILLARS,
  SECONDARY_DEFENSE_OPTIONS,
  defaultAttackConcept,
  defaultEncounterDesign,
  defaultMainEnemy,
  forgeId,
  syncPhaseCount,
  type AddGroupConcept,
  type DefenseKind,
  type EncounterDesign,
  type MainEnemyConcept,
} from './encounter-model.js';
import { analyzePartyActors, type PartyProfile } from './party-analyzer.js';
import { solveEncounterForParty, type EncounterSolution } from './solve-encounter.js';
import { validateEncounter, type EncounterWarning } from './encounter-validator.js';
...
const STEPS = ['party', 'enemies', 'actions', 'phases', 'review'] as const;
type ForgeStep = (typeof STEPS)[number];

const STEP_LABELS: Record<ForgeStep, string> = {
...
const DEFENSE_LABELS: Record<DefenseKind, string> = {
  evade: 'Evade',
  armor: 'Armor',
  parry: 'Parry',
...
function defenseOptions(kinds: DefenseKind[], current: DefenseKind | '' | undefined) {
  return kinds.map((k) => ({
    value: k,
...
function specialOptions(current: string | null) {
...
export class EncounterForgeDialog extends BaseDialog {
  private step: ForgeStep = 'party';
  private design: EncounterDesign = defaultEncounterDesign();
  private solveKey = '';
  private solution: EncounterSolution | null = null;
  private warnings: EncounterWarning[] = [];
  private party: PartyProfile | null = null;
  ...
  #selectedActors(): any[] {
    ...
    return this.#characterActors().filter((a: any) => ids.has(String(a.id)));
  }

  #ensureParty(): PartyProfile {
  ...
  /** Deterministic + memoized: identical design/party -> identical solution. */
    ...
    if (key === this.solveKey && this.solution) return;
  ...
  #enemy(enemyId: string): MainEnemyConcept | null {
    return this.design.enemies.find((e) => e.id === enemyId) ?? null;
  }

  #addGroup(groupId: string): AddGroupConcept | null {
  ...
  async _prepareContext(): Promise<Record<string, unknown>> {
    ...
    if (this.step === 'party') ctx.party = this.#partyContext();
    if (this.step === 'enemies') ctx.enemies = this.#enemiesContext();
    if (this.step === 'actions') ctx.actions = this.#actionsContext();
    if (this.step === 'phases') ctx.phases = this.#phasesContext();
    if (this.step === 'review') ctx.review = this.#reviewContext();
    return ctx;
  }
  ...
  #partyContext() {
    ...
    return {
      actors: this.#characterActors().map((a: any) => {
    ...
    return {
      ...
      list: this.design.enemies.map((enemy) => {
        ...
        const taken = (slot: 'primary' | 'secondary' | 'tertiary') => {
          const chosen = [phase.defenses.primary, phase.defenses.secondary, phase.defenses.tertiary];
          const own = phase.defenses[slot];
          return (k: DefenseKind) => chosen.includes(k) && k !== own;
        };
        return {
          id: enemy.id,
          name: enemy.name,
          concept: enemy.concept,
          primaryOptions: defenseOptions(PRIMARY_DEFENSE_PILLARS, phase.defenses.primary).map(
            (o) => ({ ...o, disabled: o.disabled || taken('primary')(o.value as DefenseKind) }),
          ),
          secondaryOptions: [
            ...
            ...defenseOptions(SECONDARY_DEFENSE_OPTIONS, phase.defenses.secondary).map((o) => ({
              ...o,
              disabled: o.disabled || taken('secondary')(o.value as DefenseKind),
            })),
          ],
          tertiaryOptions: [
            ...
            ...defenseOptions(SECONDARY_DEFENSE_OPTIONS, phase.defenses.tertiary).map((o) => ({
              ...o,
              disabled: o.disabled || taken('tertiary')(o.value as DefenseKind),
            })),
          ],
          ...
          copies: enemy.copies,
          reactionOptions: NPC_STANDARD_REACTIONS.map((r: any) => ({
            value: r.id,
            label: r.name,
  ...
  #phasesContext() {
    ...
    return {
      ...
      addGroups: this.design.addGroups.map((g) => ({
        ...
        arrivalRound:
          g.arrival.type === 'reinforcement' && g.arrival.trigger.kind === 'round'
  ...
  _onRender(): void {
    ...
    root.querySelectorAll<HTMLElement>('[data-field]').forEach((el) => {
      el.addEventListener('change', () => this.#onFieldChange(el as any));
    });
  }

  async #onAction(ev: Event, el: HTMLElement): Promise<void> {
    ev.preventDefault();
    ...
    switch (action) {
      ...
      case 'goto':
        if ((el.dataset.step as ForgeStep) && STEPS.includes(el.dataset.step as ForgeStep)) {
          this.step = el.dataset.step as ForgeStep;
        }
        break;
      case 'add-enemy':
        if (this.design.enemies.length < ENCOUNTER_FORGE_LIMITS.maxEnemies) {
          this.design.enemies.push(
            defaultMainEnemy(`Main Enemy ${this.design.enemies.length + 1}`, this.design.phaseCount),
          );
        }
        break;
      case 'remove-enemy':
      ...
      case 'add-attack': {
        ...
        if (enemy && enemy.attacks.length < ENCOUNTER_FORGE_LIMITS.maxAttacksPerEnemy) {
          ...
          for (const phase of enemy.phases) phase.attackIds.push(atk.id);
        }
        break;
      }
      case 'remove-attack': {
      ...
      case 'generate': {
        ...
        if (result) {
          ui?.notifications?.info(
            `Encounter „${this.design.name}" erstellt: ${result.actorCount} Actor(s) + Journal.`,
          );
          (this as any).close();
          return;
        }
  ...
  #onFieldChange(el: HTMLInputElement | HTMLSelectElement): void {
    ...
    switch (field) {
      ...
      // Step 2
      case 'enemy-name':
        if (enemy) enemy.name = value;
        return;
      case 'enemy-concept':
        if (enemy) enemy.concept = value;
        return;
      case 'defense-primary':
        if (enemy) enemy.phases[phaseIdx].defenses.primary = value as DefenseKind;
        break;
      case 'defense-secondary':
        if (enemy) enemy.phases[phaseIdx].defenses.secondary = (value || undefined) as DefenseKind | undefined;
        break;
      case 'defense-tertiary':
        if (enemy) enemy.phases[phaseIdx].defenses.tertiary = (value || undefined) as DefenseKind | undefined;
        break;
      case 'movement-kind':
        if (enemy) {
          for (const p of enemy.phases) {
            p.movement.kind = value as any;
        ...
        break;
      case 'escapes-melee':
        if (enemy) for (const p of enemy.phases) p.movement.escapesMelee = checked;
        break;
      case 'reaction-toggle': {
        if (enemy) {
          const rid = el.dataset.reaction as any;
          for (const p of enemy.phases) {
            ...
            if (!checked && has) p.reactions = p.reactions.filter((r) => r.id !== rid);
          }
        }
        break;
      }
      case 'copies-enabled':
        if (enemy) enemy.copies.enabled = checked;
        break;
      case 'copies-count':
        if (enemy) enemy.copies.count = Math.max(1, Math.min(6, Number(value) || 1));
        break;
      case 'copies-health':
        if (enemy) enemy.copies.health = value === 'shared' ? 'shared' : 'independent';
        break;
      case 'copies-fragile':
        if (enemy) enemy.copies.fragile = checked;
        break;
      case 'copies-attack':
        if (enemy) enemy.copies.attack = checked;
        break;
      // Step 3
      case 'attack-name':
        if (attack) attack.name = value;
        return;
      case 'attack-spell':
        if (attack) attack.resolution = checked ? 'spell' : 'martial';
        break;
      case 'attack-ranged':
        if (attack) {
      ...
      case 'attack-area':
        if (attack) {
          attack.area = value as any;
          if (attack.area !== 'single' && attack.areaSize <= 0) attack.areaSize = 3;
        }
        break;
      case 'attack-area-size':
        if (attack) attack.areaSize = Math.max(0, Number(value) || 0);
        break;
      case 'attack-range':
        if (attack) attack.range = Math.max(0, Number(value) || 0);
        break;
      case 'attack-special':
        if (attack) attack.specialId = value || null;
        break;
      case 'attack-stress':
        if (attack) attack.stress = checked;
        break;
      case 'actions-override':
        if (enemy) enemy.phases[0].overrides.offensiveActions = numOrNull();
        break;
      // Step 4
      case 'phase-attack-toggle': {
        if (enemy) {
          ...
          if (checked && !has) p.attackIds.push(atkId);
          if (!checked && has) p.attackIds = p.attackIds.filter((id) => id !== atkId);
        }
        break;
      }
      case 'phase-note':
        if (enemy) enemy.phases[phaseIdx].mechanicsNote = value;
        return;
      case 'group-name':
        if (group) group.name = value;
        return;
      case 'group-count':
        if (group) group.count = Math.max(1, Math.min(12, Number(value) || 1));
        break;
      case 'group-role':
        if (group) group.role = value as any;
        break;
      case 'group-arrival':
        if (group) {
          if (value === 'fixed') group.arrival = { type: 'fixed' };
      ...
      case 'group-round':
        ...
        break;
      case 'group-summoner':
        if (group && group.arrival.type === 'summon') group.arrival.summonerEnemyId = value;
        break;
      case 'group-hits':
        if (group) group.hitsToKill = Math.max(1, Math.min(6, Number(value) || 1));
        break;
      case 'group-attacks':
        if (group) group.attacks = checked;
        break;
      case 'group-special':
        if (group) group.specialId = value || null;
        break;
      // Step 5 overrides
      case 'override-health':
        if (enemy) enemy.phases[phaseIdx].overrides.health = numOrNull();
        break;
      case 'override-actions':
        if (enemy) enemy.phases[phaseIdx].overrides.offensiveActions = numOrNull();
        break;
      default:
        return;
    }
...
let forgeInstance: EncounterForgeDialog | null = null;

export function showEncounterForgeDialog(): void {
  if (!(game as any).user?.isGM) {
    ui?.notifications?.warn('Nur der Spielleiter kann den Encounter Forge öffnen.');
    return;
  }
  if (!forgeInstance) forgeInstance = new EncounterForgeDialog();
