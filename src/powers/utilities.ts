...
export interface ActiveUtility {
  ...
  utilityItemId: string;   // ID of the utility item
  utilityName: string;     // Name of the utility
  startRound: number;      // Round it was activated
  duration: number;        // Duration in rounds (0 = instant)
  endsOnRound: number;     // Round it expires
  targetId?: string;       // Optional target actor ID
}

/**
 * Get active utilities for an actor
 */
export function getActiveUtilities(actor: any): ActiveUtility[] {
 ...
 */
export function isUtilityActive(actor: any, utilityItemId: string): boolean {
  const activeUtilities = getActiveUtilities(actor);
  return activeUtilities.some((util: ActiveUtility) => util.utilityItemId === utilityItemId);
}
 ...
 */
export async function activateUtility(
  actor: any,
  utilityItem: any,
  targetActor?: any
): Promise<boolean> {
  ...
  const newUtility: ActiveUtility = {
 ...
 */
export async function decrementUtilityDurations(actor: any): Promise<void> {
  const combat = (game as any).combat;
  if (!combat) return;
  ...
  const stillActive = activeUtilities.filter((util: ActiveUtility) => {
 ...
 */
export async function removeUtility(actor: any, utilityId: string): Promise<void> {
  const activeUtilities = getActiveUtilities(actor);
  const filtered = activeUtilities.filter((util: ActiveUtility) => util.id !== utilityId);
 ...
 */
export function getUtilityStatusSummary(actor: any): string[] {
  const activeUtilities = getActiveUtilities(actor);
  const combat = (game as any).combat;
  const currentRound = combat?.round || 1;
  
  return activeUtilities.map((util: ActiveUtility) => {
