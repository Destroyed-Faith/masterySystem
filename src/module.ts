...
} from './vision/token-vision-defaults.js';
  ...
  Hooks.on('renderCombatTracker', (_app: any, html: any) => {
    ...
    $html.find('.combatant').each((_index: number, combatantElement: HTMLElement) => {
      ...
      if (isCurrent && canViewerSeeEndTurn(combatant.actor, game.user)) {
        ...
        delayBtn.off('click.ms-delay-turn').on('click.ms-delay-turn', async (ev: JQuery.ClickEvent) => {
          ev.preventDefault();
          ev.stopPropagation();
...
Hooks.once('ready', async function() {
  ...
  const { registerContestCardClickHandler } = await import('./contests/contest-card.js');
  registerContestCardClickHandler();
 ...
 */
Hooks.on('deleteItem', (item: any, options: any) => {
  try {
    ...
    void import('./utils/minor-magic-items.js').then(({ onMinorMagicItemDeleted }) => {
      void onMinorMagicItemDeleted(item, options);
    });
  } catch {
    /* ignore */
  }
