...
- **Martial-Skill-Erstattung:** Entfernte Kampf-Skills geben Startpunkte wieder als ungenutzte Character-Creation-Skill-Points zurück. Später mit XP gekaufte Stufen kommen als Free XP (`points.xpFree`) zurück, nicht als normales XP. `freeEarned`, Lifetime XP und die verdienten Zähler bleiben unverändert. Läuft die Migration kein zweites Mal. Unklare Historie wird nur markiert, nicht geraten.
...
- **Grapple als Kampfmanöver:** Grapple erscheint im Angriffs-Segment des Radial-Menüs neben Basic Attack (Melee Reach, 1 Attack Action). Es nutzt denselben Opposed-Attribute-Contest-Resolver wie der Bogen: Initiator wählt Might oder Agility, Ziel wählt Might oder Agility, höher gewinnt. Gewinnt der Initiator, sind **beide** Kreaturen miteinander Grappled (Speed 0 m, kein freiwilliges Wegbewegen). Option **Target is Unaware** (Vorschlag aus Surprise-Status / Wahrnehmung, immer per Häkchen überschreibbar): Advantage für den Initiator, Disadvantage für das Ziel — nur für den ersten Contest. **Escape Grapple** (Gehaltener, 1 Attack Action, weiterer Contest; Gleichstand hält) und **Release Grapple** (Grappler, kostenlos) stehen situativ im Radial-Menü. Grapple endet mit dem Kampf.
...
- **Lifetime-XP-Zeile:** Der Bogen und der Ausdruck zeigen die Stein-Felder bis 660 Lifetime XP in einer Zeile. Mehr XP hängt weitere Felder an die nächste Zeile; eine volle Zeile beginnt eine weitere, die erste Zeile bleibt bis 660.
...
- **Encounter Generator:** Suggested specials stay around 3–4 (not 12), martial AoE is a 2 m melee swing with a 3 m radius (not a 16 m blast), armor is the real sheet value (party armor + rank, not MR), damage dice are capped by rank, and HP is less padded. Enemy MR is never more than one rank above the party; the preview warns when that cap is reached.
- **NPC sheet:** Attack and damage dice are typed numbers (1–80 d8), not a dropdown. Every NPC attack defaults to 1d8 Stress. Melee AoE is around self (no extra reach). Creature type **Undead** is now **Deathless**; Requiem targets Deathless (old Undead values still count). Generated NPCs can be renamed.
...
- **Public manifest:** `system.json` is now the permanent Foundry / Forge identity for Mastery System / Destroyed Faith — homepage, YouTube, Setup artwork, world background, and GitHub releases. Future releases only need a version bump.
...
- **Release tests:** Encounter start blockers now expect “Initiative” instead of the retired Shop label.
