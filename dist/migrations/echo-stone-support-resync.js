...
function catalogRowByName(def, name) {
    const rows = Array.isArray(def.levelProgression) ? def.levelProgression : [];
    return rows.find((r) => String(r?.name ?? '') === name) ?? null;
}
function syncedRow(def, row) {
    const name = String(row?.name ?? '');
    if (!name)
        return row;
    const cat = catalogRowByName(def, name);
    if (!cat)
        return row;
    const next = { ...row };
    if (typeof cat.type === 'string')
        next.type = cat.type;
    if (typeof cat.effect === 'string')
        next.effect = cat.effect;
    if (typeof cat.special === 'string')
        next.special = cat.special;
    return next;
}
function buildEchoSupportUpdates(item, def) {
    const sys = item?.system ?? {};
    const updates = {};
    const wantExtras = (def.extraStoneFunctions ?? []).map((fn) => ({ ...fn }));
    ...
    if (Array.isArray(sys.levelProgression)) {
        const next = sys.levelProgression.map((row) => syncedRow(def, row));
    ...
    if (Array.isArray(sys.progressionPicks)) {
        const nextPicks = sys.progressionPicks.map((pick) => {
            const stages = pick.authoredStages;
...
export async function resyncActorEchoStoneSupport(actor) {
    if (!actor?.items)
        return 0;
    let updated = 0;
...
export async function runEchoStoneSupportResyncMigration() {
    if (!game.user?.isGM)
        return 0;
...
//# sourceMappingURL=echo-stone-support-resync.js.map