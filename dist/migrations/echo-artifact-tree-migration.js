...
function isLegacyEchoArtifactItem(item) {
...
export async function runEchoArtifactTreeMigration() {
    if (!game.user?.isGM)
        return;
    if (hasAlreadyRun())
        return;
    ...
    for (const actor of actors) {
        const legacy = Array.from(actor?.items ?? []).filter(isLegacyEchoArtifactItem);
        for (const item of legacy) {
            const echoKey = String(item.getFlag(SETTING_NAMESPACE, 'echoArtifactKey'));
            try {
                const granted = await grantEchoArtifactTreeToActor(actor, echoKey);
                if (!granted) {
                    failed += 1;
                ...
                failed += 1;
                console.warn(`Mastery System | echo-artifact-tree-migration: failed for "${item?.name}" on "${actor?.name}"`, err);
            }
        }
    }
...
//# sourceMappingURL=echo-artifact-tree-migration.js.map