...
const LEGACY_GEAR_SLOT_TO_SLOT = {
...
const LEGACY_ARMOR_TYPE_TO_PROFILE = {
    light: 'lightArmor',
    medium: 'mediumArmor',
    heavy: 'heavyArmor',
    robe: 'robe',
};
export function registerArtifactSpecBackfillSetting() {
    try {
...
function buildBackfillUpdate(item) {
    ...
    if (sys.echoKey === undefined) {
        ...
        if (k)
            updates['system.echoKey'] = k;
        else
            updates['system.echoKey'] = null;
    }
...
async function migrateItem(item) {
    ...
    try {
        ...
        console.warn(`Mastery System | artifact-spec-backfill: failed to update item "${item?.name}"`, err);
        return false;
    }
}
/** Execute the one-shot Artifact spec backfill. Idempotent per world. */
export async function runArtifactSpecBackfill() {
    if (!game.user?.isGM)
        return;
    if (hasAlreadyRun())
        return;
...
//# sourceMappingURL=artifact-spec-backfill.js.map