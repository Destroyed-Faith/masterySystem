 ...
 */
import { describeLighting, effectiveSightRangeM, foundryEnvironmentForLighting, isLightingState, lightingStateForManualMode, normalizeLightingMode, NORMAL_VISION_RANGE_M, resolveLightingState, watchFromHour, } from './scene-lighting.js';
...
function readSceneFlag(scene, key) {
    try {
...
export function getSceneLightingMode(scene) {
...
export function getStoredSceneLightingState(scene) {
 ...
 */
export function getCurrentWatch() {
    try {
        const settings = g().game?.settings;
        if (!settings?.get)
            return null;
        if (settings.get('mastery-system', 'calendarEnabled') === false)
            return null;
...
export function actorHasDarkvision(actor) {
 ...
 */
export function renderedSightCapM(tokenDoc, state) {
    if (!state)
        return null;
...
function readDarkness(scene) {
    ...
    if (env && typeof env.darknessLevel === 'number')
        return env.darknessLevel;
    if (typeof scene?.darkness === 'number')
        return scene.darkness;
    return null;
}
function readGlobalLight(scene) {
    const env = scene?.environment;
    if (env?.globalLight && typeof env.globalLight.enabled === 'boolean')
        return env.globalLight.enabled;
    if (typeof scene?.globalLight === 'boolean')
        return scene.globalLight;
    return null;
}
/** Scene update that renders `state`, or null when the scene already matches. */
export function planSceneRenderUpdate(scene, state) {
...
export function refreshVisionRendering() {
    try {
 ...
 */
export async function applySceneLighting(scene) {
    if (!scene)
        return null;
...
export async function setSceneLightingMode(scene, mode) {
    if (!scene)
        return null;
    if (!canWriteScene(scene)) {
    ...
    const update = { [`flags.mastery-system.${LIGHTING_MODE_FLAG}`]: next };
 ...
 */
export function installVisionRangeCap() {
    if (visionPatchInstalled)
        return true;
    try {
        ...
        proto._getVisionSourceData = function masteryCappedVisionSourceData(...args) {
            ...
            try {
                ...
                if (capM === null || !data || typeof data !== 'object')
                    return data;
                const capPx = typeof this.getLightRadius === 'function' ? this.getLightRadius(capM) : null;
                if (typeof capPx !== 'number' || !Number.isFinite(capPx))
                    return data;
                if (typeof data.radius === 'number' && data.radius > capPx)
                    data.radius = capPx;
...
export function registerSceneLightingHooks() {
    if (hooksRegistered)
        return;
    ...
    if (!Hooks?.on)
        return;
    const applyToViewedScene = async () => {
    ...
    Hooks.on('updateSetting', (setting) => {
        ...
        if (key !== 'mastery-system.currentHour' && key !== 'mastery-system.currentDayIndex' && key !== 'mastery-system.calendarEnabled')
            return;
        void applyToViewedScene();
        try {
    ...
    Hooks.on('updateScene', (scene, changes) => {
        ...
        if (!flags || (flags[LIGHTING_MODE_FLAG] === undefined && flags[LIGHTING_STATE_FLAG] === undefined))
            return;
        if (String(scene?.id) !== String(g().canvas?.scene?.id))
            return;
        refreshVisionRendering();
        try {
            g().ui?.controls?.render?.();
...
//# sourceMappingURL=scene-lighting-foundry.js.map