...
export const LIGHTING_MODES = ['auto', 'day', 'dim', 'night', 'dark'];
export const LIGHTING_STATES = ['daylight', 'dim', 'night', 'darkness'];
...
export const LIGHTING_VISUAL_RANGE_M = {
    daylight: 60,
    dim: 30,
    night: 8,
    darkness: 0,
};
export const LIGHTING_STATE_LABEL = {
    daylight: 'Daylight',
    dim: 'Dim Light',
    night: 'Night',
    darkness: 'Total Darkness',
};
export const LIGHTING_MODE_LABEL = {
    auto: 'AUTO',
    day: 'DAY',
    dim: 'DIM',
    night: 'NIGHT',
...
export const LIGHTING_STATE_ICON = {
    daylight: 'fas fa-sun',
...
const STATE_RANK = { daylight: 3, dim: 2, night: 1, darkness: 0 };
export function isLightingMode(value) {
    return typeof value === 'string' && LIGHTING_MODES.includes(value);
}
export function isLightingState(value) {
    return typeof value === 'string' && LIGHTING_STATES.includes(value);
}
export function normalizeLightingMode(raw) {
    return isLightingMode(raw) ? raw : 'auto';
}
/** Manual modes map 1:1 to a state; `auto` has none of its own. */
export function lightingStateForManualMode(mode) {
    switch (mode) {
        case 'day':
            return 'daylight';
        case 'dim':
...
export function watchFromHour(hour, hoursPerDay = 40) {
...
export function lightingStateForWatch(watch) {
    if (watch === 0 || watch === 1)
        return 'daylight';
    if (watch === 2)
        return 'dim';
 ...
 */
export function resolveLightingState(mode, watch, fallback = null) {
...
export function normalVisualRangeM(state) {
 ...
 */
export function effectiveSightRangeM(state, opts = {}) {
    const normal = normalVisualRangeM(state);
    if (!opts.darkvision)
        return normal;
    if (state === 'night' || state === 'darkness')
        return Math.max(normal, DARKVISION_RANGE_M);
    return normal;
}
/** Only the ordinary Sight channel is capped by scene lighting. */
export function isLightingAffectedSense(senseId) {
 ...
 */
export function senseRangeUnderLighting(senseId, state, opts = {}) {
    const def = COMBAT_SENSES[senseId];
    if (!isLightingAffectedSense(senseId))
        return def.rangeM;
    if (senseId === 'darkvision')
        return effectiveSightRangeM(state, { darkvision: true });
    return effectiveSightRangeM(state, opts);
}
/** The brighter of two states (light sources only ever improve lighting). */
export function brighterLighting(a, b) {
 ...
 */
export function foundryEnvironmentForLighting(state) {
    switch (state) {
        case 'daylight':
...
export function describeLighting(mode, watch, state) {
...
//# sourceMappingURL=scene-lighting.js.map