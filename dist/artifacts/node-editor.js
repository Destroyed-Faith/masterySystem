...
import { ACTIVE_TEMPLATES, ACTIVE_BUFF_TEMPLATES, REACTION_TEMPLATES, MOVEMENT_TEMPLATES, PASSIVE_TEMPLATES, getTemplate as getPowerTemplate, } from '../utils/powers/index.js';
...
function tplOpts(list) {
    return list
        .map((t) => ({ id: t.templateId, name: t.templateName }))
        .sort((a, b) => a.name.localeCompare(b.name));
}
function buildPowerCatalogOptions() {
    const actives = [
        ...martialDeliveryCatalogOptions(),
        ...tplOpts(ACTIVE_TEMPLATES.filter((t) => !isMartialDamageTemplateId(t.templateId))),
    ].sort((a, b) => a.name.localeCompare(b.name));
    return {
        active: actives,
        activeBuff: tplOpts(ACTIVE_BUFF_TEMPLATES),
        reaction: tplOpts(REACTION_TEMPLATES),
        movement: tplOpts(MOVEMENT_TEMPLATES),
        passive: tplOpts(PASSIVE_TEMPLATES),
    };
}
/** Power-mode + selected template/name for an existing pick (render helper). */
function resolvePowerPickMode(pick) {
    const tid = String(pick.powerTemplateId || '').trim();
    if (pick.delivery) {
        return {
            powerMode: 'active',
            templateId: martialDeliveryPickId(pick.delivery),
    ...
    if (tid) {
        ...
        const cat = String(tpl?.category || 'active');
...
function readSpellFieldsFromRow($row, pick) {
    ...
    return {
        isSpell: true,
        castingAttribute: String($row.find('.node-pick-casting-attr').val() || 'intellect') ||
            'intellect',
        spellResolution: String($row.find('.node-pick-spell-resolution').val() || 'spellAttack') ||
            'spellAttack',
    };
}
function syncSpellPanel($row) {
...
function deriveGearSlotFromSlot(slot) {
    switch (slot) {
        case 'head':
            return 'head';
        case 'feet':
...
export class NodeEditor extends BaseDialog {
    ...
    getData(options) {
        ...
        this._stonePowerOptionsByAttr = stonePowerOptionsByAttr;
        this._powerCatalogOptions = data.powerCatalogOptions;
        const storedPicks = Array.isArray(system.progressionPicks)
            ? system.progressionPicks
            : [];
        const pickByLevel = new Map();
        for (const p of storedPicks) {
            const lvl = Number(p?.level);
            if (lvl >= 1 && lvl <= 3)
                pickByLevel.set(lvl, p);
        }
        ...
        data.progressionPickRows = [1, 2, 3].map((lvl) => {
            ...
            const catalogSpecialKey = p?.kind === 'power' && p.chosenSpecial?.key ? p.chosenSpecial.key : '';
            const needsCatalogSpecial = p?.kind === 'power' &&
                mode.powerMode === 'active' &&
                !!mode.templateId &&
                catalogTemplateRequiresSpecial(mode.templateId);
            const showSpellConfig = p?.kind === 'power' &&
                mode.powerMode === 'active' &&
    ...
    activateListeners(html) {
        ...
        const rebuildRowsFromTemplate = ($container, rowSelector, applyRow, values) => {
        ...
        const applyBaseTypePrefill = (id) => {
            if (!isLineageRoot)
                return;
            const res = resolveArtifactBaseType(id);
            if (!res)
                return; // Custom / unknown → keep whatever is already configured.
            ...
            if (res.kind === 'weapon') {
                ...
                syncWeaponRangeLabel(html);
                rebuildRowsFromTemplate(html.find('#node-weapon-innates'), '.node-select-row', ($row, val) => $row.find('.node-weapon-innate').val(String(val || '')), res.innateAbilities);
                rebuildRowsFromTemplate(html.find('#node-weapon-specials'), '.node-special-row', ($row, ref) => {
        ...
        const powerCatalogOptions = (this._powerCatalogOptions || {});
        const populatePowerTemplateSelect = ($row, mode, desired) => {
            ...
            html.find('.node-progression-pick').each((_i, el) => {
                ...
                if (kind === 'authored') {
                    ...
                    if (sfKind && sfAttr) {
                        ...
                        if (sfKind === 'stonePowerSupport' && sfPower)
                            sf.stonePowerId = sfPower;
        ...
        const previewNodeLevel = Math.max(1, Math.min(10, Number(this.item.system?.level) || 1));
        ...
        html.on('change', '.node-pick-kind', (e) => {
            syncProgressionRow($(e.currentTarget).closest('.node-progression-pick'));
            refreshStoneFnWarning();
            rebuildLevelProgressionPreview();
        });
        html.on('change', '.node-pick-power-mode', (e) => {
        ...
        html.on('change', '.node-pick-is-spell', (e) => {
        ...
        html.on('change', '.node-pick-stone-attr', (e) => {
            const $row = $(e.currentTarget).closest('.node-progression-pick');
    ...
    async saveNode(html) {
        ...
        for (const row of baseValueRows) {
            ...
            if (!isSpecial && usedTypes.has(typeRaw))
                continue; // no duplicate non-special types
            ...
            if (isSpecial) {
                ...
                if (!label)
                    continue; // a Special row with no Special chosen is dropped
                if (usedSpecialIds.has(label))
                    continue; // no duplicate Specials
                usedSpecialIds.add(label);
            }
            else {
                label = BASE_VALUE_TYPE_LABELS[typeRaw] || '';
        ...
        const stoneFunction = stoneFnPicks[0]?.stoneFunction || null;
