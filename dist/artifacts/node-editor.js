...
export class NodeEditor extends BaseDialog {
    ...
    async saveNode(html) {
        const bonuses = {
            attack: parseInt(html.find('#node-attack').val(), 10) || 0,
            damage: html.find('#node-damage').val() || '',
            defense: parseInt(html.find('#node-defense').val(), 10) || 0,
            specials: (html.find('#node-specials').val() || '').split('\n').filter((s) => s.trim() !== '')
        };
        const requirements = {
