...
function readSpecials($b) {
    ...
    $b.find('.ep-sp-row').each((_i, row) => {
        ...
        if (raiseCost !== undefined)
            entry.raiseCost = raiseCost;
...
function parseImportedPayload(parsed) {
    ...
    else if (parsed && typeof parsed === 'object' && Array.isArray(parsed.powers)) {
        arr = parsed.powers;
    }
    ...
    return arr.map((raw) => {
        const migrated = isOldPowerStructure(raw) ? migrateArtifactPower(raw) : JSON.parse(JSON.stringify(raw));
...
export class EmbeddedPowerDialog extends BaseDialog {
    ...
    getData(options) {
        ...
        data.powerList = powers.map((p, i) => {
            ...
            const inherited = !!pid && !this._lineage.isLineageRoot && this._lineage.lockedPowerIds.has(pid);
        ...
        data.detailReadOnly = Boolean(sel &&
            !this._lineage.isLineageRoot &&
            sel.id &&
            this._lineage.lockedPowerIds.has(String(sel.id)));
        return data;
    ...
    finalizePowersForSave() {
        ...
        if (!this._lineage.isLineageRoot) {
            ...
            for (let i = 0; i < next.length; i++) {
                const p = next[i];
                const pid = p.id ? String(p.id) : '';
                if (!pid || !this._lineage.lockedPowerIds.has(pid))
                    continue;
                const baseline = this._baselinePowers.find((b) => b.id && String(b.id) === pid);
                if (baseline)
                    next[i] = foundry.utils.deepClone(baseline);
            }
    ...
    readCurrentPowerFromForm(html) {
        ...
        if (prev.cost.stones === undefined)
            delete prev.cost.stones;
        if (prev.cost.charges === undefined)
            delete prev.cost.charges;
        ...
        html.find('.ep-level-block').each((_i, el) => {
            const key = String($(el).data('level') || '');
    ...
    activateListeners(html) {
        ...
        html.find('[data-action="ep-select-power"]').on('click', (e) => {
        ...
        html.on('click', '.ep-add-special', (e) => {
    ...
    runImport(html, mode) {
        ...
        try {
            ...
            if (this._workingPowers.length && mode === 'replace')
                this._selectedIndex = 0;
...
//# sourceMappingURL=embedded-power-dialog.js.map