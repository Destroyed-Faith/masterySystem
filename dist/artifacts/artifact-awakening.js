...
export function initializeArtifactAwakening() {
    ...
    try {
        ...
        const hookId = Hooks.on('renderItemDirectory', (app, html, _data) => {
            ...
            const htmlLength = isJQuery ? html.length : (isHTMLElement ? 1 : 0);
            const htmlString = isJQuery ? html[0]?.outerHTML?.substring(0, 500) : (isHTMLElement ? html.outerHTML?.substring(0, 500) : 'NOT HTML');
            const itemsTab = $('.sidebar-tab[data-tab="items"]');
            const itemsTabHTML = itemsTab.length > 0 ? itemsTab[0].outerHTML.substring(0, 300) : 'NOT FOUND';
            ...
            const passedHtmlLength = isJQueryHtml ? html.length : (html instanceof HTMLElement ? 1 : 0);
