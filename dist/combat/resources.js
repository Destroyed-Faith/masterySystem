 ...
 */
export async function spendStones(actor, amount, reason = 'power activation') {
    if (!actor || !actor.system.resources?.stones)
        return false;
 ...
 */
export async function regenerateStones(actor) {
    if (!actor || !actor.system.resources?.stones)
        return 0;
    const stones = actor.system.resources.stones;
 ...
 * @param actor - The actor
 */
export async function restoreAllStones(actor) {
    if (!actor || !actor.system.resources?.stones)
        return;
    const stones = actor.system.resources.stones;
    await actor.update({
 ...
 * @returns Success boolean
 */
export async function applyVitalityDamage(actor, amount) {
    if (!actor || amount <= 0)
        return false;
    ...
    await ChatMessage.create({
      ...
      <div class="mastery-resource-use vitality-damage">
        ...
        <div class="resource-details">
          ...
          <p><strong>Current HP:</strong> ${actor.totalHP || 0}</p>
        </div>
      </div>
    `,
        type: CONST.CHAT_MESSAGE_TYPES.OTHER,
 ...
 */
export async function addStress(actor, amount, reason = 'stressful event') {
    if (!actor || !actor.system.resources?.stress)
        return 0;
 ...
 * @returns New Stress value
 */
export async function reduceStress(actor, amount, reason = 'stress relief') {
    if (!actor || !actor.system.resources?.stress)
        return 0;
    const stress = actor.system.resources.stress;
 ...
 * @returns Stone cost
 */
export function calculateStoneCost(usageCount) {
 ...
 */
export function getResourceStatus(actor) {
