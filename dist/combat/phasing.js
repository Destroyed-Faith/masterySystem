...
export function getPhasingCharges(actor) {
    const raw = actor?.flags?.['mastery-system']?.phasingCharges;
...
async function safeUpdate(actor, patch) {
...
function sourceKey(ownerKind, ownerId) {
 ...
 * the same combat.
 */
export async function grantPhasingCharges(actor, combat, amount, source) {
    if (!actor || !(amount > 0))
        return;
 ...
 */
export async function augmentPhasingCharges(actor, combat, addAmount, source) {
    if (!actor || !(addAmount > 0))
        return;
    const combatId = String(combat?.id ?? '');
    const state = getPhasingCharges(actor);
    ...
    if (source.ownerKind === 'buff') {
        ...
        if (!hasPassive) {
            console.warn(`Mastery System | [phasing] augment "${source.name}" ignored — no Passive base`);
            return;
        }
    }
 ...
 */
export async function removeAugmentCharges(actor, effectId) {
    if (!actor || !effectId)
        return;
    const state = getPhasingCharges(actor);
 ...
 */
export async function consumePhasingCharge(actor) {
    if (!actor)
        return false;
    const state = getPhasingCharges(actor);
    if (state.current <= 0)
...
export async function clearPhasingOnCombatEnd(actor, combat) {
    if (!actor)
        return;
    const state = getPhasingCharges(actor);
    if (state.max === 0 && state.current === 0 && Object.keys(state.sources).length === 0)
        return;
    const combatId = combat?.id ? String(combat.id) : null;
    if (combatId && state.combatId && state.combatId !== combatId)
        return;
    await safeUpdate(actor, writeState(emptyState()));
}
/** Convenience: clear phasing state on every combatant. */
export async function clearPhasingForCombat(combat) {
 ...
 */
export async function promptPhasingConsume(target, context = {}) {
    if (!target)
        return false;
 ...
 * string if no combat).
 */
export async function triggerGhostSlipReaction(actor, combat, effectId = 'ghost-slip') {
    if (!actor)
        return false;
 ...
 * registration.
 */
export function registerPhasingSettings() {
    try {
        const g = game?.settings;
        if (!g?.register)
            return;
        g.register('mastery-system', 'phasingAutoConsume', {
            ...
            hint: 'Controls whether the system prompts when a phasing charge would be consumed. ' +
...
//# sourceMappingURL=phasing.js.map