...
export function readGrappleState(actor) {
    let raw = null;
    try {
...
export function isInGrapple(actor) {
    return readGrappleState(actor) !== null;
}
export function grappleRoleOf(actor) {
 ...
 */
export function planGrapplePair(grappler, held, combat) {
    const shared = {
...
export function grappleDamage() {
    return 0;
}
/** The only status a Grapple applies. Root is a different Special. */
export function grappleStatusIds() {
    return [GRAPPLED_STATUS_ID];
}
function isUnarmedBasicAttackOption(actor, option, unarmedNow) {
    if (!option)
        return false;
    const id = str(option.id || option.maneuver?.id);
    if (id !== 'weapon-attack')
        return false;
    if (option.forcedWeaponItemId)
        return false;
    return unarmedNow;
}
/**
 ...
 */
export function attackRequiresGrappleRelease(actor, option, deps) {
    if (!option || option.slot !== 'attack')
        return false;
    if (grappleRoleOf(actor) !== 'grappler')
        return false;
    if (isUnarmedBasicAttackOption(actor, option, deps.unarmedNow))
        return false;
...
export async function resolveGrapplePartner(state) {
    if (!state)
        return null;
...
async function writeGrappleFlag(actor, state) {
    if (!actor)
        return;
...
async function setGrappledStatus(actor, active) {
    if (!actor)
        return;
...
export function grappleParticipantOf(actor, token) {
...
export async function applyGrapple(grappler, held, opts = {}) {
    const plan = planGrapplePair(grappleParticipantOf(grappler, opts.grapplerToken), grappleParticipantOf(held, opts.heldToken), opts.combat ? { id: opts.combat.id, round: opts.combat.round } : null);
...
export async function clearGrappleAfterCombat(actors) {
    for (const actor of actors) {
        if (!readGrappleState(actor))
            continue;
        try {
            await writeGrappleFlag(actor, null);
        }
...
//# sourceMappingURL=grapple-state.js.map