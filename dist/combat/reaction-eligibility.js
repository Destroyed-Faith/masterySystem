...
export function isArmorAxisReaction(item) {
    if (item?.basicReaction === 'guard')
        return true;
    const tid = templateIdOf(item);
    if (tid === 'reaction-armor' || tid === 'reaction-armor-temp-hp')
        return true;
    if (tid === 'reaction-ally-armor')
        return true;
    ...
    if (armor > 0 && evade <= 0 && dr <= 0 && !temp)
        return true;
    return false;
}
/** Damage-buffer reactions that need a hit / incoming damage (Temp HP, Armor+Temp, DR). */
export function isDamageTriggerReaction(item) {
    const tid = templateIdOf(item);
    if (tid === 'reaction-temp-hp' ||
    ...
    const armor = Math.max(0, Math.floor(Number(mech?.armor) || 0));
    const evade = Math.max(0, Math.floor(Number(mech?.evade) || 0));
    if ((temp || dr > 0) && evade <= 0)
        return true;
    if (armor > 0 && temp)
        return true;
    return false;
}
export function isCounterDamageReaction(item) {
    const tid = templateIdOf(item);
    ...
    if (tid === 'reaction-counter-damage' || tid === 'reaction-counter-damage-push')
        return true;
    if (subfamilyOf(item) === 'counter')
        return true;
    const flat = String(mechanicsOf(item)?.damageRider?.flat ?? '');
    return /d8/i.test(flat);
}
export function isSpecialIncreaseReaction(item) {
    const tid = templateIdOf(item);
    if (tid === 'reaction-special-increase')
        return true;
    const mod = mechanicsOf(item)?.modifySpecial;
    return mod?.mode === 'increaseExisting' && mod?.type === 'chosen';
}
export function isRepositionReaction(item) {
    const tid = templateIdOf(item);
    if (tid === 'reaction-reposition')
        return true;
...
export function isGhostSlipReaction(item) {
    const tid = templateIdOf(item);
    if (tid === 'reaction-phasing')
        return true;
    return !!mechanicsOf(item)?.phasing?.reactionSingleHit;
}
export function isCleanseReaction(item) {
    const tid = templateIdOf(item);
    if (tid === 'reaction-reactive-cleanse')
        return true;
    return subfamilyOf(item) === 'cleanse';
}
export function isOverloadReaction(item) {
    const tid = templateIdOf(item);
    if (tid === 'reaction-reactive-overload')
        return true;
    return subfamilyOf(item) === 'absorption';
}
export function isParryFollowUpReaction(item) {
    const tid = templateIdOf(item);
    if (tid === 'reaction-riposte' || tid === 'reaction-parry-reflection')
        return true;
...
export function actorHasPassiveDR(actor) {
    if (!actor)
        return false;
    try {
...
export function actorHasPassivePhasing(actor) {
    if (!actor)
        return false;
    try {
        ...
        for (const item of items) {
            ...
            if (sys?.powerType !== 'passive' && sys?.equipped === false)
                continue;
            const mech = resolvePowerMechanics(item);
            if (mech?.phasing)
                return true;
            const tid = String(sys?.templateId ?? '').toLowerCase();
            if (tid.includes('phasing') || tid.includes('ghost-slip'))
                return true;
        }
    }
    catch {
...
export function distanceBetweenActorsMeters(a, b) {
    if (!a || !b)
        return null;
    try {
 ...
 */
export function evaluateReactionEligibility(power, ctx) {
    ...
    const tidEmpty = (tid === 'reaction-damage-reduction' || tid === 'reaction-phasing' || tid === 'reaction-special-increase') &&
        !mech?.armor &&
        !mech?.evade &&
        !mech?.tempHP &&
...
export function buildReactionTriggerContext(params) {
    ...
    return {
        ...
        rangeToAttackerM: distanceBetweenActorsMeters(defender, attacker),
...
//# sourceMappingURL=reaction-eligibility.js.map