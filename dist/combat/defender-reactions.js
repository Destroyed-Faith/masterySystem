...
export function getEligibleReactionPowers(defender, combat) {
    ...
    for (const item of items) {
        ...
        if (sys?.showInRadialMenu === false)
            continue;
        const repeatable = String(sys?.templateId ?? '') === 'reaction-riposte' ||
            resolvePowerMechanics(item)?.repeatable === true;
        if (!repeatable && hasPowerBeenUsedThisRound(owner, combat, item.id))
            continue;
 ...
 */
export function isThreatenedDeclarationLegalReaction(item) {
    ...
    if (String(item?.basicReaction || ''))
        return false;
    if (isThreatenedRangedOffensiveReaction(item))
        return false;
    const trig = String(item?.system?.trigger ?? '').toLowerCase();
    if (/\bhit\b|\btargeted\b|take damage|would take|damage instance|fully parry|lose actual hp|ongoing effect/.test(trig)) {
        return false;
    }
    return true;
}
