 ...
 */
export function listSaveEndsCandidates(actor) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        if (!def || def.category !== 'diminishing')
            continue;
        const saveRaw = (def.save || '').toLowerCase();
        let save = null;
        if (saveRaw.startsWith('body'))
            save = 'body';
        else if (saveRaw.startsWith('mind'))
            save = 'mind';
        else if (saveRaw.startsWith('spirit'))
            save = 'spirit';
        if (!save)
            continue;
 ...
 */
export async function rollSaveEnd(actor, effect, options = {}) {
 ...
 */
export async function postSaveEndsPromptForActor(actor, combat) {
    if (!actor)
        return;
 ...
 * `init` / `ready` setup.
 */
export function registerSaveEndsChatHandlers() {
...
//# sourceMappingURL=save-ends.js.map