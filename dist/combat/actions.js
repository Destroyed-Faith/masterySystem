 ...
 */
export async function resetActionsForRound(actor) {
    if (!actor)
        return;
 ...
 */
export async function resetActionsForTurn(actor) {
    if (!actor)
        return;
    const system = actor.system;
    if (!system.actions)
    ...
    if (convertedReactions > 0) {
        const newMax = Math.max(system.actions.reaction.base, system.actions.reaction.max - convertedReactions);
        await actor.update({
 ...
 */
export async function useAction(actor, actionType, amount = 1) {
    if (!actor || !actor.system.actions)
        return false;
 ...
 */
export async function unuseAction(actor, actionType, amount = 1) {
    if (!actor || !actor.system.actions)
        return false;
    const action = actor.system.actions[actionType];
 ...
 */
export async function convertAttackAction(actor, targetType) {
    if (!actor || !actor.system.actions)
        return false;
    const system = actor.system;
    const masteryRank = system.mastery?.rank || 2;
    // Check conversion rules
    const totalConversions = system.actions.conversions.attackToMovement +
 ...
 */
export async function undoConversion(actor, conversionType) {
    if (!actor || !actor.system.actions)
        return false;
    const system = actor.system;
 ...
 */
export function getActionStatus(actor) {
    const system = actor.system;
    if (!system.actions) {
        return {
    ...
    const masteryRank = system.mastery?.rank || 2;
    const totalConversions = (system.actions.conversions?.attackToMovement || 0) +
        (system.actions.conversions?.attackToReaction || 0);
    return {
        attack: {
