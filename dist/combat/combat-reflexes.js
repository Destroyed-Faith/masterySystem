 ...
 */
export async function stepCombatReflexesInitiative(actor, combatant, delta, masteryRank) {
    if (!actor || !combatant)
        return null;
    const step = delta > 0 ? 1 : -1;
    const state = combatReflexesInitiativeState(actor, combatant, masteryRank);
    if (step > 0 && !state.canAdd)
        return null;
    if (step < 0 && !state.canRemove)
        return null;
    const initiative = Math.max(0, Math.floor(Number(combatant.initiative) || 0));
...
export async function resetCombatReflexesRoundUsage(combatant) {
    if (combatReflexesUsedThisRound(combatant) <= 0)
        return;
