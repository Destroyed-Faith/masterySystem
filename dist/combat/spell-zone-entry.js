...
import { markZoneApplied, readSpellZones, retainActiveSpellZones, spellZonesEntered, writeSpellZones, } from './spell-zones.js';
function actorList() {
...
function metersBetween(scene, x1, y1, x2, y2) {
...
export async function applyStoredSpellZonesOnEnter(creature, tokenDoc, combat) {
    if (!creature || !combat)
        return;
    ...
    for (const caster of actorList()) {
        ...
        const next = active.map((zone) => {
            if (!hitById.has(zone.id))
                return zone;
        ...
        for (const hit of hits) {
            ...
            try {
                await globalThis.ChatMessage?.create?.({
                    speaker: globalThis.ChatMessage?.getSpeaker?.({ actor: creature }),
        ...
        if (changed)
            await writeSpellZones(caster, next);
    }
}
//# sourceMappingURL=spell-zone-entry.js.map