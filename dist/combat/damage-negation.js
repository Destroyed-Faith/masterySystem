...
export function findDamageNegationItem(actor) {
    if (!actor)
        return null;
...
export function damageNegationReserveForLevel(level) {
...
export function damageNegationHalfPoolCap(originalDice) {
 ...
 */
export function getDamageNegationRemaining(actor, combat) {
    ...
    const flag = owner?.getFlag?.(FLAG_SCOPE, FLAG_RESERVE);
    if (!flag || flag.combatId !== String(combat.id))
        return max;
    return Math.max(0, Math.min(max, Math.floor(Number(flag.remaining) || 0)));
}
export async function spendDamageNegation(actor, combat, amount) {
    const n = Math.max(0, Math.floor(Number(amount) || 0));
    if (n <= 0)
        return true;
    const owner = getActionEconomyActor(actor) ?? actor;
    const remaining = getDamageNegationRemaining(owner, combat);
    if (remaining < n || !combat?.id)
        return false;
...
export function getTempDamageNegation(actor, combat) {
    if (!combat)
        return 0;
    try {
...
export function resolveNpcActiveCombatBlock(actor) {
    ...
    if (phases.length > 0) {
        ...
        if (phaseCombat && typeof phaseCombat === 'object')
            return phaseCombat;
    }
 ...
 */
export function getNpcDamageNegationDice(actor) {
 ...
 */
export async function promptDamageNegationSpend(target, opts) {
    const none = { diceRemoved: 0, note: '' };
    try {
        ...
        const picked = await new Promise((resolve) => {
            new Dialog({
                ...
                content: `<p><strong>${name}</strong> is assigned a Damage Pool of <strong>${Math.floor(opts.totalDice)}d8</strong> from ${attackerName}.</p>` +
        ...
        if (tempDn > 0 && combat) {
            ...
            try {
                ...
                if (rs.stoneBonuses) {
                    rs.stoneBonuses.tempDamageNegation = Math.max(0, (rs.stoneBonuses.tempDamageNegation ?? 0) - useTemp);
                    await srs(owner, rs);
                }
            }
            catch (e) {
...
export async function clearDamageNegationForCombat(combat) {
...
//# sourceMappingURL=damage-negation.js.map