...
export const FAMILIAR_UPGRADE_CATEGORY_OPTIONS = [
...
export const FAMILIAR_ARMOR_BY_TIER = [0, 3, 6, 9, 12, 15, 18, 21, 24];
export const FAMILIAR_EVADE_BY_TIER = [4, 8, 12, 16, 20, 24, 28, 32, 36];
...
function tierLookup(table, index, label, warnings) {
    if (index < 0) {
...
function attackDamageString(dice) {
    return `${dice}d8`;
}
/**
 ...
 */
function aggregateUpgradePicks(upgradeStones, warnings) {
    const counts = {
        hp: 0,
        armor: 0,
        evade: 0,
    ...
    for (const stone of upgradeStones) {
        ...
        if (a === b) {
            warnings.push(`Upgrade stone "${stone.id}": duplicate category (${a}) on the same stone — only one pick counts.`);
            bump(a);
            continue;
        }
        bump(a);
...
function dedupeSharedSenses(groups, warnings) {
    const seen = new Set();
    const out = [];
...
export function buildFamiliarResult(input) {
    const warnings = [];
    ...
    if (rawPickSum !== expectedPicks && upgradeStones > 0) {
        warnings.push(`Expected ${expectedPicks} effective upgrade picks for ${upgradeStones} upgrade stone(s); got ${rawPickSum} (duplicate categories on a stone reduce effective picks).`);
    }
    ...
    if (totalBoundStones > cap) {
        warnings.push(`Bound stones (${totalBoundStones}) exceed per-Familiar limit (Mastery Rank × 4 = ${cap}).`);
    }
...
//# sourceMappingURL=familiar-rules.js.map