 ...
 */
import { getRoundState, refundMovementAction, setRoundState, spendMovementAction, } from '../combat/action-economy.js';
import { PENDING_SAFE_MOVEMENT_FLAG, PENDING_SLIP_FLAG, SLIP_MOVEMENT_ACTIVE_FLAG, canCommitSafeMovement, markSlipUsed, readSlipArm, safeMovementMeters, slipCanTrigger, } from './agility-movement.js';
function actorToken(actor) {
    const id = String(actor?.id || '');
    if (!id)
        return null;
    const placeables = globalThis.canvas?.tokens?.placeables ?? [];
    return placeables.find((token) => String(token?.actor?.id || '') === id) ?? null;
}
async function confirmChoice(title, content, yes, no) {
 ...
 */
export async function presentSafeMovement(actor, tier) {
    ...
    if (!canCommitSafeMovement(budget)) {
        await postChat(`<div class="mastery-stone-resolution"><p><strong>${name}</strong> — Safe Movement</p><p>Normal Movement is already spent, so Safe Movement does not grant another move.</p></div>`);
        await actor?.unsetFlag?.('mastery-system', PENDING_SAFE_MOVEMENT_FLAG);
        return true;
    }
    const choice = await confirmChoice('Safe Movement', `${name}: move up to <strong>${meters} m</strong> as your Movement. This replaces normal Movement, does not cost an Attack Action or a Reaction, and does not provoke movement-triggered Reactions.`, 'Move', 'Later');
    if (choice !== true)
        return false;
    const token = actorToken(actor);
    if (!token) {
        globalThis.ui?.notifications?.warn('Safe Movement needs a token on the canvas.');
        return false;
    }
    const previous = {
        movementPowerUsed: !!roundState.movementPowerUsedThisRound,
    ...
    if (!spent) {
        await postChat(`<div class="mastery-stone-resolution"><p><strong>${name}</strong> — Safe Movement</p><p>Normal Movement is already spent.</p></div>`);
        return true;
    }
    ...
    await actor?.unsetFlag?.('mastery-system', PENDING_SAFE_MOVEMENT_FLAG);
    await postChat(`<div class="mastery-stone-resolution"><p><strong>${name}</strong> — Safe Movement</p><p>Moved up to <strong>${meters} m</strong>. This was your Movement for the Turn. Normal Movement is no longer available. Movement-triggered Reactions do not apply.</p></div>`);
    return true;
}
...
export async function maybeOfferSlip(args) {
    ...
    if (!slip || !slipCanTrigger(slip, event))
        return;
    const name = String(defender.name || 'Someone');
    const choice = await confirmChoice('Slip', `${name}: an Attack missed you. Slip up to <strong>${slip.meters} m</strong>? This does not use your normal Movement and does not provoke movement-triggered Reactions.`, 'Slip', 'Not now');
    if (choice !== true)
        return;
    const token = actorToken(defender);
    if (!token) {
        globalThis.ui?.notifications?.warn('Slip needs a token on the canvas.');
        return;
    }
    ...
    await postChat(`<div class="mastery-stone-resolution"><p><strong>${name}</strong> — Slip</p><p>Moved up to <strong>${slip.meters} m</strong> after an Attack missed. Normal Movement was not spent. Movement-triggered Reactions do not apply. Slip is used until your next Turn.</p></div>`);
}
//# sourceMappingURL=agility-movement-ui.js.map