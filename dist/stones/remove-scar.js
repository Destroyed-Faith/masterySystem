...
export async function clearRemoveScarResolvedTiers(actor) {
    const owner = getActionEconomyActor(actor) ?? actor;
    if (owner?.getFlag?.('mastery-system', REMOVE_SCAR_RESOLVED_FLAG) == null)
        return;
    await owner?.unsetFlag?.('mastery-system', REMOVE_SCAR_RESOLVED_FLAG);
}
/**
 ...
 */
export function computeRemoveScarPayment(resolvedMax, targetTier, supportPrefillTier = 0) {
...
export function inferRemoveScarTargetTier(resolvedMax, supportPrefillTier = 0) {
    const resolved = clampTier(resolvedMax);
    const support = clampTier(supportPrefillTier);
    if (resolved >= 4)
        return 4;
...
export function countScarredHealthBars(actor) {
    const bars = Array.isArray(actor?.system?.health?.bars) ? actor.system.health.bars : [];
...
export function buildRecoverScarredBarUpdates(actor, count) {
...
export async function recoverScarredBars(actor, count) {
...
export async function sealVitalityStones(actor, amount) {
...
export function titanScarsAllowsTouchRemoveScar(actor) {
    return titanScarsArtifactLevel(actor) >= 9;
}
function listTouchedWillingActors(caster) {
    const out = [caster];
    ...
    try {
        ...
        if (!selfToken)
            return out;
        const seen = new Set([String(caster?.id ?? '')]);
        const touchMeters = Math.max(2, Number(canvas?.grid?.distance ?? 2) * 1.08);
        for (const token of (canvas.tokens?.placeables ?? [])) {
            const other = token?.actor;
            if (!other || token.id === selfToken.id)
                continue;
            const id = String(other.id ?? '');
            if (!id || seen.has(id))
                continue;
            if (tokenIsHostileTo(selfToken, token) || tokenIsHostileTo(token, selfToken))
                continue;
            const dist = distanceBetweenTokensMeters(selfToken, token);
            if (!Number.isFinite(dist) || dist > touchMeters)
                continue;
            seen.add(id);
            out.push(other);
        }
...
export async function resolveRemoveScarHealTarget(caster) {
    if (!titanScarsAllowsTouchRemoveScar(caster))
        return caster;
    ...
    try {
        ...
        return candidates.find((a) => String(a.id) === String(id)) || caster;
    }
    catch {
        return caster;
    }
}
export async function applyRemoveScarEffect(actor, targetTier, healTarget) {
...
export async function payAndApplyRemoveScar(actor, opts = {}) {
    ...
    const targetTier = opts.targetTier != null ? clampTier(opts.targetTier) : inferRemoveScarTargetTier(resolved, support);
    ...
    if (current < payment.sealCost) {
        ui.notifications?.warn(`Not enough Vitality stones to Seal! Need ${payment.sealCost}, have ${current}`);
        return false;
    }
    const healTarget = opts.healTarget ??
    ...
    ui.notifications?.info(`${owner?.name}: Remove Scar T${targetTier} — recovered ${restored} Scarred Health Bar(s); ${sealedNote} until Safe Haven Rest.`);
    return true;
}
/** Attribute this power may spend. Colorless is never legal. */
export function removeScarSpendableAttributes() {
    return ['vitality'];
}
//# sourceMappingURL=remove-scar.js.map