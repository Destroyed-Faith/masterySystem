 ...
 */
import { MINOR_MAGIC_FORM_LABELS, MINOR_MAGIC_FORMS, MINOR_MAGIC_REST_REQUIRED, canManageMinorMagic, createMinorMagicItem, defaultMinorMagicName, listEligibleMinorMagicPowers, minorMagicSheetView, resolveMinorMagicPower, snapshotPowerForMinorMagic, snapshotSummaryLines, } from '../utils/minor-magic-items.js';
export class MinorMagicPanel {
    ...
    bind(root) {
        ...
        root.querySelector('.js-mm-create')?.addEventListener('click', (ev) => {
            ev.preventDefault();
            ev.stopPropagation();
            const name = root.querySelector('.js-mm-name')?.value ?? this.itemName;
    ...
    async #create(name) {
        ...
        ui.notifications?.info(`${result.item.name} added to inventory.`);
        this.#syncDefaultName();
...
export const MinorMagicDialog = {
    async show(actor) {
        ...
        ui.notifications?.warn('Open the character sheet to create a Minor Magic Item.');
    },
};
//# sourceMappingURL=minor-magic-dialog.js.map