...
export function resolveBloodIntensity(opts) {
    if (opts.intensity === 'splatter' || opts.intensity === 'puddle')
...
async function createPersistentBloodTile(center, intensity, damage, gridSize, r, g, b, darkerColor, pixiColor) {
    try {
        ...
        const multiplier = intensity === 'puddle'
...
function animateScaleIn(graphic, durationMs, from = 0.15, to = 1) {
...
function createAnimatedSplatters(token, center, damage, gridSize, pixiColor, darkerColor) {
    ...
    group.alpha = 0.8;
    group.msBloodPool = true;
    group.msBloodKind = 'splatter';
    group.msTokenId = token?.id ?? token?.document?.id;
...
function createAnimatedPuddle(token, center, damage, gridSize, pixiColor, darkerColor) {
    const PIXI = globalThis.PIXI;
    const container = getBackgroundContainer();
    if (!PIXI?.Graphics || !container) {
    ...
    bloodPool.msBloodKind = 'puddle';
    bloodPool.msTokenId = token?.id ?? token?.document?.id;
