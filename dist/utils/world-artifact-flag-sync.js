...
export function canUpdateWorldItem(item) {
    if (typeof game === 'undefined' || !item)
        return false;
    if (game.user?.isGM)
        return true;
    try {
 ...
 */
export async function setRootActorLevels(rootItem, levels) {
    if (!rootItem?.id)
        return false;
...
export async function upsertRootActorProgress(rootItem, actorId, progress) {
    if (!rootItem?.id || !actorId)
        return false;
    const levels = {
...
export async function upsertRootActorProgressForActor(rootItem, actorId, patch) {
...
function requestGmSetRootActorLevels(rootItemId, levels) {
...
export function registerWorldArtifactFlagSyncSocket() {
    if (typeof game === 'undefined' || socketRegistered)
        return;
    socketRegistered = true;
    game.socket?.on(SOCKET_NAME, async (payload) => {
        if (payload?.action !== 'setRootActorLevels')
            return;
        if (!game.user?.isGM)
            return;
...
//# sourceMappingURL=world-artifact-flag-sync.js.map