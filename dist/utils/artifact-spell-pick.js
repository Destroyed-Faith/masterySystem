...
import { isMartialDeliveryPickId, parseMartialDeliveryPickId, } from './artifact-power-pick.js';
/** Map artifact level (1–9 staged rows) to catalog Power Level (4 / 10 / 16). */
export function artifactLevelToTemplateRank(artifactLevel) {
...
export function uiTemplateIdCanBeSpell(uiTemplateId) {
...
export function artifactPickCanBeSpell(pick, uiTemplateId) {
    const tid = String(pick.powerTemplateId || '').trim();
    if (tid && activeTemplateCanBeSpell(tid))
        return true;
    if (pick.delivery === 'ranged-single' || pick.delivery === 'ranged-aoe')
        return true;
    if (uiTemplateId)
        return uiTemplateIdCanBeSpell(uiTemplateId);
    return false;
}
//# sourceMappingURL=artifact-spell-pick.js.map