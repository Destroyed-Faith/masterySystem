...
export const SAFE_HAVEN_REST_INFO = 'Safe Haven Rest: HP, Stress, Scars, Stones, Mastery Charges, Skills, Reroll Points and Echo uses fully restored. You may create, replace, or dismiss Minor Magic Items.';
/** Actor updates for one Safe Haven Rest (no flags / side effects). */
export function buildSafeHavenRestUpdates(system) {
    const skillsSpent = {};
    ...
    if (hpBars.length > 0) {
        updates['system.health.bars'] = hpBars.map((b) => ({ ...b, current: b.max }));
    ...
    if (stressBars.length > 0) {
        updates['system.stress.bars'] = stressBars.map((b) => ({ ...b, current: b.max }));
