...
export function metersToSceneDistance(rangeMeters) {
    ...
    const u = String(grid?.units ?? canvas.scene?.grid?.units ?? "m").toLowerCase();
 ...
 */
export function gridStepsBetweenCenters(from, to, maxSteps) {
    const grid = canvas.grid;
    if (!grid || grid.type === CONST.GRID_TYPES.GRIDLESS)
        return null;
    ...
    for (let step = 1; step <= cap; step++) {
        ...
        for (const o of frontier) {
            for (const n of gn(o)) {
                ...
                if (visited.has(k))
                    continue;
                if (cand.i === goal.i && cand.j === goal.j)
                    return step;
                visited.add(k);
                next.push(cand);
            }
...
export function masteryPowerMaxSteps(statMeters) {
 ...
 */
export function isWithinMasteryPowerRange(from, to, statMeters) {
...
export function masteryAoERadiusPixels(radiusStat) {
    const grid = canvas.grid;
