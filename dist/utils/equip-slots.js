...
export function inferDefaultEquipSlotsForType(item) {
    ...
    if (t === 'weapon') {
        ...
        const innates = Array.isArray(item?.system?.innateAbilities)
            ? item.system.innateAbilities
            : [];
        const isLight = innates.some((a) => /^light\b/i.test(String(a ?? '').trim()));
...
export function inferArtifactEquipSlots(system) {
    ...
    if (newSlot) {
        if (baseProfile === 'twoHandedWeapon' || baseProfile === 'twoHandedWeaponRanged' || newSlot === 'bothHands')
