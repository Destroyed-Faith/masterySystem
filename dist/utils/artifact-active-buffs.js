...
export function getArtifactActiveBuffs(actor) {
    const effects = actor?.effects;
    if (!effects)
        return [];
    return effects.filter((e) => e.flags?.['mastery-system']?.artifactActiveBuff === true);
}
/** True when this exact artifact row is already running as a buff. */
export function isArtifactBuffActive(actor, buffKey) {
    return getArtifactActiveBuffs(actor).some((e) => e.flags?.['mastery-system']?.artifactBuffKey === buffKey);
}
/** Any Mastery active buff (power or artifact) currently maintained on the actor. */
function hasAnyMaintainedActiveBuff(actor) {
    const effects = actor?.effects;
    if (!effects)
        return null;
    for (const e of effects) {
        const flags = e.flags?.['mastery-system'];
        if (flags?.activeBuff === true && flags?.isUtility !== true)
            return e;
    }
    return null;
}
/** Parse the Growth Form Power Level out of the row effect text (fallback by row level). */
function growthPowerLevelFor(option) {
    const text = `${option.description || ''}`;
    const m = text.match(/power level\s*(\d+)/i);
    if (m)
        return Math.max(1, Math.min(16, parseInt(m[1], 10)));
 ...
 */
function growthFormMechForLevel(pl) {
    const tpl = ALL_POWER_TEMPLATES.find((t) => t.templateId === 'ab-growth-form');
    if (!tpl)
        return null;
    const key = String(Math.max(1, Math.min(16, pl)));
    const row = tpl.levels?.[key] ?? tpl.levels?.['1'];
    const mech = row?.mechanics;
    return mech && typeof mech === 'object' ? mech : null;
}
/** Absolute square token size for a Size-Stage footprint (1 hex → 1, 3 → 2, 7 → 3). */
function growthTokenSizeForFootprint(footprintHexes) {
    if (footprintHexes >= 7)
        return 3;
    if (footprintHexes >= 3)
        return 2;
 ...
 */
async function applyGrowthToTokens(actor, targetSize) {
    const snapshots = [];
    if (targetSize <= 1)
        return snapshots;
    let tokenObjects = [];
    try {
        ...
        tokenObjects = actor.getActiveTokens?.() || [];
    }
    catch {
        tokenObjects = [];
    }
    ...
    for (const tok of tokenObjects) {
        ...
        if (!perScene.has(scene.id))
            perScene.set(scene.id, { scene, updates: [] });
...
export async function restoreGrowthFromEffect(effect) {
    const flags = effect?.flags?.['mastery-system'];
    const snapshots = Array.isArray(flags?.growthTokenSizes)
    ...
    for (const snap of snapshots) {
        ...
        if (!scene)
            continue;
        if (!perScene.has(snap.sceneId))
            perScene.set(snap.sceneId, { scene, updates: [] });
 ...
 */
export async function activateArtifactActiveBuff(actor, artifactItem, option) {
    if (!actor || !artifactItem || !option)
        return false;
    ...
    if (existing) {
        ui.notifications?.warn(`Cannot activate ${option.name}: ${existing.name || 'Another active buff'} already uses your maintained Active Buff slot.`);
        return false;
    }
    const isGrowth = String(option.artifactRowSpecial || '').toLowerCase().includes('growth');
    const masteryRank = getMasteryRank(actor);
    const currentRound = getCurrentRound();
    let growthTokenSizes = [];
    ...
    if (isGrowth) {
        ...
        const footprint = Number(growthMech?.growthForm?.footprintHexes) || 1;
        growthStageLabel = String(growthMech?.growthForm?.sizeStage || 'Growth Form');
    ...
    const mechanicsSnapshot = isGrowth && growthMech
        ? ({ ...growthMech, applyWhen: 'activeBuff-active' })
        : undefined;
    const effectData = {
        name: option.name,
    ...
    try {
        ...
        if (isGrowth) {
            lines.push(`<p>Growth Form: <strong>${esc(growthStageLabel)}</strong> — the token grows to <strong>${esc(sizeLabel)}</strong>.</p>`);
        }
...
//# sourceMappingURL=artifact-active-buffs.js.map