...
export function buildLevels(factory) {
    const out = {};
    for (let i = 1; i <= 16; i++) {
        out[String(i)] = { lvl: i, ...factory(i) };
    }
    return out;
}
/** Helper for Movement templates: identical shape per level, distance varies. */
export function movementRow(distanceM, effectText) {
    return {
        type: 'Movement',
        range: { kind: 'self' },
        aoe: { shape: 'none' },
...
export function reactionRow(partial) {
    return {
        ...
        effect: { text: partial.effectText },
...
export function activeRow(partial) {
    return {
        ...
        aoe: partial.aoe ?? { shape: 'none' },
        duration: partial.duration ?? { kind: 'instant' },
        effect: { text: partial.effectText, dice: partial.dice },
        specials: partial.specials ?? [],
        mechanics: {
