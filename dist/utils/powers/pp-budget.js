...
export function specialCost(tier, x, opts) {
 ...
 */
export function rangeCost(rangeM) {
    if (!Number.isFinite(rangeM) || rangeM <= RANGE_FREE_BASELINE_M)
        return 0;
 ...
 */
export function maxSpecialForBudget(tier, budget, opts) {
    let x = 0;
    while (specialCost(tier, x + 1, opts) <= budget) {
        x += 1;
        if (x > 64)
            break; // safety – no Active should ever realistically need 64+
    }
    return x;
}
/**
 ...
 */
export function solveDamageRow(tier, lvl, opts = {}) {
...
//# sourceMappingURL=pp-budget.js.map