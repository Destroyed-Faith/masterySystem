...
export function skillUsePointsPerBox(masteryRank) {
    return Math.max(1, Math.floor(Number(masteryRank) || 1));
}
export function buildSkillUseBoxes(rating, spent, masteryRank) {
    const pointsPerUse = skillUsePointsPerBox(masteryRank);
    const rated = Math.min(Math.max(0, Math.floor(Number(rating) || 0)), calculateMaxSkillRank(masteryRank));
...
//# sourceMappingURL=skill-use-boxes.js.map