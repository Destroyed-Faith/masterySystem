...
export function escapeXpHistoryHtml(value) {
    return String(value ?? '')
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
export function historyChangeKey(change, details) {
...
function stepCost(category, from, to) {
...
function signedForEntry(entry, amount) {
    ...
    if (kind === 'spend')
        return -Math.abs(amount);
    if (kind === 'adjust' && /refund/i.test(String(entry.note || '')))
        return Math.abs(amount);
...
export function expandHistoryRows(entries) {
...
function actorArtifactItems(actor) {
    const items = actor?.items;
    if (!items)
        return [];
    if (typeof items.filter === 'function') {
        return Array.from(items.filter((i) => i.type === 'artifact'));
    }
    if (Array.isArray(items))
        return items.filter((i) => i.type === 'artifact');
    if (Array.isArray(items.contents)) {
        return items.contents.filter((i) => i.type === 'artifact');
    }
    return [];
}
...
function artifactSystemLevel(item) {
    return Math.max(1, Math.floor(Number(item?.system?.level) || 1));
}
function loggedArtifactNet(rows, artifactId, name) {
...
export function inferMissingArtifactHistoryEntries(actor, existing) {
    ...
    const inferred = [];
    const note = localizeXpHistory('MASTERY.xp.history.inferredArtifact', 'Inferred from current artifact level');
    let ts = Date.now();
...
export function historyEntriesForActor(actor) {
    const stored = Array.isArray(actor?.system?.xp?.history)
        ? [...actor.system.xp.history]
        : [];
...
export function renderXpHistoryTableHtml(actorName, entries, options) {
    ...
    html += `<p class="xp-history-hint">${escapeXpHistoryHtml(localizeXpHistory('MASTERY.xp.history.hint', 'Each row is one XP step. Drag the window corner to resize.'))}</p>`;
    ...
    if (rows.length === 0) {
        html += `<tr><td colspan="${colSpan}" class="empty-message">${escapeXpHistoryHtml(localizeXpHistory('MASTERY.xp.history.empty', 'No history entries.'))}</td></tr>`;
    }
    else {
        for (const row of rows) {
            ...
            if (showRefund) {
                if (options.canRefund(row)) {
                    html += `<td><button type="button" class="refund-history-btn" data-category="${escapeXpHistoryHtml(row.category)}" data-key="${escapeXpHistoryHtml(row.key)}" data-from="${row.from ?? ''}" data-to="${row.to ?? ''}" data-what="${escapeXpHistoryHtml(row.what)}">${escapeXpHistoryHtml(localizeXpHistory('MASTERY.xp.history.refund', 'Refund'))}</button></td>`;
                }
                else {
                    html += '<td></td>';
                }
...
export async function openXpHistoryDialog(actor, options) {
    const DialogCtor = globalThis.Dialog;
    if (!DialogCtor || !actor)
        return;
    const { canRefundHistoryRow, planHistoryRefund, refundHistoryRow } = await import('./xp-history-refund.js');
    let history = Array.isArray(actor.system?.xp?.history)
        ? [...actor.system.xp.history]
        : [];
    ...
    new DialogCtor({
        ...
        default: 'close',
        render: (html) => {
            const $html = globalThis.$ ? globalThis.$(html) : html;
            $html.find?.('.refund-history-btn')?.on?.('click', async (event) => {
                ...
                if (!plan.refundable) {
                    globalThis.ui?.notifications?.warn(plan.reason || 'This step cannot be refunded.');
                    return;
                }
                const confirmed = await DialogCtor.confirm({
                    title: localizeXpHistory('MASTERY.xp.history.refundTitle', 'Refund XP step'),
                    content: `<p>${escapeXpHistoryHtml(localizeXpHistory('MASTERY.xp.history.refundConfirm', 'Refund {label}: {current} → {target} (+{xp} XP)? Later steps on the same thing are undone too.', {
                        label: plan.label,
                ...
                ui?.notifications?.info(localizeXpHistory('MASTERY.xp.history.refunded', 'Refunded {xp} XP ({current} → {target}).', {
                    xp: String(plan.refundXp),
                    current: String(plan.current),
                    target: String(plan.target),
                }));
...
//# sourceMappingURL=xp-history.js.map