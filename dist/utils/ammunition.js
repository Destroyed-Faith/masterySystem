...
const loadLocks = new Set();
function loc(key, fallback) {
    const raw = globalThis.game?.i18n?.localize?.(`MASTERY.ammunition.${key}`);
...
function locFormat(key, data, fallback) {
    ...
    if (formatted && formatted !== `MASTERY.ammunition.${key}`)
        return formatted;
    return fallback;
}
...
export function catalogAmmunitionType(item) {
...
export function requiresAmmunition(item) {
    if (!item)
        return false;
    if (item.system?.requiresAmmunition === true)
        return true;
...
function isAmmunitionWeaponShape(item) {
    if (!item)
        return false;
    if (item.type === 'weapon')
        return String(item.system?.weaponType || '').toLowerCase() === 'ranged';
...
export function getQuiverCapacity(item) {
    const raw = Number(item?.system?.capacity);
    if (Number.isFinite(raw) && raw > 0)
        return Math.floor(raw);
    return DEFAULT_QUIVER_CAPACITY;
}
export function getQuiverCurrent(item) {
...
export function getQuiverFreeSpace(item) {
    return Math.max(0, getQuiverCapacity(item) - getQuiverCurrent(item));
}
export function formatAmmunitionDisplay(current, capacity) {
    return locFormat('display', { current, max: capacity }, `Ammunition: ${current}/${capacity}`);
}
export function quiverAmmunitionLabel(item) {
...
export function isAmmoContainerEffectActive(actor, item) {
    if (!isAmmoContainer(item) || !item)
        return false;
...
export function getItemInHandSlotLocal(actor, slotKey) {
    for (const it of collectItems(actor)) {
        if (equipmentFlags(it).slot === slotKey)
            return it;
    }
    return null;
}
export function getActiveAmmoPair(actor) {
...
export function findEquippedAmmunitionWeapon(actor) {
    const main = getItemInHandSlotLocal(actor, 'mainhand');
    const off = getItemInHandSlotLocal(actor, 'offhand');
    if (requiresAmmunition(main))
        return main;
    if (requiresAmmunition(off))
        return off;
    return null;
}
export function planAmmunitionStackSplit(quantity, maxStack) {
...
export function planQuiverLoad(current, capacity, available) {
...
export function countAmmunitionShotsForOption(option) {
    if (!option)
        return 1;
    if (option.splitAttack === true || option.npcSplitAttack === true)
        return 2;
    ...
    if (Number.isFinite(volley) && volley > 1)
        return Math.floor(volley);
    return 1;
}
function optionLooksLikeSpell(option) {
    if (!option)
        return false;
    if (option.isSpell === true || option.item?.system?.isSpell === true)
        return true;
    const tags = Array.isArray(option.tags) ? option.tags : [];
    return tags.some((t) => /\bspell\b/i.test(String(t)));
}
export function attackUsesAmmunitionWeapon(actor, option) {
    if (!actor || optionLooksLikeSpell(option))
        return false;
    ...
    if (!equipped)
        return false;
    if (option?.id === 'weapon-attack' || option?.source === 'weapon')
        return true;
    const range = Number(option?.range ?? option?.rangeMeters);
    if (option?.slot === 'attack' && Number.isFinite(range) && range > 4)
        return true;
    return false;
}
export function evaluateAmmunitionAttack(actor, shots = 1) {
    ...
    if (current <= 0)
        return { ok: false, reason: 'empty-quiver' };
    if (current < needed)
        return { ok: false, reason: 'not-enough-ammunition' };
    return { ok: true, weapon: pair.weapon, quiver: pair.quiver };
}
export function ammunitionAttackMessage(reason) {
    if (reason === 'empty-quiver')
        return loc('emptyQuiver', 'The equipped quiver is empty.');
    if (reason === 'not-enough-ammunition')
        return loc('notEnoughAmmunition', 'Not enough ammunition for this attack.');
    return loc('compatibleQuiverRequired', 'A compatible quiver is required.');
}
export function warnAmmunitionAttack(reason) {
    globalThis.ui?.notifications?.warn(ammunitionAttackMessage(reason));
}
export function gateAmmunitionAttack(actor, option) {
    if (!attackUsesAmmunitionWeapon(actor, option))
        return true;
...
export async function consumeAmmunitionForAttack(actor, shots = 1) {
    const needed = Math.max(1, Math.floor(Number(shots) || 1));
...
export async function refundAmmunitionForAttack(actor, shots = 1) {
    const pair = getActiveAmmoPair(actor);
    if (!pair)
        return;
    const needed = Math.max(1, Math.floor(Number(shots) || 1));
...
export function validateHandEquip(actor, item, slot) {
    ...
    if (requiresAmmunition(item)) {
        if (!other || other.id === item.id)
            return { ok: true };
        if (isAmmoContainer(other) && ammunitionTypesMatch(item, other))
            return { ok: true };
    ...
    if (isAmmoContainer(item)) {
        if (!other || other.id === item.id)
            return { ok: true };
        if (requiresAmmunition(other) && ammunitionTypesMatch(item, other))
            return { ok: true };
...
function isCompatibleReplacement(current, incoming) {
...
export async function loadAmmunitionIntoContainer(actor, ammo, quiver) {
    ...
    if (ammo.parent && quiver.parent && !sameActor(ammo, quiver)) {
        globalThis.ui?.notifications?.warn(loc('incompatibleAmmunition', 'That ammunition does not fit this quiver.'));
    ...
    try {
        ...
        if (plan.moved <= 0) {
            globalThis.ui?.notifications?.warn(loc('quiverFull', 'The quiver is full.'));
...
export function findAmmoContainerFromDropPath(actor, path) {
...
export function migrateItemAmmunitionFields(item) {
    if (!item)
        return null;
    const patch = {};
...
export function normalizeAmmoWeaponSetHands(actor, hands) {
    if (!hands.mainhand || hands.mainhand !== hands.offhand)
        return hands;
...
export async function migrateActorAmmunition(actor) {
    ...
    for (const item of items) {
        ...
        if (patch)
            fieldUpdates.push({ _id: item.id, ...patch });
    }
...
function foundryClone(data) {
    const fu = globalThis.foundry?.utils?.deepClone;
...
async function updateItem(item, data) {
    ...
    for (const [key, value] of Object.entries(data)) {
        if (key === 'system.currentAmmunition')
            item.system.currentAmmunition = value;
        if (key === 'system.quantity')
            item.system.quantity = value;
    }
}
async function updateEmbedded(actor, updates) {
    if (!updates.length)
        return;
...
async function refreshAmmunitionSurfaces(actor) {
    try {
...
export function resetAmmunitionLocks() {
    loadLocks.clear();
}
