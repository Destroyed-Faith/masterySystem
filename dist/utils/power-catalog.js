...
export function labelPowerSubfamily(key) {
    const k = String(key || '').trim();
    if (!k)
        return '';
    if (SUBFAMILY_LABELS[k])
        return SUBFAMILY_LABELS[k];
    return k
        .split('-')
        .map((part) => (part.length ? part[0].toUpperCase() + part.slice(1) : part))
        .join(' ');
}
export function powerSubfamilyHint(key) {
 ...
 */
export function creationPowerRequirementsForMasteryRank(masteryRank) {
...
export function isDispelCatalogEntry(entry) {
    if (entry.templateId.includes('dispel'))
        return true;
    if (entry.subfamily === 'support-dispel')
        return true;
    if (entry.chosenSpecial?.key === 'dispel-magic')
        return true;
    return false;
}
