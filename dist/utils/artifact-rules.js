...
export function parseArtifactWeaponRangeMeters(raw) {
...
export function resolveArtifactWeaponKind(aw, baseProfile) {
    const fromProfile = baseProfile ? weaponBasicsForProfile(baseProfile) : null;
    if (fromProfile)
        return fromProfile.weaponType;
...
function artifactWeaponHasReach(aw) {
    ...
    const tags = [
        ...
        ...specials.map((s) => String(s?.specialId ?? s?.key ?? s)),
    ];
 ...
 */
export function formatArtifactWeaponRangeDisplay(aw, baseProfile) {
...
export function parseSpellFocusDice(value) {
 ...
 */
export function artifactSystemHasSpellFocus(system) {
    const bvs = Array.isArray(system?.baseValues) ? system.baseValues : [];
    return bvs.some((bv) => bv?.type === 'spellFocus');
}
/** Total Spell Focus bonus dice (d8) authored on an artifact `system`. */
export function spellFocusDiceFromSystem(system) {
    const bvs = Array.isArray(system?.baseValues) ? system.baseValues : [];
    let dice = 0;
    for (const bv of bvs) {
        if (bv?.type === 'spellFocus')
            dice += parseSpellFocusDice(bv?.value);
    }
    return dice;
}
// ----------------------------------------------------------------------
 ...
 */
export function ringAmuletCombinedBaseValueError(ringBaseValues, amuletBaseValues) {
