...
export function applyDamage(bars, currentBar, damage) {
    ...
    if (!Number.isFinite(remainingDamage))
        remainingDamage = 0;
 ...
 */
export function isStressTrackCollapsed(bars, currentBar) {
    if (!Array.isArray(bars) || bars.length === 0)
        return false;
    if (Math.floor(Number(currentBar) || 0) >= bars.length)
        return true;
...
export function resetStressBarsToClear(bars) {
    return (bars || []).map((b) => {
 ...
 */
export function calculateAgilityEvadeBonus(_agility) {
    return 0;
}
/**
