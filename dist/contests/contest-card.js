 ...
 */
import { buildContestRollOptions, compareContestResults, contestAttributeDice, contestAttributeLabel, contestKeepDice, contestOutcomeText, contestSideFromRoll, isContestAttributeKey, opponentAttributeChoices, surpriseContestModifiers, } from './opposed-attribute-contest.js';
...
export function contestTitle(context) {
    if (context === 'grapple')
        return 'Grapple — Opposed Attribute Contest';
    if (context === 'grapple-escape')
        return 'Grapple Escape — Opposed Attribute Contest';
    return 'Attribute Contest';
}
function diceHtml(side) {
...
function sideHtml(side, roleLabel) {
    const mods = [];
    if (side.advantage)
        mods.push('Advantage');
    if (side.disadvantage)
        mods.push('Disadvantage');
...
function opponentPromptHtml(state) {
...
export function buildContestCardHtml(state) {
 ...
 */
export function contestConsequence(context, outcome, initiatorName, opponentName) {
    if (context === 'grapple') {
...
function speakerFor(actor, token) {
    const CM = globalThis.ChatMessage;
    if (typeof CM?.getSpeaker !== 'function')
        return undefined;
    return CM.getSpeaker({ actor, token: token?.document ?? token });
}
async function rollContestSide(actor, setup, label) {
    ...
    const roll = await masteryRoll(options);
    try {
    ...
    return contestSideFromRoll(setup, Number(roll?.dice?.length ?? options.numDice), Number(options.keepDice), roll);
}
/**
 ...
 */
export async function startAttributeContest(params) {
    const { context, initiatorActor, opponentActor } = params;
    if (!initiatorActor || !opponentActor)
        return null;
    if (!isContestAttributeKey(params.attributeKey)) {
        globalThis.ui?.notifications?.warn?.('Pick a valid Attribute for the contest.');
        return null;
    }
    if (String(initiatorActor.id) === String(opponentActor.id) && !params.opponentToken) {
        globalThis.ui?.notifications?.warn?.('A creature cannot contest itself.');
        return null;
    }
    const g = globalThis;
    let attackActionSpent = false;
    let economyActor = null;
...
/*  Answer                                       */
/* -------------------------------------------- */
async function resolveContestActor(side) {
    const g = globalThis;
 ...
 */
export async function answerAttributeContest(messageId, attributeKey) {
    const g = globalThis;
    const message = g.game?.messages?.get?.(messageId);
    if (!message)
        return false;
    const flags = message.flags?.['mastery-system'] ?? {};
    const state = flags[ATTRIBUTE_CONTEST_FLAG];
    ...
    if (answersInFlight.has(messageId))
        return false;
    ...
    try {
        ...
        const opponentResult = await rollContestSide(opponentActor, opponentSetup, `${contestTitle(state.context)} — ${opponentSetup.name}`);
...
export function registerContestCardClickHandler() {
    if (registered)
        return;
    registered = true;
    const $ = globalThis.$;
    ...
    $(document)
        ...
        .on('click.msContest', '[data-action="ms-contest-answer"]', async (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        ...
        if (!ok)
            card.find('[data-action="ms-contest-answer"]').prop('disabled', false);
    });
}
/**
 ...
 */
export async function detectTargetUnaware(initiatorActor, targetActor) {
    try {
        const { actorHasSurprise } = await import('../combat/surprise.js');
        if (actorHasSurprise(targetActor))
            return true;
    }
    catch {
        /* ignore */
    ...
    try {
        ...
        if (targetUnseenByObserver(targetActor, initiatorActor))
            return true;
    }
    catch {
        /* ignore */
 ...
 */
export async function promptContestAttribute(actor, choices, opts) {
    const DialogCtor = globalThis.Dialog;
    const keep = contestKeepDice(actor);
    let best = choices[0];
    for (const key of choices) {
        if (contestAttributeDice(actor, key) > contestAttributeDice(actor, best))
            best = key;
    }
    ...
    return new Promise((resolve) => {
        ...
        try {
            const dialog = new DialogCtor({
                ...
                render: (html) => {
                    try {
                        const $html = globalThis.$?.(html) ?? html;
                        setTimeout(() => {
...
//# sourceMappingURL=contest-card.js.map