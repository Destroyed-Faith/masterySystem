 ...
 */
export async function performCheck(actor, checkData) {
 ...
 * @returns Promise resolving when check is complete
 */
export async function performCheckWithDialog(actor, baseData) {
      ...
      `,
            buttons: {
                roll: {
                    ...
                    label: 'Roll',
                    callback: async (html) => {
                        const tn = parseInt(html.find('[name="tn"]').val() || '16');
                        ...
                        const flavor = html.find('[name="flavor"]').val() || '';
                        const checkData = {
 ...
 * @returns Roll result
 */
export async function performQuickCheck(actor, attribute, tn, label, skill, declaredRaises = 0) {
 ...
 * @returns Roll result
 */
export async function performSave(actor, saveType, tn, label) {
