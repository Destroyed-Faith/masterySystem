...
export function unluckRankFromDetails(details) {
    const raw = details && typeof details === 'object' ? details.rank : undefined;
    const n = Math.floor(Number(raw) || 0);
    return Math.max(1, Math.min(3, n || 1));
}
export function unluckDiceSpec(rank) {
    const r = Math.max(1, Math.min(3, Math.floor(Number(rank) || 1)));
    if (r >= 3)
        return { formula: '2d8', divideBy: 1, label: '2d8' };
    if (r >= 2)
        return { formula: '1d8', divideBy: 1, label: '1d8' };
    return { formula: '1d8', divideBy: 2, label: '1d8 / 2' };
}
export function tokensFromUnluckDice(diceTotal, divideBy) {
...
export function collectUnluckCharacters(actors) {
    const out = [];
    for (const actor of actors ?? []) {
        if (String(actor?.type || '') !== 'character')
            continue;
        const list = actor?.system?.disadvantages;
        if (!Array.isArray(list))
            continue;
        const entry = list.find((d) => String(d?.id || '') === 'unluck');
        if (!entry)
            continue;
...
export function registerUnluckSettings() {
    const g = globalThis;
...
export function readUnluckSession() {
    const g = globalThis;
    try {
        ...
        return {
            rolled: !!raw.rolled,
            rolledAt: Math.max(0, Math.floor(Number(raw.rolledAt) || 0)),
            rolls,
            added: Math.max(0, Math.floor(Number(raw.added) || 0)),
        };
    }
    catch {
        return { ...EMPTY_SESSION };
...
async function writeUnluckSession(state) {
    const g = globalThis;
    try {
...
export async function clearUnluckSession() {
    await writeUnluckSession({ ...EMPTY_SESSION });
}
export function spendReasonLabel(reasonId) {
...
async function rollFormula(formula) {
    ...
    for (let i = 0; i < count; i++)
        total += 1 + Math.floor(Math.random() * 8);
    return { total };
}
...
//# sourceMappingURL=unluck.js.map