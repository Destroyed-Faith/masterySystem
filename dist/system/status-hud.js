...
export function statusIdFromHudTarget(target) {
    ...
    if (isAssignableStatusId(raw))
        return raw;
    if (el.dataset.action === 'effect') {
        const fallback = String(el.dataset.id || '').trim().toLowerCase();
        if (isAssignableStatusId(fallback))
            return fallback;
    }
    return '';
}
function hudRoot(html) {
    if (!html)
        return null;
    if (typeof HTMLElement !== 'undefined' && html instanceof HTMLElement)
        return html;
    const first = html?.[0];
    if (typeof HTMLElement !== 'undefined' && first instanceof HTMLElement)
        return first;
    return null;
}
function paletteToggle(target) {
    if (!target || typeof target.closest !== 'function')
        return null;
    const el = target.closest('[data-action="effects"], [data-action="statusEffects"], [data-palette="effects"]');
    if (!el)
        return null;
    if (el.dataset?.statusId || el.dataset?.action === 'effect')
        return null;
    return el;
}
function openStatusPalette(app) {
    if (typeof app?.togglePalette !== 'function')
        return;
...
function clickKey(actor, statusId) {
    return `${String(actor?.id || '')}:${statusId}`;
}
function cancelPending(actor, statusId) {
 ...
 */
export function bindStatusHudGestures(root, actor, app) {
    if (!root || !actor || root.dataset.msStatusHud === '1')
        return;
...
export function registerStatusHud() {
    const HooksApi = globalThis.Hooks;
    if (!HooksApi?.on)
        return;
    HooksApi.on('renderTokenHUD', (app, html) => {
...
//# sourceMappingURL=status-hud.js.map