...
function refreshMovementPreview(state, destX, destY) {
    ...
    if (isHexGrid && canvas.grid?.getOffset && highlightLayer) {
        try {
            ...
            const originOffset = canvas.grid.getOffset(origin.x, origin.y);
            const destOffset = canvas.grid.getOffset(dest.x, dest.y);
            if (originOffset && destOffset) {
                ...
                for (const hex of pathHexes) {
                    ...
                    if (canvas.interface?.grid && typeof canvas.interface.grid.highlightPosition === 'function') {
                        try {
                            canvas.interface.grid.highlightPosition(state.highlightId, {
                                x: hexCenterX,
                                y: hexCenterY,
        ...
        const originOffset = canvas.grid.getOffset(origin.x, origin.y);
        const destOffset = canvas.grid.getOffset(dest.x, dest.y);
        if (originOffset && canvas.interface?.grid && typeof canvas.interface.grid.highlightPosition === 'function') {
            try {
                canvas.interface.grid.highlightPosition(state.highlightId, {
                    x: origin.x,
                    y: origin.y,
        ...
        if (destOffset && canvas.interface?.grid && typeof canvas.interface.grid.highlightPosition === 'function') {
            try {
                canvas.interface.grid.highlightPosition(state.highlightId, {
                    x: dest.x,
                    y: dest.y,
