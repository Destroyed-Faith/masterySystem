 ...
 */
async function openMasteryActionDialog(token) {
    const currentAction = token.document.getFlag('mastery-system', 'currentAction') || {};
            ...
            ok: {
                ...
                callback: async (html) => {
                    ...
                    try {
                        ...
                        openCombatOptionDialog(token, { category: category, kind });
                    }
                    catch (error) {
 ...
 */
function getCombatOptionsForActor(actor, selection) {
    const options = [];
    ...
    if (selection.kind === 'maneuver') {
        ...
        for (const maneuver of availableManeuvers) {
            ...
            if (maneuver.slot !== selection.category)
                continue;
            options.push({
                id: maneuver.id,
                name: maneuver.name,
 ...
 */
function openCombatOptionDialog(token, selection) {
 ...
 */
function handleChosenCombatOption(token, option) {
