...
export function rollExplodingD8(rng = Math.random) {
    let total = 0;
 ...
 */
export function rollKeepSample(numDice, keep, rng = Math.random) {
    const n = Math.max(1, Math.floor(numDice));
    const k = Math.max(1, Math.min(n, Math.floor(keep)));
    const dice = [];
    for (let i = 0; i < n; i++)
        dice.push(rollExplodingD8(rng));
    dice.sort((a, b) => b - a);
    let sum = 0;
    for (let i = 0; i < k; i++)
        sum += dice[i];
 ...
 */
export function simulateAttackTotals(numDice, keep, samples = 3000, rng = Math.random) {
    const out = [];
    for (let i = 0; i < samples; i++)
        out.push(rollKeepSample(numDice, keep, rng));
    out.sort((a, b) => a - b);
    return out;
}
...
export function hitRate(sortedAsc, tn) {
    if (sortedAsc.length === 0)
        return 0;
    let lo = 0;
...
export function meanRaisesOnHit(sortedAsc, tn) {
    let count = 0;
    let sum = 0;
...
export function estimateWeaponDamageMean(weaponSystem) {
    ...
    if (raw) {
        ...
        let mean = 0;
        if (dice)
            mean += parseInt(dice[1], 10) * EXPLODING_D8_MEAN;
        if (flat)
            mean += parseInt(flat[1].replace(/\s+/g, ''), 10);
        if (mean > 0)
            return mean;
 ...
 */
export function extractPartyMember(actor, samples = 3000, rng = Math.random) {
...
export function buildPartyMetrics(members) {
    const pooled = [];
    for (const m of members)
        pooled.push(...m.attackTotals);
    pooled.sort((a, b) => a - b);
    return {
        members,
        size: members.length,
...
export function analyzeParty(actors, samples = 3000, rng = Math.random) {
    const members = actors.map((a) => extractPartyMember(a, samples, rng));
    return buildPartyMetrics(members);
}
//# sourceMappingURL=encounter-generator-analysis.js.map