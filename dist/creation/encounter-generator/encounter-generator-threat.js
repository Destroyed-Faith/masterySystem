 ...
 */
import { EXPLODING_D8_MEAN, hitRate, quantile, simulateAttackTotals, } from './encounter-generator-analysis.js';
...
export function buildThreatReport(party, plan, rng = Math.random) {
    const warnings = [];
    ...
    const burst = p90Hits * Math.max(0, (burstPerHitRaw - party.avgArmor) * (1 - drFraction)) +
    ...
    const lowestPc = party.members.reduce((min, m) => (m.effectiveHP < min.effectiveHP ? m : min), party.members[0] ?? { effectiveHP: party.avgHP, barCount: 4, evade: party.avgEvade });
    ...
    if (usesPersistent && !partyHasCleanse) {
        ...
        warnings.push(`Die Gruppe hat kein Cleanse. Hohes ${label} kann auch nach dem Encounter gefährlich bleiben — Anwendung reduzieren oder Counterplay einbauen.`);
    }
    const burstHL = burst / hlSize;
    if (burstHL >= 2) {
        warnings.push(`Maximaler Runde-1-Burst auf ein Ziel entspricht ${round1(burstHL)} Health Levels — kann einen Charakter fast ausschalten.`);
    }
    if (round1HL > 1.3) {
        warnings.push(`Der zerbrechlichste Charakter verliert in Runde 1 erwartbar ${round1(round1HL)} Health Levels — Erstrundendruck ggf. senken.`);
    }
    if (plan.adds) {
...
//# sourceMappingURL=encounter-generator-threat.js.map