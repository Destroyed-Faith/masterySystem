...
/* Attack rows                                                         */
/* ------------------------------------------------------------------ */
function specialDisplayName(id) {
...
export function solvedAttackToNpcRow(attack) {
    const row = {
        name: attack.name,
        attackDiceCount: Math.max(1, attack.attackPool),
        damageDiceCount: Math.max(0, attack.damageDice),
        specials: attack.specialId && attack.specialValue > 0
...
function reactionRows(enemy, solved) {
    const rows = [];
...
function movementNote(enemy, phaseIndex) {
...
export function buildForgeNpcSystem(design, enemy, phases) {
    const solvedPerPhase = phases.map((p) => p.enemies.find((e) => e.enemyId === enemy.id));
    ...
    const bioParts = [];
    if (enemy.concept.trim())
        bioParts.push(`<p>${enemy.concept.trim()}</p>`);
    bioParts.push(`<p><strong>Bewegung:</strong> ${movementNote(enemy, 0)}</p>`);
    if (first.defensePackage.defenses.ward > 0) {
        bioParts.push(`<p><strong>Ward ${first.defensePackage.defenses.ward}:</strong> Eingehende Special-Werte werden um ${first.defensePackage.defenses.ward} reduziert (im Balancing eingerechnet; am Tisch beim Anwenden von Specials abziehen).</p>`);
    }
    if (enemy.copies.enabled && enemy.copies.count > 0) {
        bioParts.push(`<p><strong>Kopien:</strong> ${enemy.copies.count}× (${enemy.copies.health === 'shared' ? 'geteilte Health' : enemy.copies.fragile ? 'unabhängig, zerbrechlich (1 Treffer)' : 'unabhängig'})${enemy.copies.attack ? ', greifen mit an' : ''}.</p>`);
    }
    const system = {
...
export function buildForgeAddSystem(add, npcMr) {
...
export function buildForgeSummaryHtml(design, solution, warnings) {
    const parts = [];
    parts.push(`<h2>${design.name || 'Encounter'}</h2>`);
    parts.push(`<p>Gruppe: ${solution.party.size} Charaktere · Phasen: ${design.phaseCount} · Hauptgegner: ${design.enemies.length} · Erwartete Gesamtdauer: <strong>~${fmt(solution.totalExpectedRounds)} Runden</strong></p>`);
    solution.phases.forEach((phase) => {
        parts.push(`<h3>Phase ${phase.phaseIndex + 1}</h3>`);
        const d = phase.durability;
        parts.push(`<p>Dauer erwartet <strong>${fmt(d.expectedPhaseRounds)}</strong> Runden (günstig ${fmt(d.favorableRounds)}, ungünstig ${fmt(d.unfavorableRounds)}, mit Opening-Burst ${fmt(d.burstRounds)}).` +
        ...
        for (const e of phase.enemies) {
            ...
            const atks = e.attacks
                .map((a) => `${a.name}: ${a.attackPool}k${a.keep}, ${a.damageDice}d8${a.specialId ? `, ${specialDisplayName(a.specialId)}(${a.specialValue})` : ''}${a.usesPerRound > 1 ? ` ×${a.usesPerRound}` : ''}${a.occupancy ? ` — AoE (typisch ${a.occupancy.typical} Ziele)` : ''}`)
                .join('<br>');
            parts.push(`<tr><td><strong>${e.enemyName}</strong></td><td>${e.health}</td><td>${defs}</td><td>${e.offensiveActions}</td><td>${atks}</td></tr>`);
        }
        parts.push('</tbody></table>');
        if (phase.adds.length) {
            parts.push('<ul>');
            for (const a of phase.adds) {
                parts.push(`<li><strong>${a.name}</strong> ×${a.count} — Auftritt Runde ${a.arrivalRound}, ${a.healthPerAdd} HP${a.attacks ? `, Angriff ${a.attacks.attackPool}k, ${a.attacks.damageDice}d8` : ', greift nicht an'}</li>`);
            }
            parts.push('</ul>');
        }
        ...
        for (const pc of o.perPc) {
            parts.push(`<tr><td>${pc.name}</td><td>${fmt(pc.expectedHlLostPerRound, 2)}</td><td>${fmt(pc.expectedHlLostPerPhase, 2)}</td><td>${fmt(pc.peakSpecialStacks, 1)}</td></tr>`);
        }
        parts.push('</tbody></table>');
        parts.push(`<p>Höchstes Risiko: <strong>${o.highestRiskPcName}</strong>. Härtester Einzeltreffer (90. Perzentil): „${o.worstSingleHitAttackName}" vs ${o.worstSingleHitTargetName} ≈ ${fmt(o.worstSingleHitQ90 * 100, 0)}% der Gesamt-HP.</p>`);
    });
    if (warnings.length) {
    ...
    for (const d of solution.diagnostics)
        parts.push(`<li><code>${d}</code></li>`);
    parts.push('</ul>');
    return parts.join('');
}
/* ------------------------------------------------------------------ */
/* World write                                                         */
/* ------------------------------------------------------------------ */
export async function applyEncounterForge(design, solution, warnings) {
    if (!game.user?.isGM) {
    ...
    const addsFolder = solution.phases.some((p) => p.adds.length > 0)
...
//# sourceMappingURL=encounter-forge-apply.js.map