...
import { expectedHitDamage, hitDamageQuantile, npcSpellCastingTn, rawHitChance, } from './combat-math.js';
import { applyExpectedSpecial, processTurnStart, totalNegativeStacks, } from './special-sim.js';
...
export function recommendActionEconomy(party, bodyCount, attackingAdds) {
    ...
    for (let i = 0; i < bodyCount; i += 1) {
        ...
        const share = Math.max(tuning.minActionsPerBody, Math.min(tuning.maxActionsPerBody, Math.round(remaining / bodiesLeft)));
        perBody.push(share);
        remaining -= share;
    }
    return {
...
export function aoeOccupancy(partySize) {
 ...
 * has one reaction.
 */
function npcConnectChance(pool, npcMr, isSpell, pc, totalHostileAttacks) {
    const reactionShare = (ENCOUNTER_TUNING.reactionUsageRate * (2 * pc.mr)) / Math.max(1, totalHostileAttacks);
    if (isSpell) {
...
function npcExpectedHitDamage(damageDice, penetration, pc, totalHostileAttacks) {
    // Guard reaction EV: +2×MR armor against a share of incoming attacks.
    const guardShare = (ENCOUNTER_TUNING.reactionUsageRate * (2 * pc.mr)) / Math.max(1, totalHostileAttacks);
 ...
 */
export function specialPressurePerPoint(id, pc, avgNpcHitDamage, avgNpcConnect) {
    ...
    switch (id) {
        ...
        case 'corrode':
            ...
            return Math.max(0, (expectedHitDamage({ dice: 4, armor: Math.max(0, pc.armor - 1), drPct: pc.drPct }) -
...
export function solveAttack(concept, ctx) {
    const tuning = ENCOUNTER_TUNING;
    ...
    if (attackPool <= 0) {
        ...
        for (let pool = tuning.minAttackPool; pool <= tuning.maxAttackPool; pool += 1) {
            const avg = views.reduce((a, v) => a + npcConnectChance(pool, ctx.npcMr, isSpell, v, ctx.totalHostileAttacks), 0) / views.length;
    ...
    const expectedHlPerUse = (dice) => {
        let hl = 0;
        for (let j = 0; j < views.length; j += 1) {
            const dmg = connects[j] * npcExpectedHitDamage(dice, penetration, views[j], ctx.totalHostileAttacks);
    ...
    if (hasSpecial && specialValue <= 0) {
        const avgHitDamage = views.reduce((a, v) => a + npcExpectedHitDamage(damageDice, penetration, v, ctx.totalHostileAttacks), 0) / views.length;
        ...
        const perPoint = members.reduce((a, m) => a + specialPressurePerPoint(concept.specialId, m, avgHitDamage, avgConnect), 0) / members.length;
        ...
        specialValue = Math.round(Math.min(tuning.maxSpecialValue, Math.max(tuning.minSpecialValue, rawValue)));
    }
    return {
...
solverHostileAttacks) {
    ...
    const poolPenaltyByRound = members.map(() => []);
    const hlTotals = members.map(() => 0);
    const perAttackThreat = members.map((_, j) => attacks.map((atk) => {
        const connect = npcConnectChance(atk.attackPool, npcMr, atk.resolution === 'spell', views[j], totalAttacks);
    ...
    for (let round = 1; round <= rounds; round += 1) {
        const appliedThisRound = members.map(() => new Map());
        members.forEach((member, j) => {
            ...
            const fraction = HEALTH_PENALTY_FRACTIONS[Math.min(barIndex, HEALTH_PENALTY_FRACTIONS.length - 1)];
            poolPenaltyByRound[j].push(fraction);
        });
    }
    const perPc = members.map((member, j) => ({
    ...
    const highestRisk = perPc.reduce((best, r) => r.expectedHlLostPerPhase > best.expectedHlLostPerPhase ? r : best);
    return {
        perPc,
...
//# sourceMappingURL=offense-solver.js.map