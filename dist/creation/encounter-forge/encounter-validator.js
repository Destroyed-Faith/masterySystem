...
export function validateEncounter(design, solution) {
...
function validatePhaseDuration(phase, label, warnings) {
    const t = ENCOUNTER_TUNING;
...
function validateDefenses(design, phase, label, warnings) {
    const t = ENCOUNTER_TUNING;
    for (const enemy of phase.enemies) {
        ...
        const biggest = contributions.reduce((best, c) => (c.preventedPerRound > best.preventedPerRound ? c : best), contributions[0]);
...
function validateOffense(phase, label, warnings) {
    const t = ENCOUNTER_TUNING;
...
function validateActionEconomy(phase, label, warnings, solution) {
    const t = ENCOUNTER_TUNING;
    const totalHostile = phase.enemies.reduce((a, e) => a + e.offensiveActions, 0) + phase.actionEconomy.addActions;
...
function validateEscalation(phase, label, warnings) {
...
//# sourceMappingURL=encounter-validator.js.map