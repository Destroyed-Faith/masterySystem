...
import { solveDefenses, solvePhaseHealth, } from './defense-solver.js';
import { recommendActionEconomy, simulateNpcOffense, solveAttack, } from './offense-solver.js';
import { defaultAttackConcept } from './encounter-model.js';
/* ------------------------------------------------------------------ */
/* Helpers                                                             */
/* ------------------------------------------------------------------ */
function addArrivalRound(add, phaseIndex) {
    switch (add.arrival.type) {
        ...
        case 'reinforcement': {
            ...
            if (t.kind === 'round')
                return phaseIndex === 0 ? Math.max(1, t.round) : null;
            if (t.kind === 'phaseStart')
                return t.phase - 1 === phaseIndex ? 1 : null;
...
function averagePartyHitDamage(party) {
    let total = 0;
...
/* Main solve                                                          */
/* ------------------------------------------------------------------ */
export function solveEncounter(design, partyActors) {
    const party = analyzePartyActors(partyActors);
    return solveEncounterForParty(design, party);
}
export function solveEncounterForParty(design, party) {
    const tuning = ENCOUNTER_TUNING;
    const diagnostics = [];
    const npcMr = Math.max(1, Math.min(8, party.medianMr));
    diagnostics.push(`NPC MR = ${npcMr} (Median der Gruppe; nur Keep/Spell-TN/Recovery — kein Balancing-Hebel).`);
    const phases = [];
    let totalRounds = 0;
    for (let phaseIndex = 0; phaseIndex < design.phaseCount; phaseIndex += 1) {
        ...
        const phaseAdds = design.addGroups
            ...
            .filter((x) => x.arrival != null);
        const avgHit = averagePartyHitDamage(party);
        const addWaves = phaseAdds
        ...
        const attackingCopies = enemies.reduce((a, e) => a + (e.copies.enabled && e.copies.attack ? e.copies.count : 0), 0);
        /* ---- Action economy ---- */
        const economy = recommendActionEconomy(party, enemies.length, attackingAdds + attackingCopies);
        /* ---- Defense solving per enemy ---- */
        const defensePackages = [];
        const bodies = [];
        enemies.forEach((enemy, i) => {
            ...
            if (o.evade != null)
                pkg.defenses.evade = o.evade;
            if (o.armor != null)
                pkg.defenses.armor = o.armor;
            if (o.parry != null)
                pkg.defenses.parryStrip = o.parry;
            if (o.ward != null)
                pkg.defenses.ward = o.ward;
            if (o.damageNegation != null)
                pkg.defenses.damageNegationDice = o.damageNegation;
            if (o.damageReductionPct != null)
                pkg.defenses.drPct = o.damageReductionPct;
            if (o.spellResistance != null)
                pkg.defenses.spellResistance = o.spellResistance;
            if (o.phasingCharges != null)
                pkg.defenses.phasingCharges = o.phasingCharges;
        ...
        const healthOverrides = enemies.map((e) => e.phases[Math.min(phaseIndex, e.phases.length - 1)].overrides.health);
        ...
        const perInstanceBudget = totalWeightedInstances > 0 ? partyHlBudget / totalWeightedInstances : partyHlBudget;
        const solvedEnemies = [];
        const allPhaseAttacks = [];
        enemies.forEach((enemy, i) => {
            const phase = enemy.phases[Math.min(phaseIndex, enemy.phases.length - 1)];
            ...
            const paysSummonAction = phaseIndex === 0 &&
                design.addGroups.some((g) => g.arrival.type === 'summon' && g.arrival.summonerEnemyId === enemy.id);
            const solved = concepts.map((concept) => solveAttack(concept, {
                party,
                npcMr,
                totalHostileAttacks,
        ...
        /* ---- Add attacks ---- */
        const solvedAdds = phaseAdds.map(({ group, arrival }) => {
            let attack = null;
            if (group.attacks) {
        ...
        const hostileAfterFirstDrop = solvedEnemies.reduce((a, e) => a + e.offensiveActions, 0) -
            (solvedEnemies[0]?.offensiveActions ?? 0) +
            economy.addActions;
        diagnostics.push(`Phase ${phaseIndex + 1}: Basis-Output ${defensePackages[0]?.baselineOutputPerRound.toFixed(1)} dmg/R, ` +
...
//# sourceMappingURL=solve-encounter.js.map