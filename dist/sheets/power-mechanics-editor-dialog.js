...
function readCurrent(power, scope, rank) {
    ...
    if (scope === 'rank') {
        ...
        return (lvl?.mechanics ?? null);
    }
    return (sys.mechanics ?? null);
}
function esc(s) {
    return String(s ?? '')
...
function readFormValues(root) {
    ...
    root.querySelectorAll('[data-mech]').forEach(parse);
...
export async function openPowerMechanicsEditor({ actor, power }) {
    ...
    let scope = sys.levels?.[String(rank)]?.mechanics ? 'rank' : 'power';
    const renderBody = () => {
    ...
    const dialog = new foundry.appv1.api.Dialog({
        ...
        buttons: {
            save: {
                ...
                label: 'Save',
                callback: async (html) => {
                    const root = (html[0] ?? html).querySelector('.pme-form');
                    const jsonRaw = root.querySelector('.pme-json')?.value?.trim() ?? '';
                    const scopeSel = root.querySelector('select[name="scope"]')?.value;
                    const activeScope = scopeSel || scope;
                    let nextMech = null;
                    if (jsonRaw) {
                        try {
        ...
        default: 'save',
        render: (html) => {
            const $root = (html && typeof html.find === 'function') ? html : (html?.[0] ? $(html[0]) : null);
            if (!$root)
                return;
            $root.find('select[name="scope"]').on('change', async (ev) => {
                scope = ev.currentTarget.value;
                // Re-render body in place.
                const panels = $root.find('.pme-panels')[0];
                const current = readCurrent(power, scope, rank);
                panels.querySelector('.pme-form-panel').innerHTML = renderForm(current);
                const jsonEl = panels.querySelector('.pme-json');
                if (jsonEl)
                    jsonEl.value = current ? JSON.stringify(current, null, 2) : '';
            });
        },
    }, {
...
//# sourceMappingURL=power-mechanics-editor-dialog.js.map