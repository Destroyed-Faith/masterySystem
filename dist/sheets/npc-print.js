...
function formatCompactRangeTag(atk) {
    const kind = String(atk.npcRangeKind || '').toLowerCase();
    const aoeM = Math.floor(num(atk.npcAoeRadiusM, 0));
    if (kind === 'ranged') {
        const longM = Math.floor(num(atk.npcRangeMeters, 24));
 ...
 */
export function formatNpcCompactAttackPlayLine(atk, opts) {
    const index = opts.index ?? 0;
    const name = String(atk.name || '').trim() || (index === 0 ? 'Waffenangriff' : `Power ${index + 1}`);
    ...
    if (aoe !== '—')
        parts.push(aoe);
    if (atk.npcIsSpell)
        parts.push(`Spell TN ${opts.castingTn}`);
    if (atk.npcSplitAttack)
        parts.push('Split');
    ...
    if (specials)
        parts.push(specials.replace(/,\s*/g, ' · '));
    if (apr > 1)
        parts.push(`×${apr}`);
 ...
 */
export function buildNpcCompactPrintContext(actor) {
    const system = actor?.system ?? {};
    const name = String(actor?.name ?? system?.bio?.name ?? 'NPC').trim() || 'NPC';
    const masteryRank = Math.max(1, Math.floor(num(system?.mastery?.rank, 1)));
    const castingTn = 8 * masteryRank;
    ...
    const strips = collectPhaseSources(system).map((src) => {
        ...
        const initiative = iniNet === 0 ? `${masteryRank}d8` : `${masteryRank}d8 ${iniSigned}`;
 ...
 */
export async function openNpcPrintSheet(actor, options = {}) {
