...
import { calculateShopCost, validateShopPurchases } from '../utils/initiative.js';
export class InitiativeShopDialog extends Dialog {
     ...
     */
    static async show(actor, rawInitiative) {
     ...
     */
    static _activateListeners(html, data) {
        const { rawInitiative } = data;
        // Update summary whenever values change
        const updateSummary = () => {
            const extraMovement = parseInt(html.find('[name="extraMovement"]').val() || '0');
     ...
     */
    static _parseFormData(html, rawInitiative) {
        const extraMovement = parseInt(html.find('[name="extraMovement"]').val() || '0');
        const initiativeSwap = html.find('[name="initiativeSwap"]').is(':checked');
        const extraAttack = html.find('[name="extraAttack"]').is(':checked');
        const purchases = {
