...
import { buildStoneProgressionSlots, chunkLifetimeSlots, deriveLifetimeXp, permanentColorlessCount, readAssignments, usesV099Stones, } from '../progression/v099-rules.js';
...
import { STONE_POWERS_BY_ATTRIBUTE, effectiveStoneSupportPrefillTier, isPremiumStonePower, stonePowerRankCost, } from '../stones/stone-powers.js';
 ...
 */
const STONE_PRINT_TIER_LAYOUTS = ['t1', 't2', 't3', 't4'];
...
export function buildCharacterPrintContext(actor, options = {}) {
    ...
    const allStoneGroups = STONE_GROUPS.map(({ key, label }) => {
        ...
        return {
            ...
            powers: orderPowersRampFirst(list, (p) => isPremiumStonePower(String(p.id))).map((p) => {
...
function buildPrintLifetimeProgression(system) {
    ...
    const slots = buildStoneProgressionSlots(lifetimeXp ?? 0, assignments, undefined, colorless, slotOrder, 'visible').map((slot) => lifetimeXp == null
