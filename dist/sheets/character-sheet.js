...
import { canGiveBackMinorMagic, dismissMinorMagicItem, giveMinorMagicItemToActor, listMinorMagicGiveTargets, minorMagicSheetView, readMinorMagicFlag, returnMinorMagicItemToCreator, useMinorMagicItem, } from '../utils/minor-magic-items.js';
import { applySafeHavenRest, SAFE_HAVEN_REST_INFO } from '../utils/safe-haven-rest.js';
import { buildConsumableSlotView, equippedConsumableActionRows, equipConsumableToSlot, isConsumableItem, itemOccupyingConsumableSlot, listCarriedConsumableItems, readConsumableSlotIndex, transferConsumableToActor, unequipConsumable, useEquippedConsumable, validateUnequipConsumable, } from '../utils/consumable-slots.js';
...
import { buildCancelSkillsRedistributeUpdates, buildFinishSkillsRedistributeUpdates, buildStartSkillsRedistributeUpdates, canStartSkillsRedistribute, getCreationSkillBudget, isSkillsRedistributing, nextCreationSkillValue, prevCreationSkillValue, validateCreationSkillAllocation, } from '../utils/skills-redistribute.js';
...
import { ATTRIBUTE_ABBREV, ATTRIBUTE_KEYS, buildStoneProgressionSlots, canPlacePermanentStone, permanentStonesFromLifetimeXp, readAssignments, stoneConcentrationCap, usesV099Stones, } from '../progression/v099-rules.js';
...
function localizeSheet(key, fallback, data) {
...
export class MasteryCharacterSheet extends BaseActorSheet {
    ...
    #bindSheetGmMenu(html) {
        this.#sheetGmMenuAbort?.abort();
        this.#sheetGmMenuAbort = null;
        const menu = html.find('.sheet-gm-menu')[0];
        if (!menu)
            return;
        const toggle = menu.querySelector('.sheet-gm-menu-toggle');
        const list = menu.querySelector('.sheet-gm-menu-list');
        if (!toggle || !list)
            return;
        const setOpen = (open) => {
    ...
    async #onCombatSenseGrantToggle(event) {
        ...
        if (el.disabled)
            return;
        const senseId = String($(el).data('sense-id') || '').trim();
        if (!senseId)
            return;
        const data = normalizeCombatSensesData(this.actor.system?.combatSenses);
    ...
    async #onCombatSenseDarkvisionToggle(event) {
        event.stopPropagation();
        if (!this.isEditable || !this.actor.isOwner)
            return;
        const el = event.currentTarget;
        const current = !!normalizeCombatSensesData(this.actor.system?.combatSenses).hasDarkvision;
    ...
    async _prepareContext(options) {
        ...
        if (this.actor.type === 'character') {
            ...
            const flag = (key) => {
                try {
                    if (this.actor.getFlag?.('mastery-system', key) === true)
                        return true;
                }
                catch {
                    /* stored flags below */
                }
                return this.actor.flags?.['mastery-system']?.[key] === true;
            };
        ...
        return {
            ...
            equipSlots: slotDefs.map((def) => {
                ...
                return {
                    ...
                    emptyUnarmed: isHand,
                    emptyLabel: isHand
                        ? (globalThis.game?.i18n?.localize?.('MASTERY.weaponSets.emptyHandLabel') || 'Unarmed')
                        : 'Drop',
    ...
    #prepareSkills(skillValues = {}, skillsSpent = {}) {
        ...
        const masteryRank = Math.max(1, Math.floor(Number(this.actor.system?.mastery?.rank) || 2));
    ...
    activateListeners(html) {
        ...
        html
            ...
            .on('click.skills-redistribute', (e) => {
            e.preventDefault();
            e.stopPropagation();
        ...
        const actorType = String(this.actor?.type || '');
        ...
        html.find('.minor-expressions-open').on('click', async (ev) => {
            ev.preventDefault();
            if (!this.actor.isOwner) {
                ui.notifications?.warn('Only the owner can choose Minor Expressions.');
                return;
            }
         ...
         */
        bindReliableControlClick(html, '.open-languages-btn', async (ev) => {
            ev.preventDefault();
        ...
        bindReliableControlClick(html, '.v099-open-respec', (ev) => {
            ev.preventDefault();
            void this.#openV099Migration();
        });
        bindReliableControlClick(html, '.v099-open-lifetime', (ev) => {
            ev.preventDefault();
        ...
        html.on('click', '[data-action="switchWeaponSet"]', async (ev) => {
            ...
            if (!canCurrentUserUpdateDocument(this.actor))
                return;
            const raw = String(ev.currentTarget?.dataset?.weaponSet || '');
            const target = raw === 'unarmed' ? 'unarmed' : raw === '2' ? 2 : raw === '1' ? 1 : null;
            if (!target)
                return;
            await swapWeaponSet(this.actor, target);
        });
        bindReliableControlClick(html, '[data-action="openArtifactEvolution"]', async (ev) => {
            ev.preventDefault();
            if (!this.actor.isOwner)
                return;
        ...
        bindReliableControlClick(html, '[data-action="openProgressionHub"]', async (ev) => {
            ev.preventDefault();
            if (!this.actor.isOwner)
                return;
        ...
        html.on('click.summonBonds', '.js-sheet-summon-bond-dissolve', async (ev) => {
            ...
            const confirmed = typeof globalThis.foundry?.applications?.api?.DialogV2?.confirm === 'function'
                ...
                : globalThis.confirm?.(`Dissolve this Summon Bond? Bound Stones return to the owner. Existing summon tokens will be removed.\n\n${bond.name}`);
            if (!confirmed)
                return;
        ...
        const invEquipSelector = '.tab.equipment .df-enc-band .df-draggable-item, .tab.equipment .df-consumable-slot .df-draggable-item';
        const ContextMenuCls = foundry.applications?.ux?.ContextMenu;
        const rootEl = (html?.[0] ?? this.element ?? null);
        ...
        if (rootEl) {
            ...
            const hint = localizeSheet('MASTERY.inventory.slotFillHint', 'Right-click to choose an item from inventory');
    ...
    async #onAttributeIncreaseXP(event) {
        ...
        if (effectiveAfter > MAX_ATTRIBUTE) {
            ...
            ui.notifications?.warn('This attribute cannot exceed maximum value (40).');
            return;
        }
        /**
    ...
    async #onSkillRoll(event) {
        ...
        const pool = getSkillRollDicePool(this.actor, skillKey, rollOptions.attributeKey);
      ...
      <form class="mastery-dialog-form">
        ...
        <div class="md-group">
          ...
          <select name="challengeMR" id="skill-roll-challengeMR" class="md-select">
            ...
            .map((mr) => `<option value="${mr}" ${mr === challengeMR ? 'selected' : ''}>MR ${mr} (Standard ${standardTnForMasteryRank(mr)})</option>`)
            .join('')}
          </select>
        </div>
    ...
    async #onGmResetRerollPoints(event) {
        event.preventDefault();
        if (!game.user?.isGM) {
            ui.notifications?.warn('Only a GM can reset Reroll Points.');
            return;
        }
        const system = this.actor.system;
        const max = Math.max(0, Number(system.faithFractures?.maximum) || 0);
        ...
        if (cur === max) {
            ui.notifications?.info(`${this.actor.name} is already at ${cur}/${max} Reroll Points.`);
            return;
        }
        await this.actor.update({ 'system.faithFractures.current': max });
        ui.notifications?.info(`${this.actor.name}: Reroll Points ${cur} → ${max}.`);
        this.render();
    }
     ...
     */
    async #onGmEditXp(event) {
        event.preventDefault();
        event.stopPropagation();
        if (!game.user?.isGM) {
            ui.notifications?.warn('Only a GM can edit XP.');
            return;
        }
        const system = this.actor.system ?? {};
        const points = system.points ?? {};
      ...
      <form class="gm-edit-xp-form">
        ...
        const newAvail = await new Promise((resolve) => {
            let settled = false;
        ...
        if (newAvail === null || newAvail === current)
            return;
        ...
        ui.notifications?.info(`${this.actor.name}: XP auf ${newAvail} gesetzt (${delta >= 0 ? '+' : ''}${delta}).`);
        this.render();
    }
    /**
    ...
    async #onSkillSpendPoint(event) {
        ...
        if (isSkillsRedistributing(this.actor)) {
            ui.notifications?.warn('Finish or cancel skill redistribution before spending XP on skills.');
            return;
        }
        // Check if user is owner
        if (!this.actor.isOwner) {
    ...
    async #onSkillRefundPoint(event) {
        ...
        event.stopPropagation();
        if (isSkillsRedistributing(this.actor)) {
            ui.notifications?.warn('Finish or cancel skill redistribution before adjusting skills with XP.');
            return;
        }
        // Check if user is owner
        if (!this.actor.isOwner) {
        ...
        return new Promise((resolve) => {
            new Dialog({
                ...
                render: (html) => {
                    ...
                    const updateRaiseTn = () => {
                        const tn = parseInt($html.find('[name="tn"]').val()) || 0;
                        const raises = parseInt($html.find('[name="raises"]').val()) || 0;
     ...
     */
    #armSheetButtonsAgainstTooltipSteal(html) {
    ...
    #onResetCreationAttributes(event) {
        ...
        new Dialog({
            ...
            content: '<p class="mastery-reset-attrs-msg">Set <strong>all seven attributes</strong> to <strong>2</strong>. You can then pick the distribution again (2×4, 2×3, 3×2). Skills, powers, and disadvantages are unchanged.</p>',
            buttons: {
                reset: {
    ...
    async #onV099StoneSlot() {
        ...
        const storedLife = sys?.progression?.lifetimeXp;
        const needsLife = this.actor.getFlag?.('mastery-system', V099_LIFETIME_FLAG) === true
    ...
    async #onCreationSkillIncrease(event) {
        ...
        if (creationComplete && !redistributing)
            return;
        if (!this.actor.isOwner && !game.user?.isGM) {
    ...
    async #onStartSkillsRedistribute() {
        if (!this.actor.isOwner && !game.user?.isGM) {
...
<p><em>Only available when the character has no XP yet. Cancel restores the previous allocation.</em></p>`,
    ...
    async #onCancelSkillsRedistribute() {
        if (!this.actor.isOwner && !game.user?.isGM) {
    ...
    async #onFinalizeCreation(event) {
        ...
        const c3 = attrValues.filter((v) => v === 3).length;
        const c2 = attrValues.filter((v) => v === 2).length;
        if (c4 !== 2 || c3 !== 2 || c2 !== 3) {
            ui.notifications?.error(`Attributes must be 2×4, 2×3, 3×2. Currently: ${c4}×4, ${c3}×3, ${c2}×2`);
            return;
        }
    ...
    async #giveMinorMagicItem(item) {
        ...
        ui.notifications?.info(ownLimit
    ...
    async #returnMinorMagicItem(item) {
        const result = await returnMinorMagicItemToCreator(this.actor, item);
        if (!result.ok) {
            ui.notifications?.warn(result.error);
            return false;
        }
        ui.notifications?.info(`${item.name} returned to ${result.creator.name}. They can give it again. It still counts against their Minor Magic limit.`);
        return true;
    }
    /** @override */
    ...
    async _onDrop(event) {
        ...
        const dropPath = (event.composedPath?.() || [event.target]);
        // Get dropped item
    ...
    #onEquipSlotContextMenu = (ev) => {
        if (!this.isEditable || !this.actor.isOwner)
            return;
        const target = ev.target;
        const slotEl = target?.closest?.('.df-equip-slot, .df-consumable-slot');
        const root = this.element;
        if (!slotEl || !root?.contains(slotEl))
            return;
        const filledTile = target?.closest?.('.df-draggable-item');
        if (filledTile && slotEl.contains(filledTile))
            return;
        ev.preventDefault();
        ev.stopPropagation();
        ev.stopImmediatePropagation();
    ...
    #openEquipSlotFillMenu(event, slotEl) {
        ...
        const slotLabel = slotEl.querySelector('.df-slot-label')?.textContent?.trim() ||
        ...
        menu.innerHTML = `<div class="df-equip-slot-fill-menu__title">${escapeSheetHtml(title)}</div>${rows.length ? rows.join('') : `<div class="df-equip-slot-fill-menu__empty">${escapeSheetHtml(emptyText)}</div>`}`;
        const pad = 8;
        ...
        menu.addEventListener('click', async (clickEv) => {
            const button = clickEv.target?.closest?.('[data-item-id]');
            if (!button)
                return;
        ...
        const entries = [
            ...
            },
            {
                ...
                callback: async (target) => {
                    ...
                    if (!item)
                        return;
                    if (await this.#giveMinorMagicItem(item))
                        await this.render(false);
                },
            },
            {
                ...
                callback: async (target) => {
                    ...
                    if (!item)
                        return;
                    if (await this.#returnMinorMagicItem(item))
                        await this.render(false);
                },
            },
            {
