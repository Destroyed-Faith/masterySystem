...
function buildNpcStatusRows(raw) {
    return coerceStatusEffectsArray(raw).map((entry, index) => {
        ...
        const name = displayNpcSpecialName(rawName || id) ||
            rawName ||
            id ||
            `Status ${index + 1}`;
...
function resolveNpcAttackWritePath(el) {
    if (!el)
        return '';
    const fromData = String(el.dataset?.npcAttackPath || '').trim();
    if (isValidNpcAttackWritePath(fromData))
        return fromData;
    const fromName = String(el.name || '')
        .replace(/\.npcRangeKind$/, '')
        .replace(/\.npcAoeRadiusM$/, '')
        .replace(/\.npcRangeMeters$/, '')
        .replace(/\.npcAoeShape$/, '')
        .trim();
    if (isValidNpcAttackWritePath(fromName))
        return fromName;
    const card = el.closest?.('.npc-attack-card');
    if (!card)
        return '';
    ...
    if (card.classList.contains('npc-attack-card--base')) {
        if (hasPhase && Number.isFinite(pi))
            return `system.phases.${pi}.npcBaseAttack`;
        return 'system.npcBaseAttack';
    }
    if (attackRaw !== undefined && attackRaw !== null && String(attackRaw) !== '' && Number.isFinite(ai)) {
        if (hasPhase && Number.isFinite(pi))
            return `system.phases.${pi}.attackValues.${ai}`;
 ...
 */
function readLiveNpcAttackFormFields(root, path) {
    const out = {};
    if (!root || !path)
        return out;
    const pick = (suffix, asNumber = false) => {
        const el = root.querySelector(`[name="${path}.${suffix}"]`);
        if (!el)
            return;
...
export class MasteryNpcSheet extends MasteryCharacterSheet {
    ...
    static DEFAULT_OPTIONS = {
        ...
        actions: {
            ...
            msNpcPrintCompactSheet: function () {
    ...
    async _prepareContext(options) {
        ...
        if (isNpcLike && context.system) {
            ...
            if (isSummon) {
                ...
                if (phases.length > 0) {
                    ...
                    context.system.phases = phases.map((phase, i) => ({
                        ...phase,
                        ...
                        attackSlots: clampNpcAttackSlots(phase?.attackSlots ??
                        ...
                        attackValues: Array.isArray(phase?.attackValues)
                            ? phase.attackValues.map((r) => normalizeNpcAttackRowForContext(r))
                            : normalizeAttackValuesArray(phase?.attackValues).map((r) => normalizeNpcAttackRowForContext(r)),
                    }));
                }
        ...
        if (isNpcLike && context.system) {
            ...
            context.npcCatalogReactions = listNpcCatalogReactions();
            context.npcStandardReactions = NPC_STANDARD_REACTIONS;
            ...
            const actorDoc = context.actor;
            const tokenDisp = Number(actorDoc?.token?.disposition ??
            ...
            const statusList = coerceStatusEffectsArray(this.actor?.system?.statusEffects ?? context.system.statusEffects);
            context.system.statusEffects = statusList;
            context.npcStatusEffects = buildNpcStatusRows(statusList);
            context.hasNpcStatusEffects = statusList.length > 0;
        }
        return context;
    }
    /**
    ...
    async #persistNpcAttackTargeting(path, patch, reason) {
        ...
        const { coerceNpcPhasesArray, npcAttackUsageKey, sanitizeNpcAttackTargetingFields, } = await import('../utils/npc-attack-model.js');
        const { logNpcTargeting, logNpcTargetingRow, logNpcAttackListDump, logNpcActorTargetingCompare, npcTargetingSnap, } = await import('../utils/npc-targeting-debug.js');
        const beforeRow = path.split('.').reduce((acc, key) => (acc == null ? acc : acc[key]), actor);
        logNpcTargeting(`SHEET WRITE begin — ${reason}`, {
            ...
            phasesType: actor.system?.phases == null
                ? 'null'
        ...
        const liveFields = readLiveNpcAttackFormFields(this.element, path);
        ...
        logNpcTargeting(`SHEET WRITE payload — ${reason}`, {
            ...
            updatePreview: {
                npcBaseAttack: update['system.npcBaseAttack']
                    ? npcTargetingSnap(update['system.npcBaseAttack'])
                    : undefined,
                phases0Base: Array.isArray(update['system.phases'])
                    ? npcTargetingSnap(update['system.phases'][0]?.npcBaseAttack)
                    : undefined,
            },
        });
        const targets = [actor];
        ...
        const world = globalThis.game?.actors?.get(actor.id);
        const tokenActor = actor.isToken
            ? actor
    ...
    _prepareSubmitData(event, form, formData, updateData) {
        ...
        const existingSystem = this.actor?.system ?? {};
        ...
        if (data.system.phases != null) {
            ...
            if (submitPhases.length === 0 && existingPhases.length > 0) {
                console.warn('[MS NPC Targeting] FORM SUBMIT — empty coerced phases; keeping existing', {
                    ...
                    rawType: data.system.phases == null
                        ? 'null'
                        : Array.isArray(data.system.phases)
                            ? 'array'
                            : typeof data.system.phases,
                });
                submitPhases = existingPhases.map((p) => dup(p));
            }
            data.system.phases = submitPhases.map((phase, i) => {
                ...
                return {
                    ...
                    attackSlots: clampNpcAttackSlots(phase.attackSlots ?? prev.attackSlots ?? existingSystem.attackSlots),
                };
            });
        }
        if (data.system.attackSlots != null || existingSystem.attackSlots != null) {
            data.system.attackSlots = clampNpcAttackSlots(data.system.attackSlots ?? existingSystem.attackSlots);
        }
    ...
    activateListeners(html) {
        ...
        html.find('select.npc-aoe-radius, [name$=".npcAoeRadiusM"]').on('change', (ev) => {
            ...
            if (!path) {
                console.warn('[MS NPC Targeting] AoE change ignored — could not resolve attack path', {
                    name: radEl.name,
                    dataPath: radEl.dataset?.npcAttackPath,
                });
                return;
            }
        ...
        html.find('.npc-reaction-catalog-filter').on('input', (ev) => {
            const input = ev.currentTarget;
            const term = String(input.value || '').trim().toLowerCase();
            const select = input
                .closest('.npc-reactions-catalog')
                ?.querySelector('select.npc-reaction-catalog-select');
            if (!select)
                return;
     ...
     */
    async #onNpcDispositionChange(event) {
        ...
        try {
            ...
            for (const token of tokens) {
                ...
                if (!doc?.update)
                    continue;
                if (Number(doc.disposition) === value)
                    continue;
                try {
    ...
    async #onRemoveStatusEffect(event) {
        ...
        const list = coerceStatusEffectsArray(this.actor.system?.statusEffects);
        if (!Number.isFinite(index) || index < 0 || index >= list.length)
            return;
        const next = list.filter((_, i) => i !== index);
        await this.actor.update({ 'system.statusEffects': next });
    }
    async #onReduceStatusEffect(event) {
        event.preventDefault();
        event.stopPropagation();
    ...
    async #mutateNpcReactions(phaseRaw, mutator, extra, updateOptions) {
        ...
        if (phaseIndex != null && Number.isFinite(phaseIndex)) {
            ...
            if (extra?.slots != null)
                phases[phaseIndex].npcReactionSlots = extra.slots;
        ...
        const patch = { 'system.npcReactions': rows };
        if (extra?.slots != null)
            patch['system.npcReactionSlots'] = extra.slots;
    ...
    async #onNpcReactionAddStandard(event) {
        event.preventDefault();
        event.stopPropagation();
        const btn = event.currentTarget;
        const wrap = btn.closest('.npc-reactions-standard');
        const select = wrap?.querySelector('select.npc-reaction-standard-select');
        ...
        await this.#mutateNpcReactions(phaseRaw, (rows) => {
            if (rows.some((r) => r.source === 'basic' && r.basicId === row.basicId))
                return rows;
            return [...rows, row];
        });
    }
    async #onNpcReactionAddCatalog(event) {
        event.preventDefault();
        event.stopPropagation();
        const btn = event.currentTarget;
        const wrap = btn.closest('.npc-reactions-catalog');
        const select = wrap?.querySelector('select.npc-reaction-catalog-select');
        const rankEl = wrap?.querySelector('select.npc-reaction-catalog-rank');
        ...
        if (Number.isFinite(rankN) && rankN >= 1)
            row.rank = Math.min(16, rankN);
        const phaseRaw = $(btn).data('phase-index');
        await this.#mutateNpcReactions(phaseRaw, (rows) => {
            if (rows.some((r) => r.source === 'catalog' && r.templateId === row.templateId))
                return rows;
            return [...rows, row];
        });
    }
    async #onNpcReactionDelete(event) {
        event.preventDefault();
        event.stopPropagation();
        const $t = $(event.currentTarget);
    ...
    async #onPhaseAdd(event) {
        ...
        if (phases.length === 0) {
            const rootSlots = clampNpcAttackSlots(system.attackSlots ?? sumNpcAttackSlotsFromPowers(system));
            phases.push({
                name: 'Phase 1',
        ...
        const prevSlots = clampNpcAttackSlots(prev.attackSlots ?? system.attackSlots ?? sumNpcAttackSlotsFromPowers(system));
        phases.push({
