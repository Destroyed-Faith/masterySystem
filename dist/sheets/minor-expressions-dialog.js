 ...
 */
import { MINOR_EXPRESSION_ATTRIBUTES, MINOR_EXPRESSION_MIN_ATTRIBUTE, listMinorExpressionsByAttribute, sanitizeMinorExpressionIds, tierBodyForExpression } from '../utils/minor-expressions.js';
const ATTR_LABEL = {
    might: 'Might',
    agility: 'Agility',
    intellect: 'Intellect',
...
function buildContent(defsByAttr, attrValues, selected, _focusAttribute) {
    ...
    for (const attr of MINOR_EXPRESSION_ATTRIBUTES) {
        ...
        const sectionClass = _focusAttribute === attr ? 'me-attr-section me-attr-section--focus' : 'me-attr-section';
            ...
            render: (html) => {
                ...
                $html.find('.me-pick').on('change', function () {
                    ...
                    if (countChecked($html) > mr) {
                        ...
                        globalThis.ui?.notifications?.warn(`Maximal ${mr} Minor Expression${mr === 1 ? '' : 's'} (Mastery Rank).`);
                    }
                    syncCount();
                });
                syncCount();
