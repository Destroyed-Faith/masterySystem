...
function tierScaleBlock(def, attrVal, eligible) {
    if (!eligible)
        return '';
    ...
    return new Promise((resolve) => {
        const dialog = new Dialog({
                ...
                save: {
                    ...
                    callback: async (html) => {
                        ...
                        if (prev !== next && sanitized.length > cleaned.length) {
                            globalThis.ui?.notifications?.info('Minor Expressions were adjusted to Mastery Rank or attribute values.');
                        }
                        ...
                        if (ffMax > 0) {
                            ...
                            if (added > 0 || removed > 0) {
                                ...
                                if (afterRefund < added) {
                                    globalThis.ui?.notifications?.warn(`Not enough Reroll Points: ${added} new Minor Expression${added === 1 ? '' : 's'} cost 1 Reroll Point each (available: ${afterRefund}).`);
                                    resolve();
                                    return true;
                                }
                                const newCurrent = afterRefund - added;
                                updateData['system.faithFractures.current'] = newCurrent;
                                globalThis.ui?.notifications?.info(`Reroll Points: ${current} → ${newCurrent} (${added ? `−${added} new` : ''}${added && removed ? ', ' : ''}${removed ? `+${removed} refunded` : ''}).`);
                            }
                        }
            ...
            render: (html) => {
                ...
                const syncTierPreview = ($select) => {
                    const attrVal = Math.floor(Number($select.data('attr-val')) || 0);
                    const tier = Number($select.val());
                    ...
                    if (!def || !MINOR_EXPRESSION_TIERS.includes(tier))
                        return;
