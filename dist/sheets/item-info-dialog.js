...
function formatArtifactWeaponSpecialLines(specials) {
    ...
    for (const s of specials) {
        ...
        if (s && typeof s === 'object' && typeof s.specialId === 'string') {
            out.push(formatEffectReference(s));
        }
    }
    return out;
}
function artifactGearSlotLabel(value) {
...
function embeddedArtifactPowerRows(powers) {
    const out = [];
...
export class ItemInfoDialog extends BaseDialog {
    ...
    async _prepareContext(_options) {
        ...
        if (t === 'artifact') {
            ...
            const showLegacyBonuses = atk !== 0 || def !== 0 || (dmgLegacy !== '' && dmgLegacy !== '—');
            const powersArr = Array.isArray(sys.powers) ? sys.powers : [];
            const kindRaw = sys.artifactKind;
            const kind = kindRaw === 'weapon' || kindRaw === 'armor' || kindRaw === 'shield' || kindRaw === 'gear'
                ? kindRaw
                : 'weapon';
            ...
            const shieldTypePretty = shieldTypeKey === 'parry'
                ? 'Parry'
