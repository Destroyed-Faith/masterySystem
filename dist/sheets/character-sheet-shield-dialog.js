...
export async function showShieldCreationDialog(actor) {
    ...
    const dialog = new foundry.applications.Dialog({
        ...
        buttons: {
            add: {
                ...
                callback: async (html) => {
                    const $html = html;
                    const shieldName = $html.find('#shield-select').val();
                    if (!shieldName) {
                        ui.notifications?.warn('Please select a shield.');
                        return false;
                    }
                    const option = $html.find('#shield-select option:selected')[0];
...
//# sourceMappingURL=character-sheet-shield-dialog.js.map