...
function wallConst(group, key, fallback) {
    const table = globalThis.CONST?.[group];
...
export function coordsOf(wall) {
    const c = Array.isArray(wall?.c) ? wall.c : wall?.document?.c;
    const nums = (c ?? [0, 0, 0, 0]).map((n) => Number(n) || 0);
    return [
        { x: nums[0] ?? 0, y: nums[1] ?? 0 },
        { x: nums[2] ?? 0, y: nums[3] ?? 0 },
    ];
}
export function classifyWall(wall) {
    ...
    if (door === doors.DOOR || door === doors.SECRET) {
        const doorState = ds === states.OPEN ? 'open' : ds === states.LOCKED ? 'locked' : 'closed';
    ...
    if (isWindow)
        return { kind: 'window', doorState: null, secret: false };
    return { kind: 'wall', doorState: null, secret: false };
}
export function readWallView(wall) {
...
export function readSceneWalls(scene) {
    const walls = scene?.walls;
    const list = walls?.contents ?? (typeof walls === 'object' ? Array.from(walls) : []);
    return list.filter((w) => w).map(readWallView);
}
function typeFields(kind, doorState = 'closed') {
    const doors = wallDoorTypes();
    const states = wallDoorStates();
    const sense = wallSense();
    const move = wallMove();
    if (kind === 'door') {
        const ds = doorState === 'open' ? states.OPEN : doorState === 'locked' ? states.LOCKED : states.CLOSED;
        return {
            door: doors.DOOR,
            ds,
            move: move.NORMAL,
...
export function wallPayload(a, b, kind, options = {}) {
    ...
    if (options.suggestionId)
        flags.suggestionId = options.suggestionId;
    return {
        c: [a.x, a.y, b.x, b.y],
        ...fields,
...
//# sourceMappingURL=wall-adapter.js.map