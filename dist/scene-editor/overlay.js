...
export class SceneEditorOverlay {
    ...
    attach() {
        ...
        if (typeof parent.sortChildren === 'function')
            parent.sortChildren();
    }
    detach() {
        this.clearMask();
    ...
    draw(state) {
        ...
        if (!g || !this.root)
            return;
        g.clear();
        if (state.layers.mask && state.debug?.mask)
            this.drawMask(state);
        else
            this.clearMask();
        if (state.layers.lines && state.debug?.lines) {
            for (const line of state.debug.lines)
                this.stroke(g, line.a, line.b, 0x90a4ae, 1, [4, 4], 0.45);
        }
        if (state.layers.hints) {
            for (const hint of state.hints)
                this.drawHint(g, hint, state.selectedHintIds.includes(hint.id));
        }
        ...
        if (state.layers.confirmed) {
            for (const wall of state.walls) {
                ...
                if (selected)
                    this.handles(g, wall.a, wall.b, state.handleRadius);
            }
        }
        ...
        if (state.snap)
            this.dot(g, state.snap, SCENE_EDITOR_COLORS.snap, 6);
    }
    drawKindMark(g, wall) {
    ...
    stroke(g, a, b, color, width, dash, alpha) {
        if (typeof g.setStrokeStyle === 'function') {
            g.setStrokeStyle({ width, color, alpha, cap: 'round' });
            if (dash.length && typeof g.setDash === 'function')
                g.setDash(dash);
            g.moveTo(a.x, a.y);
            g.lineTo(b.x, b.y);
            g.stroke();
            if (typeof g.setDash === 'function')
                g.setDash([]);
            return;
        }
...
export function screenHandleRadius() {
...
export function hitHandle(p, a, b, radius) {
    if (distance(p, a) <= radius * 1.4)
        return 'a';
    if (distance(p, b) <= radius * 1.4)
        return 'b';
    return null;
}
