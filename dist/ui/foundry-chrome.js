...
function unlockFadedControls(root) {
    const nodes = root.querySelectorAll('[inert], [aria-hidden="true"], button, .control-tool, [data-tool], a.item, li');
    nodes.forEach((el) => {
...
function onChromePointerDown(ev) {
    ...
    const control = stack.find((el) => el instanceof HTMLElement &&
...
function onChromePointerOver(ev) {
    unlockChromeAtPoint(ev.clientX, ev.clientY);
}
function bindChromeRoot(el) {
    const node = el;
    if (node.__msFadedUnlock)
        return;
