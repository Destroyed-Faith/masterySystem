 ...
 */
export function getActiveSpells(actor) {
    const spells = [];
 ...
 */
export async function dispelSpell(caster, target, spellLevel) {
    ...
    const result = await performQuickCheck(caster, 'intellect', tn, `Dispel ${spellToDispel.name}`);
    if (result.success) {
 ...
 */
export async function suppressSpells(target, duration, source) {
 ...
 */
export async function counterspell(caster, counterer, spellLevel) {
    ...
    const { performQuickCheck } = await import('../rolls/checks.js');
    const result = await performQuickCheck(counterer, 'intellect', tn, `Counterspell (Level ${spellLevel})`);
    if (result.success) {
        // Consume Reaction
 ...
 */
export function hasSpellImmunity(actor) {
 ...
 */
export function getSpellSchool(item) {
