...
function describe(el) {
    ...
    if (pointerEvents !== 'none') {
        ...
        else if (el.id === 'tooltip' || el.classList.contains('toolclip'))
            reason = 'Foundry tooltip/toolclip';
        ...
        else if ((Number(opacity) || 1) < 0.05 && area > 10_000)
            reason = 'nearly invisible but clickable';
        else if (transparentBg && area > 80_000 && !el.innerText?.trim())
...
export function sampleHitStack(x, y, limit = 12) {
...
export class HitBlockerProbe {
    ...
    enable() {
        ...
        this.ensureHud();
        ui.notifications?.info('Hit Probe ON — HUD under cursor; Ctrl+Alt+H to stop. Clicks log the blocker stack to the console.');
        console.info('%cMastery Hit Probe ON', 'color:#e94560;font-weight:bold', 'Move mouse over a dead button. Red outline = suspicious overlay. Click logs the full stack.');
    }
    disable() {
        if (!this.enabled)
            return;
    ...
    dumpAt(x, y) {
        ...
        if (blockers.length)
            console.warn('Suspicious overlays:', blockers.map(labelOf));
        console.groupEnd();
        return stack;
    }
    ensureHud() {
    ...
    handleClick(ev) {
        if (!this.enabled)
            return;
        const stack = sampleHitStack(ev.clientX, ev.clientY);
        const receiver = ev.target ?? null;
        const topClickable = stack.find((s) => s.pointerEvents !== 'none');
        console.groupCollapsed(`%cMastery Hit Probe CLICK @ (${ev.clientX}, ${ev.clientY})`, 'color:#ffd54f;font-weight:bold');
...
let singleton = null;
export function getHitBlockerProbe() {
    if (!singleton)
        singleton = new HitBlockerProbe();
    return singleton;
}
export function initializeHitBlockerProbe() {
    const probe = getHitBlockerProbe();
    probe.bindHotkey();
    Hooks.once('ready', () => {
        globalThis.game.masteryHitProbe = probe;
        console.info('Mastery Hit Probe ready — Ctrl+Alt+H or game.masteryHitProbe.enable() when a button will not click.');
    });
}
//# sourceMappingURL=hit-blocker-probe.js.map