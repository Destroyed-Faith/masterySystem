    ...
    $(document).off('click', '.roll-attack-btn').on('click', '.roll-attack-btn', async (ev) => {
        ...
        try {
            ...
            const result = await masteryRoll({
                ...
                flavor: `Roll ${numDice}d8 keep ${keepDice} vs ${game.actors?.get(flags.targetId)?.name || 'Target'}'s Evade (${currentTargetEvade}${raises > 0 ? `, ${raises} raise${raises > 1 ? 's' : ''}` : ''})${disadvantageNote}`,
