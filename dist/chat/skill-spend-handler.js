...
export function registerSkillSpendClickHandler() {
    ...
    Hooks.on('renderChatMessageHTML', (message, htmlRaw) => {
        try {
            ...
            if (spendButton.data('handler-attached'))
                return;
            ...
            spendButton.off('click.skill-spend').on('click.skill-spend', async (event) => {
                ...
                const keptIndices = rollResult.keptIndices || [];
    ...
    Hooks.on('renderChatMessageHTML', (message, htmlRaw2) => {
        try {
            ...
            spendButton.off('click.vitality-spend').on('click.vitality-spend', async (event) => {
                event.preventDefault();
                event.stopPropagation();
                const button = $(event.currentTarget);
                const actorId = button.data('actor-id');
                const spendAmount = parseInt(button.data('spend') || '0');
                const messageFlags = message.flags?.['mastery-system'] || {};
                if (!messageFlags.isSaveRoll || !messageFlags.actorId)
                    return;
                const isAuthor = message.user?.id === game.user?.id;
                const isGM = game.user?.isGM;
                if (!isAuthor && !isGM) {
