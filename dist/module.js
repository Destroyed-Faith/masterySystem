...
Hooks.once('init', async function () {
    ...
    Hooks.on('combatStart', async (combat) => {
        try {
            ...
            if (game.user?.isGM) {
                const { syncCombatTurnToHighestInitiativeFirst } = await import('./combat/initiative-roll.js');
...
function registerHandlebarsHelpersImmediate() {
    ...
    Handlebars.registerHelper('powerSpecials', function (specials, chosenKey) {
    ...
    const refreshPlacedTokenStatusIcons = () => {
        void import('./system/assign-status.js').then(({ syncTokenStatusIcons }) => {
            ...
            for (const token of tokens) {
                if (token?.actor)
                    void syncTokenStatusIcons(token.actor);
            }
        });
    };
